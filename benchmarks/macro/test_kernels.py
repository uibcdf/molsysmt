...
def run_coordinate_benchmarks(output_path: str | None = None) -> list[dict]:
    ...
    # Public APIs
    harness_center_public = BenchmarkHarness(
        "macro_get_center_public_api", iterations=50, repeats=5
    )
    harness_rmsd_public = BenchmarkHarness(
        "macro_get_rmsd_public_api", iterations=50, repeats=5
    )
    harness_distances_public = BenchmarkHarness(
        "macro_get_distances_public_api", iterations=10, repeats=5
    )

    # Raw JIT Kernels
    harness_center_jit = BenchmarkHarness(
        "macro_get_center_jit_kernel", iterations=100, repeats=5
    )
    harness_rmsd_jit = BenchmarkHarness(
        "macro_get_rmsd_jit_kernel", iterations=100, repeats=5
    )
    harness_distances_jit = BenchmarkHarness(
        "macro_get_distances_jit_kernel", iterations=20, repeats=5
    )
    ...
    res_rmsd_pub = harness_rmsd_public.run(
        warmup_func=lambda: msm.structure.get_rmsd(
            system, reference_structure_index=0, selection="all"
        ),
        timed_func=lambda: msm.structure.get_rmsd(
            system, reference_structure_index=0, selection="all"
        ),
    )
