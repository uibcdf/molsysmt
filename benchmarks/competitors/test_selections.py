...
def run_selection_benchmarks(
    ...
    h_msm_simple = BenchmarkHarness(
        "competitor_selection_molsysmt_simple", iterations=50, repeats=5
    )
    h_mdt_simple = BenchmarkHarness(
        "competitor_selection_mdtraj_simple", iterations=50, repeats=5
    )
    h_mda_simple = BenchmarkHarness(
        "competitor_selection_mdanalysis_simple", iterations=50, repeats=5
    )
    ...
    h_msm_complex = BenchmarkHarness(
        "competitor_selection_molsysmt_complex", iterations=50, repeats=5
    )
    h_mdt_complex = BenchmarkHarness(
        "competitor_selection_mdtraj_complex", iterations=50, repeats=5
    )
    h_mda_complex = BenchmarkHarness(
        "competitor_selection_mdanalysis_complex", iterations=50, repeats=5
    )
    ...
    results.append(
        h_msm_simple.run(warmup_func=select_msm_simple, timed_func=select_msm_simple)
    )
    results.append(
        h_mdt_simple.run(warmup_func=select_mdt_simple, timed_func=select_mdt_simple)
    )
    results.append(
        h_mda_simple.run(warmup_func=select_mda_simple, timed_func=select_mda_simple)
    )

    print("Running complex selection benchmarks...")
    results.append(
        h_msm_complex.run(warmup_func=select_msm_complex, timed_func=select_msm_complex)
    )
    results.append(
        h_mdt_complex.run(warmup_func=select_mdt_complex, timed_func=select_mdt_complex)
    )
    results.append(
        h_mda_complex.run(warmup_func=select_mda_complex, timed_func=select_mda_complex)
    )
