...
def test_an_unknown_residue_still_raises():
    ...
    with pytest.raises(UnknownGroupInTableError):
        group_table_value(
            VALUES, "HED", table="charge", caller="molsysmt.physchem.get_charge"
        )
...
def test_the_failure_names_the_residue_the_table_the_caller_and_the_remedy():
    ...
    assert "molsysmt.physchem.get_charge" in reported, (
        "which function could not proceed"
    )
...
@pytest.mark.parametrize(
    ("group_name", "expected"), [("ALA", 0.0), ("ASP", -1.0), ("LYS", 1.0)]
)
