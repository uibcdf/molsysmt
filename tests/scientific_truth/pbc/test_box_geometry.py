...
def test_triclinic_box_has_exact_lengths_angles_and_volume(
    ...
    box_values = np.array([[[2.0, 0.0, 0.0], [1.0, sqrt_three, 0.0], [0.0, 0.0, 3.0]]])
...
def test_box_constructor_matches_the_canonical_triclinic_matrix(
    public_six_decimal_atol,
):
    ...
    expected = np.array([[[2.0, 0.0, 0.0], [1.0, np.sqrt(3.0), 0.0], [0.0, 0.0, 3.0]]])
...
def test_triclinic_volume_from_lengths_and_angles_matches_closed_form(
    float64_kernel_atol,
):
