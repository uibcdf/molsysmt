...
def test_pbc_vector_helpers_on_orthogonal_and_triclinic_boxes():
    ...
    tri = np.array(
        [[2.0, 0.0, 0.0], [0.5, 2.0, 0.0], [0.1, 0.2, 2.0]], dtype=np.float64
    )

    assert box_is_orthogonal_single_structure(ortho)
    assert not box_is_orthogonal_single_structure(tri)
    assert np.array_equal(
        box_is_orthogonal(np.stack([ortho, tri])), np.array([True, False])
    )
    ...
    pbc = wrap_to_pbc_vector_single_structure(
        np.array([-0.1, 3.1, 4.2], dtype=np.float64), ortho, None, None
    )
    assert np.all(pbc >= -1e-8)
    assert np.all(pbc < np.array([2.0, 3.0, 4.0]) + 1e-8)

    center_pbc = wrap_to_pbc_center_vector_single_structure(
        np.array([2.8, -1.2, 4.1], dtype=np.float64), ortho, None, None
    )
    assert center_pbc.shape == (3,)

    tri_mic = wrap_to_mic_vector_single_structure(
        np.array([2.2, -1.4, 0.6], dtype=np.float64), tri, None, None
    )
