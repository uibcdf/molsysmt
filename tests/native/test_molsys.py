...
def build_minimal_molsys(n_structures=2):
    ...
    molsys.structures.coordinates = puw.quantity(
        np.arange(n_structures * 4 * 3, dtype=np.float64).reshape((n_structures, 4, 3)),
        "nm",
    )
    molsys.structures.structure_id = np.array(
        [str(ii) for ii in range(n_structures)], dtype=object
    )
    return molsys
...
def test_extract_subsets_atoms_and_structures():
    ...
    extracted = molsys.extract(
        atom_indices=[0, 1], structure_indices=[1], skip_digestion=True
    )
...
def test_remove_atoms_and_structures():
    molsys = build_minimal_molsys(n_structures=3)
    trimmed = molsys.remove(
        atom_indices=[2, 3], structure_indices=[0, 2], skip_digestion=True
    )
...
def test_add_missing_bonds_appends_inferred_pairs_without_replacing_existing(
    monkeypatch,
):
    molsys = build_minimal_molsys(n_structures=1)
