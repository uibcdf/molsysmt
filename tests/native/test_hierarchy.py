...
from molsysmt.native._hierarchy import (
    fallback_ids,
...
def test_infer_component_indices_matches_rebuild_components(t4_h5msm_molsys):
    ...
    assert group_component_index.tolist() == rebuilt.groups["component_index"].to_list()
...
def test_project_component_queries_follow_prepared_topology(t4_h5msm_molsys):
    ...
    group_component_index = topology.groups["component_index"].to_numpy(dtype=np.int64, na_value=-1)
...
def test_project_molecule_queries_follow_prepared_topology(t4_h5msm_molsys):
    ...
    group_index_from_atom = topology.atoms["group_index"].to_numpy(dtype=np.int64, na_value=-1)
    component_index_from_group = topology.groups["component_index"].to_numpy(dtype=np.int64, na_value=-1)
    ...
    expected_chain = []
    chain_index_from_group = topology.groups["chain_index"].to_numpy(dtype=np.int64, na_value=-1)
...
def test_prepare_topology_for_chain_queries_populates_missing_columns(t4_h5msm_molsys):
    ...
    assert "chain_index" in prepared.groups.columns
...
def test_project_chain_queries_follow_prepared_topology(t4_h5msm_molsys):
    ...
    group_chain_index = topology.groups["chain_index"].to_numpy(dtype=np.int64, na_value=-1)
