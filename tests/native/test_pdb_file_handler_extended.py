...
from molsysmt.native.pdb_file_handler import guess_format_version, parse_format33
...
def test_hetatm_record_present():
    ...
    pdb = parse_format33(_path("181l"))
    hetatm_records = [
        r for m in pdb.coordinate.model for r in m.record if r.recordName == "HETATOM"
    ]
...
def test_regression_3c0f_source():
    ...
    handler = PDBFileHandler(_path("3c0f"))
    assert (
        handler.entry.title.source[0].organism_scientific
