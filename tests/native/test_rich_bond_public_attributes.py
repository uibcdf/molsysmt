...
def test_topology_get_set_and_capability_deliver_every_rich_bond_attribute():
    ...
    assert msm.get(topology, element="bond", selection=[1], bond_is_aromatic=True) == [
        True
    ]
    assert msm.select(topology, "bond_is_aromatic==True", element="bond") == [1]
...
def test_molsys_delegates_rich_bond_delivery_and_explicit_state_selection():
    ...
    assert msm.get(molsys, element="bond", chemical_state=0, bond_order=True) == [1, 0]
    assert msm.get(molsys, element="bond", chemical_state=1, bond_order=True) == [2]
    msm.set(
        molsys,
        element="bond",
        chemical_state=1,
    ...
    assert msm.get(molsys, element="bond", chemical_state=1, bond_evidence=True) == [
        "user_defined"
    ]
...
def test_optional_bond_capability_distinguishes_support_from_instance_values():
    ...
    assert msm.has_attribute(topology, "fractional_bond_order", include_none=True)
    assert msm.get(topology, element="bond", fractional_bond_order=True) == [None]
...
def test_h5msm_v04_delivers_public_rich_bond_attributes(tmp_path):
    ...
    assert msm.get(str(filename), element="bond", bond_id=True) == ["shared", "shared"]
    assert msm.get(str(filename), element="bond", bond_is_aromatic=True) == [
        False,
        True,
    ]
    assert msm.get(str(filename), element="bond", bond_stereo_atom_indices=True) == [
        [0, 1],
        [2, 3],
    ]
