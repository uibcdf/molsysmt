...
def test_legacy_structures_association_migrates_to_molsys_authority():
    ...
    legacy.structures._chemical_state_indices = pd.array([0, -1, pd.NA], dtype="Int64")
...
def test_nullable_and_invalid_associations_fail_closed():
    ...
    with pytest.raises(
        StructuralInconsistencyError, match="no chemical-state association"
    ):
        msm.get(
            molsys,
            element="atom",
...
def test_structure_resolver_drives_get_select_and_rejects_mixed_states():
    ...
    with pytest.raises(
        StructuralInconsistencyError, match="span multiple chemical states"
    ):
        msm.select(
            molsys,
            "formal_charge!=0",
