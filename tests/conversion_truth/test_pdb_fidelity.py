...
def _pdb_link_line(atom1, group1, chain1, group_id1, atom2, group2, chain2, group_id2):
    line = [" "] * 80
    line[0:6] = "LINK  "
...
def test_pdb_numeric_capacity_is_reported_and_rejected(rich_molsys):
    ...
    assert "coordinates" not in {issue.attribute for issue in selected_report.issues}
...
def test_pdb_alternate_locations_survive_selection_and_native_roundtrip():
    ...
    np.testing.assert_allclose(
        ...
        puw.get_value(
            source.structures.alternate_location[0][1]["coordinates"], to_unit="nm"
        ),
        atol=5.1e-5,
    )
...
def test_file_pdb_reports_presence_of_alternate_locations(tmp_path):
    assert has_atoms_with_alternate_locations(PDB_WITH_ALTERNATE_LOCATIONS) is True
    ...
    assert has_atoms_with_alternate_locations(str(pdb_without_alternates)) is False
...
def test_pdb_multimodel_alternate_locations_share_a_canonical_atom_axis():
    ...
    assert [list(item) for item in roundtrip.structures.alternate_location] == [
        [0],
        [0],
    ]
...
def test_pdb_formal_charge_and_link_are_imported_as_explicit_chemical_state_data():
    ...
    restored = msm.convert(pdb_text, to_form="molsysmt.MolSys", get_missing_bonds=False)
    ...
    assert (
        restored.topology._reference_chemical_state.connectivity_completeness
        == "partial"
    )
    ...
    assert (
        roundtrip.topology._get_chemical_state_atom_attribute("formal_charge").iloc[0]
        == 2
    )
...
def test_pdb_openmm_discovered_bonds_are_marked_as_inferred(monkeypatch):
    ...
    restored = msm.convert(pdb_text, to_form="molsysmt.MolSys", get_missing_bonds=True)
    bonds = restored.topology._get_chemical_state_bonds()
...
def test_pdb_report_exposes_ambiguous_explicit_bond_endpoints():
    ...
    assert any(
        issue.attribute == "bonded_atoms" and issue.kind == "adapter_limitation"
        for issue in report.issues
    )
...
def test_pdb_report_exposes_malformed_charge_conect_and_bioassembly_records():
    pdb_text = (
        ...
        + _pdb_atom_line(1, "CA", "ALA", "A", 1, (0.0, 0.0, 0.0), "C", charge="X+")
        + "CONECT    1   99\n"
        + "END\n"
    )
