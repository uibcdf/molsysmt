...
def test_structuresdict_preserves_native_thermodynamic_series(rich_molsys):
    ...
    source.potential_energy = puw.quantity(np.array([-10.0, -9.0, -8.0]), "kJ/mol")
    source.kinetic_energy = puw.quantity(np.array([2.0, 2.5, 3.0]), "kJ/mol")
    ...
    assert report.outcome == "equivalent"
    assert np.allclose(
        puw.get_value(target["temperature"], to_unit="K"), [290, 295, 300]
    )
    assert np.allclose(
...
def test_structuresdict_native_routes_are_exhaustive_and_subset_safe(rich_molsys):
    ...
    assert "coordinates" in {issue.attribute for issue in topology_report.issues}
