...
def test_add_forbidden_plane_region_1():
    ...
    fi = msm.thirds.openmm.forces.add_forbidden_plane_region(
        system,
        point="[1,2,3] nm",
        normal_vector=[1, 0, 0],
        width="0.5 nm",
    ...
    assert (
        f_expression
        == "0.5*Kf*q^2; q = min(0, d-wf); d = periodicdistance(x, 0, 0, px, 0, 0)"
    )
...
def test_add_forbidden_plane_region_4():
    ...
    assert not f_pbc
    assert f_expression == (
        "0.5*Kf*q^2; q = min(0, d-wf); d = abs((x-px)*vx+(y-py)*vy+(z-pz)*vz);"
    )
