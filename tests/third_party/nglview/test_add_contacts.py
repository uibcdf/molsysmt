...
def test_add_contacts_1():
    ...
    for ii in range(n_contacts):
        ...
        check_6 = np.allclose(aux["kwargs"]["position1"], coordinates[contacts[ii][0]])
        check_7 = np.allclose(aux["kwargs"]["position2"], coordinates[contacts[ii][1]])
        check_all_contacts = all(
            [check_1, check_2, check_3, check_4, check_5, check_6, check_7]
        )
        if not check_all_contacts:
            break

    assert check_all_contacts


def test_add_contacts_2():
    ...
    view = msm.view(molsys, viewer="NGLView")
    start_index = len(view._ngl_msg_archive)
    msm.thirds.nglview.add_contacts(
        view,
        selection='atom_name=="CA"',
        threshold="9 angstroms",
