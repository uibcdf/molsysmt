...
def test_add_cylinders_1():
    ...
    msm.thirds.nglview.add_cylinders(
        view,
        bottom=bottom,
        top=top,
        color="#ff0000",
    ...
    assert np.allclose(
        view._ngl_msg_archive[0]["kwargs"]["position2"], [10.0, 0.0, 0.0]
    )
    ...
    assert np.allclose(
        view._ngl_msg_archive[1]["kwargs"]["position2"], [0.0, 10.0, 0.0]
    )
...
def test_add_cylinders_2():
    ...
    contacts = msm.structure.get_contacts(
        ...
        threshold="9 angstroms",
        output_type="pairs",
        output_indices="atom",
    )

    start = msm.get(
        molsys, element="atom", selection=np.array(contacts[0])[:, 0], coordinates=True
    )
    end = msm.get(
        molsys, element="atom", selection=np.array(contacts[0])[:, 1], coordinates=True
    )
    ...
    for ii in range(n_contacts):
        ...
        assert np.allclose(
            view._ngl_msg_archive[ii + offset]["kwargs"]["position1"], start[ii]
        )
        assert np.allclose(
            view._ngl_msg_archive[ii + offset]["kwargs"]["position2"], end[ii]
        )
...
def test_add_cylinders_3():
    ...
    start, groups_start = msm.get(
        molsys,
        element="atom",
        selection=np.array(contacts[0])[:, 0],
        coordinates=True,
        group_index=True,
    )
    end, groups_end = msm.get(
        molsys,
        element="atom",
        selection=np.array(contacts[0])[:, 1],
        coordinates=True,
        group_index=True,
    )
    ...
    msm.thirds.nglview.add_cylinders(
        ...
        color_values=groups_start,
        color_values_2=groups_end,
        colormap="viridis",
    )
    ...
    assert np.allclose(
        view._ngl_msg_archive[offset]["kwargs"]["color"], (0.267004, 0.004874, 0.329415)
    )
    assert np.allclose(
        view._ngl_msg_archive[offset]["kwargs"]["color2"],
        (0.276022, 0.044167, 0.370164),
    )

    assert view._ngl_msg_archive[-1]["args"][0] == "cylinder"
    assert np.allclose(
        view._ngl_msg_archive[-1]["kwargs"]["color"], (0.926106, 0.89733, 0.104071)
    )
    assert np.allclose(
        view._ngl_msg_archive[-1]["kwargs"]["color2"], (0.993248, 0.906157, 0.143936)
    )
