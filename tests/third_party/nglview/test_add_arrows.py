...
def test_add_arrows_1():
    ...
    msm.thirds.nglview.add_arrows(
        view,
        vectors="[2,2,2] angstroms",
        origin='atom_name=="CA"',
        color="#808080",
        radius="0.2 angstroms",
    )
    n_arrows, coordinates = msm.get(
        molsys,
        element="atom",
    ...
    for ii in range(n_arrows):
        ...
        check_5 = np.allclose(
            aux["kwargs"]["position1"],
            puw.get_value(coordinates[0, ii], to_unit="angstroms"),
        )
        check_6 = np.allclose(
            aux["kwargs"]["position2"],
            puw.get_value(coordinates[0, ii], to_unit="angstroms")
...
def test_add_arrows_2():
    ...
    for ii in range(n_arrows):
        ...
        check_6 = np.allclose(
            aux["kwargs"]["position2"], puw.get_value(arrows[ii], to_unit="angstroms")
        )
        check_all_arrows = all([check_1, check_2, check_3, check_4, check_5, check_6])
        if not check_all_arrows:
            break
...
def test_add_arrows_3():
    ...
    coordinates = msm.get(
        view,
        element="atom",
        selection='atom_name=="CA" and group_index in [10,30,60,90]',
        coordinates=True,
    )
    msm.thirds.nglview.add_arrows(
        view,
        vectors=arrows,
        origin='atom_name=="CA" and group_index in [10,30,60,90]',
        color="#0B6E4F",
    ...
    for ii in range(n_arrows):
        ...
        check_6 = np.allclose(
            aux["kwargs"]["position2"],
            puw.get_value(coordinates[0, ii] + arrows[ii], to_unit="angstroms"),
        )
