...
def test_matmul_and_transpmatmul():
    ...
    v = _v()
    assert np.allclose(
        rb.matmul(m, v, backend="numba"),
        rb.matmul(m, v, backend="rust"),
        rtol=0.0,
        atol=1e-14,
    )
    assert np.allclose(
        rb.transpmatmul(m, v, backend="numba"),
        rb.transpmatmul(m, v, backend="rust"),
        rtol=0.0,
        atol=1e-14,
    )
...
def test_normalize_vector():
    ...
    assert np.allclose(nb, rs, rtol=0.0, atol=1e-14)
    assert np.isclose(np.linalg.norm(rs), 1.0, rtol=0.0, atol=1e-14)


def test_inverse_matrix_3x3_lower_triangular():
    m = np.array([[6.0, 0.0, 0.0], [1.2, 6.0, 0.0], [0.8, 0.6, 6.0]])
    assert np.allclose(
        rb.inverse_matrix_3x3(m, backend="numba"),
        rb.inverse_matrix_3x3(m, backend="rust"),
        rtol=0.0,
        atol=1e-14,
    )
...
def test_quaternion_to_rotation_matrix():
    ...
    assert np.allclose(rs @ rs.T, np.eye(3), rtol=0.0, atol=1e-12)
...
def test_rodrigues_rotation():
    ...
    assert np.allclose(nb, rs, rtol=0.0, atol=1e-14)
    assert np.isclose(np.linalg.norm(rs), np.linalg.norm(v), rtol=0.0, atol=1e-12)
