"""Parity: Rust (non-periodic) distance family vs the Numba oracle (bit-for-bit).
...
def test_single_system():
    ...
    assert np.allclose(rs, np.transpose(rs, (0, 2, 1)), atol=1e-12)
    assert np.allclose(np.einsum("sii->si", rs), 0.0)
