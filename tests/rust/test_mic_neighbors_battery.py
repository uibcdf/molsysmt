...
over a wide (±3) image shell — not against Numba, which is not a valid oracle on skewed
boxes. Every result must match exactly (integer neighbour sets) or to a tight tolerance
...
def test_self_neighbour_list_matches_ground_truth(box_name, cutoff, sort):
    ...
    off, idx, dist = rb.neighbor_list_csr_multi(
        c[None], None, box=box[None], cutoff=cutoff, exclude_self=True,
        sort_by_distance=sort, backend="rust")
    got = _rust_pairs(off, idx, 120)
    truth = _truth_pairs(c, c, box, cutoff, exclude_self=True)
    assert got == truth, f"{box_name} cut={cutoff}: missing {len(truth-got)}, extra {len(got-truth)}"
...
def test_disjoint_neighbour_list_matches_ground_truth(box_name, cutoff):
    ...
    off, idx, _ = rb.neighbor_list_csr_multi(
        q[None], r[None], box=box[None], cutoff=cutoff, exclude_self=False,
        sort_by_distance=False, backend="rust")
...
def test_small_box_fewer_than_three_cells_no_double_count(box_name):
    ...
    truth = _truth_pairs(c, c, box, cutoff, exclude_self=True)
    assert got == truth, f"{box_name} small: missing {len(truth-got)}, extra {len(got-truth)}"
    for i in range(60):
        row = idx[off[i]:off[i + 1]]
...
def test_single_atom_has_no_neighbours():
    ...
    off, idx, _ = rb.neighbor_list_csr_multi(
        c[None], None, box=box[None], cutoff=2.0, exclude_self=True,
        sort_by_distance=False, backend="rust")
...
def test_atoms_on_cell_boundaries():
    ...
    box = BOXES["mild-tric"]
    fr = np.array([[0.0, 0.0, 0.0], [0.5, 0.0, 0.0], [0.999, 0.5, 0.25],
                   [0.0, 0.999, 0.75], [1.0 - 1e-12, 1.0 - 1e-12, 1.0 - 1e-12]])
...
def test_many_structures_are_independent():
    ...
    off, idx, _ = rb.neighbor_list_csr_multi(
        cs, None, box=b, cutoff=1.2, exclude_self=True, sort_by_distance=False, backend="rust")
    # compare each structure's block against its own ground truth
    na = 60
    for s in range(4):
        base = s * na
        got = {(i, int(idx[p])) for i in range(na)
...
def test_dense_mic_distance_matrix_matches_ground_truth(box_name):
    ...
    assert np.allclose(D, truth, atol=1e-9), f"{box_name}: max diff {np.abs(D-truth).max():.2e}"
...
def test_cell_list_sasa_equals_brute_force_across_boxes():
    ...
    for box_name in ["orthogonal", "mild-tric", "skewed", "skewed-2"]:
        ...
        brute = rb.get_mic_sasa(c[None], box[None], radii, sphere, probe, backend="rust")
        cell = rb.get_mic_sasa_cell_list(c[None], box[None], radii, sphere, probe, cutoff,
                                         backend="rust")
...
def test_mic_angles_use_the_true_minimum_image(box_name):
    ...
    assert np.allclose(got, truth, atol=1e-9), f"{box_name}: angles diverge from ground truth"
