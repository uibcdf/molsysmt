...
class TestGetCenterParity:
    def test_file_path_flat_selection(self, h5msm_path):
        ...
        c_e = get_center(
            h5msm_path, selection="all", structure_indices=indices, heavy_mode="off"
        )
        c_h = get_center(
            h5msm_path, selection="all", structure_indices=indices, heavy_mode="force"
        )
    ...
    def test_handler_flat_selection(self, handler):
        ...
        indices = list(range(N_FRAMES))
        c_e = get_center(
            handler, selection="all", structure_indices=indices, heavy_mode="off"
        )
        c_h = get_center(
            handler, selection="all", structure_indices=indices, heavy_mode="force"
        )
        diff = np.max(np.abs(puw.get_value(c_e) - puw.get_value(c_h)))
        assert diff == 0.0, f"center eager vs heavy max_diff={diff}"
...
class TestGetRMSDParity:
    ...
    def test_first_frame_rmsd_is_zero(self, h5msm_path):
        ...
        r = get_rmsd(
            h5msm_path, selection="all", structure_indices=[0], heavy_mode="force"
        )
...
class TestGetDistancesParity:
    ...
    def test_shape(self, h5msm_path):
        ...
        d = get_distances(
            h5msm_path,
            selection="all",
            structure_indices=list(range(5)),
            heavy_mode="force",
        )
