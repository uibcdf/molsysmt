...
from molsysmt._private.execution import (
    ChunkedExecutor,
    Reducer,
    decide_mode,
    estimate_footprint,
...
def test_memory_pressure_warning_emitted(pentalanine_h5msm, monkeypatch):
    ...
    try:
        ...
        try:
            ...
            pressure_warnings = [
                w for w in caught if issubclass(w.category, MemoryPressureWarning)
            ]
...
def test_memory_pressure_warning_not_emitted_below_threshold(pentalanine_h5msm):
    ...
    try:
        ...
        try:
            ...
            assert len(pressure_warnings) == 0, (
                "Unexpected MemoryPressureWarning emitted"
            )
        finally:
            config.memory_pressure_threshold = old_threshold
            config.emit_heavy_telemetry = old_telemetry
