...
def test_wrap_to_pbc_molsysmt_StructuresDict_30():
    ...
    box_center = puw.quantity([3.0, 3.0, 3.0], "nm")
    box = msm.pbc.get_box_with_shape(shape="truncated octahedral", length="2.0 nm")
    coordinates = np.dot(box[0].T, np.array([0.0, 0.0, 0.0])) + puw.quantity(
        [3.0, 3.0, 3.0], "nm"
    )
    coordinates = coordinates.reshape(1, 1, 3)
    molsys = {"coordinates": coordinates, "box": box}
...
def test_wrap_to_pbc_molsysmt_StructuresDict_31():
    ...
    box_center = puw.quantity([3.0, 3.0, 3.0], "nm")
    box = msm.pbc.get_box_with_shape(shape="truncated octahedral", length="2.0 nm")
    coordinates = np.dot(box[0].T, np.array([1.0, 1.0, 1.0])) + puw.quantity(
        [3.0, 3.0, 3.0], "nm"
    )
    coordinates = coordinates.reshape(1, 1, 3)
    molsys = {"coordinates": coordinates, "box": box}
...
def test_wrap_to_pbc_molsysmt_StructuresDict_32():
    ...
    box_center = puw.quantity([3.0, 3.0, 3.0], "nm")
    box = msm.pbc.get_box_with_shape(shape="truncated octahedral", length="2.0 nm")
    coordinates = np.dot(box[0].T, np.array([-0.5, -0.5, -0.5])) + puw.quantity(
        [3.0, 3.0, 3.0], "nm"
    )
    coordinates = coordinates.reshape(1, 1, 3)
    molsys = {"coordinates": coordinates, "box": box}
...
def test_wrap_to_pbc_molsysmt_StructuresDict_33():
    ...
    coordinates = coordinates.reshape(1, 1, 3)
    fin_coors = np.dot(box[0].T, np.array([-0.5, -0.5, -0.5])) + puw.quantity(
        [3.0, 3.0, 3.0], "nm"
    )
    fin_coors = fin_coors.reshape(1, 1, 3)
    molsys = {"coordinates": coordinates, "box": box}
    ...
    assert np.allclose(
        puw.get_value(molsys["coordinates"]), puw.get_value(fin_coors)
    ) or np.allclose(
        puw.get_value(molsys["coordinates"]), [[[1.33333333, 2.52859548, 3.81649658]]]
    )
...
def test_wrap_to_pbc_molsysmt_StructuresDict_34():
    ...
    for ii in [-0.5, 0.5]:
        for jj in [-0.5, 0.5]:
            for kk in [-0.5, 0.5]:
                aux = np.dot(box[0].T, np.array([ii, jj, kk])) + np.array(
                    [3.0, 3.0, 3.0]
                )
                if aux[0] < ref_min[0]:
                    ref_min[0] = aux[0]
