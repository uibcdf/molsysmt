...
def test_get_volume_from_box_cubic_geometry(met_enkephalin_pdb_molsys):
    ...
    check = np.allclose(
        msm.pyunitwizard.get_value(volume, to_unit="nm**3"), [30.4765808]
    )
    assert check
...
def test_get_volume_from_box_octahedral_geometry(met_enkephalin_pdb_molsys):
    ...
    volume = msm.pbc.get_volume_from_box(box)
    check = np.allclose(
        msm.pyunitwizard.get_value(volume, to_unit="nm**3"), [23.4608828]
    )
    assert check
...
def test_get_volume_from_box_dodecahedral_geometry(met_enkephalin_pdb_molsys):
    ...
    volume = msm.pbc.get_volume_from_box(box)
    check = np.allclose(
        msm.pyunitwizard.get_value(volume, to_unit="nm**3"), [21.5501970]
    )
    assert check
