...
def test_get_lengths_and_angles_from_box_cubic_geometry(met_enkephalin_pdb_molsys):
    ...
    molsys = msm.build.solvate(
        molsys, box_shape="cubic", clearance="14.0 angstroms", engine="PDBFixer"
    )
    box = msm.get(molsys, element="system", box=True)
    lengths, angles = msm.pbc.get_lengths_and_angles_from_box(box)
    check_lengths = np.allclose(
        msm.pyunitwizard.get_value(lengths, to_unit="nm"), [[3.1236, 3.1236, 3.1236]]
    )
    check_angles = np.allclose(
        msm.pyunitwizard.get_value(angles, to_unit="degrees"),
        [[90.000001, 90.000001, 90.000001]],
    )
...
def test_get_lengths_and_angles_from_box_dodecahedral_geometry(
    met_enkephalin_pdb_molsys,
):
    molsys = met_enkephalin_pdb_molsys
    molsys = msm.build.solvate(
        molsys,
        box_shape="rhombic dodecahedral",
        clearance="14.0 angstroms",
        engine="PDBFixer",
    )
    box = msm.get(molsys, element="system", box=True)
    ...
    check_angles = np.allclose(
        msm.pyunitwizard.get_value(angles, to_unit="degrees"), [[60.0, 60.0, 90.000001]]
    )
    assert check_lengths
    assert check_angles
