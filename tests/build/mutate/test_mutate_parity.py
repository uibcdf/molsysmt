...
def ala_val_pro_heavy():
    ...
    mol = msm.build.build_peptide(
        "AlaValPro", engine="MolSysMT", to_form="molsysmt.MolSys"
    )
...
def test_parity_group_name_val_to_gly(ala_val_pro):
    ...
    for eng in ("MolSysMT", "PDBFixer"):
        r = msm.build.mutate(
            ala_val_pro, mutations={1: "GLY"}, keys="group_index", engine=eng
        )
...
def test_parity_backbone_present_both_engines(ala_val_pro_heavy):
    ...
    for eng in ("MolSysMT", "PDBFixer"):
        r = msm.build.mutate(
            ala_val_pro_heavy, mutations={1: "GLY"}, keys="group_index", engine=eng
        )
        heavy = msm.get(
            r,
            element="atom",
...
def test_parity_gly_no_sidechain_native(ala_val_pro_heavy):
    ...
    r = msm.build.mutate(
        ala_val_pro_heavy, mutations={1: "GLY"}, keys="group_index", engine="MolSysMT"
    )
    heavy = msm.get(
        r, element="atom", selection='group_index==1 and atom_type!="H"', atom_name=True
    )
...
def test_parity_gly_no_sidechain_pdbfixer(ala_val_pro_heavy):
    ...
    r = msm.build.mutate(
        ala_val_pro_heavy, mutations={1: "GLY"}, keys="group_index", engine="PDBFixer"
    )
    heavy = msm.get(
        r, element="atom", selection='group_index==1 and atom_type!="H"', atom_name=True
    )
...
def test_parity_val_to_gly_reduces_heavy_count(ala_val_pro_heavy):
    ...
    msm.get(ala_val_pro_heavy, n_atoms=True)
    for eng in ("MolSysMT", "PDBFixer"):
        r = msm.build.mutate(
            ala_val_pro_heavy, mutations={1: "GLY"}, keys="group_index", engine=eng
        )
        # PDBFixer may add OXT to C-terminal, so use ≤ instead of <
        n_after_in_g1 = msm.get(
            r,
            element="atom",
            selection='group_index==1 and atom_type!="H"',
            n_atoms=True,
        )
        assert n_after_in_g1 == 4, (
            f"{eng}: GLY group 1 has {n_after_in_g1} heavy atoms (expected 4)"
        )
...
def test_parity_trp_sidechain_present_pdbfixer(ala_val_pro_heavy):
    ...
    r = msm.build.mutate(
        ala_val_pro_heavy, mutations={1: "TRP"}, keys="group_index", engine="PDBFixer"
    )
    heavy = set(
        msm.get(
            r,
