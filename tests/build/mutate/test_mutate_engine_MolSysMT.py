...
def test_mutate_group_name_changes(ala_val_pro):
    mutated = msm.build.mutate(
        ala_val_pro, mutations={1: "GLY"}, keys="group_index", engine="MolSysMT"
    )
...
def test_mutate_backbone_atoms_present(ala_val_pro):
    ...
    mutated = msm.build.mutate(
        ala_val_pro, mutations={1: "LEU"}, keys="group_index", engine="MolSysMT"
    )
    heavy = msm.get(
        mutated,
        element="atom",
...
def test_mutate_preserves_hydrogens_when_present(ala_val_pro):
    ...
    n_h_before = len(
        msm.get(
            ala_val_pro, element="atom", selection='atom_type=="H"', atom_index=True
        )
    )
    mutated = msm.build.mutate(
        ala_val_pro, mutations={1: "ALA"}, keys="group_index", engine="MolSysMT"
    )
    n_h_after = len(
        msm.get(mutated, element="atom", selection='atom_type=="H"', atom_index=True)
    )
    assert n_h_after > 0
...
def test_mutate_via_string_list(ala_val_pro):
    mutated = msm.build.mutate(ala_val_pro, mutations=["VAL-1-GLY"], engine="MolSysMT")
    names = msm.get(mutated, element="group", group_name=True)
    assert names[1] == "GLY"
...
def test_mutate_via_group_id_dict(ala_val_pro):
    mutated = msm.build.mutate(
        ala_val_pro, mutations={"1": "ALA"}, keys="group_id", engine="MolSysMT"
    )
    names = msm.get(mutated, element="group", group_name=True)
    assert names[1] == "ALA"
