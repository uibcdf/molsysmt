...
def test_his_variants_geometry(his_variant):
    ...
    molsys = msm.build.build_peptide(
        f"ALA{his_variant}ALA", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
...
def test_his_long_variants(his_variant):
    ...
    molsys = msm.build.build_peptide(
        f"GLY{his_variant}GLY", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
...
def test_cyx_builds():
    ...
    molsys = msm.build.build_peptide(
        "ALACYXALA", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
    n_groups = msm.get(molsys, element="system", n_groups=True)
    assert n_groups == 3
...
def test_ace_only_sequence():
    ...
    molsys = msm.build.build_peptide(
        "ACE", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
...
def test_nme_only_sequence():
    ...
    molsys = msm.build.build_peptide(
        "NME", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
...
def test_ace_nme_no_amino_acid():
    ...
    molsys = msm.build.build_peptide(
        "ACENME", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
...
def test_output_string_pdb_text():
    ...
    result = msm.build.build_peptide(
        "ALAG", to_form="string:pdb_text", engine="MolSysMT"
    )
...
def test_output_openmm_topology():
    ...
    result = msm.build.build_peptide(
        "AGV", to_form="openmm.Topology", engine="MolSysMT"
    )
...
def test_three_letter_string_input():
    ...
    molsys = msm.build.build_peptide(
        "AlaGlyVal", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
    n_groups = msm.get(molsys, element="system", n_groups=True)
    assert n_groups == 3
...
def test_first_group_no_head_index():
    ...
    molsys = msm.build.build_peptide(
        "ACEGLYGLY", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert molsys is not None
    # ACE + GLY + GLY
    n_groups = msm.get(molsys, element="system", n_groups=True)
...
def test_input_is_molsys():
    ...
    peptide = msm.build.build_peptide(
        "AGVIL", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    rebuilt = msm.build.build_peptide(
        peptide, to_form="molsysmt.MolSys", engine="MolSysMT"
    )
