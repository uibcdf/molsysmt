...
def test_capped_single_aa_groups():
    ...
    molsys = msm.build.build_peptide(
        "ACEALANME", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
...
def test_capped_gly_geometry():
    ...
    molsys = msm.build.build_peptide(
        "ACEGLYNME", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert _basic_geometry_ok(molsys, min_bond=0.09, max_bond=0.185, min_nonbonded=0.10)
...
def test_ace_only_at_n_terminus():
    ...
    molsys = msm.build.build_peptide(
        "ACEALA", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    group_names = list(molsys.topology.groups["group_name"])
...
def test_nme_only_at_c_terminus():
    ...
    molsys = msm.build.build_peptide(
        "ALANME", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    group_names = list(molsys.topology.groups["group_name"])
...
def test_consecutive_prolines():
    ...
    molsys = msm.build.build_peptide(
        "PPP", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert msm.get(molsys, n_groups=True) == 3
    bl = _bond_lengths_nm(molsys)
    assert np.max(bl) <= 0.185
...
def test_pro_in_capped_sequence():
    ...
    molsys = msm.build.build_peptide(
        "ACEPRONME", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    assert msm.get(molsys, n_groups=True) >= 2
...
def test_chain_name_is_A():
    ...
    molsys = msm.build.build_peptide(
        "ALA", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
...
def test_bonds_atom_indices_are_sorted():
    ...
    molsys = msm.build.build_peptide(
        "AGS", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    bonds = molsys.topology.bonds[["atom1_index", "atom2_index"]].to_numpy(dtype=int)
...
def test_n_components_is_one():
    ...
    molsys = msm.build.build_peptide(
        "AGVIL", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
...
def test_output_form_topology():
    ...
    topology = msm.build.build_peptide(
        "AG", to_form="molsysmt.Topology", engine="MolSysMT"
    )
...
def test_output_form_structures():
    ...
    structures = msm.build.build_peptide(
        "AG", to_form="molsysmt.Structures", engine="MolSysMT"
    )
...
def test_coordinates_shape():
    ...
    molsys = msm.build.build_peptide(
        "AGA", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
...
def test_coordinates_finite():
    ...
    molsys = msm.build.build_peptide(
        "ACDEFGHIKL", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    coords = _get_coordinates_nm(molsys)
...
def test_coordinates_not_all_zero():
    ...
    molsys = msm.build.build_peptide(
        "AGV", to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    coords = _get_coordinates_nm(molsys)
...
def test_deterministic_output():
    ...
    seq = "ACVLIMFYW"
    molsys_a = msm.build.build_peptide(
        seq, to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    molsys_b = msm.build.build_peptide(
        seq, to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    coords_a = _get_coordinates_nm(molsys_a)
    coords_b = _get_coordinates_nm(molsys_b)
    bonds_a = molsys_a.topology.bonds[["atom1_index", "atom2_index"]].to_numpy(
        dtype=int
    )
    bonds_b = molsys_b.topology.bonds[["atom1_index", "atom2_index"]].to_numpy(
        dtype=int
    )
