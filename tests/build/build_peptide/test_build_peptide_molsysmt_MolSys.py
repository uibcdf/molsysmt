...
_RANDOM_SEQUENCES_LEN10_EXTENDED = _random_sequences(
    length=10, n_sequences=40, seed=20260216
)
...
def _get_build_metrics(molsys):
    ...
    np.array(molsys.topology.atoms["atom_type"].to_numpy(), dtype=object)
...
def test_build_peptide_molsysmt_MolSys_5(sequence):
    molsys = msm.build.build_peptide(
        sequence, to_form="molsysmt.MolSys", engine="MolSysMT"
    )
...
def test_build_peptide_molsysmt_MolSys_6(sequence):
    molsys_tleap = msm.build.build_peptide(
        sequence, to_form="molsysmt.MolSys", engine="LEaP"
    )
    molsys_molsysmt = msm.build.build_peptide(
        sequence, to_form="molsysmt.MolSys", engine="MolSysMT"
    )
    ...
    assert (
        abs(
            metrics_molsysmt["min_nonbonded_heavy_nm"]
            - metrics_tleap["min_nonbonded_heavy_nm"]
        )
        <= 0.01
    )
...
@pytest.mark.parametrize(
    "sequence", ["AP", "GP", "PP", "PAP", "GPPG", "APPPG", "ACEPPPNME"]
)
...
def test_build_peptide_molsysmt_MolSys_8(sequence):
    ...
    coords_a = msm.pyunitwizard.get_value(
        molsys_a.structures.coordinates[0], to_unit="nm"
    )
    coords_b = msm.pyunitwizard.get_value(
        molsys_b.structures.coordinates[0], to_unit="nm"
    )
    bonds_a = np.array(
        molsys_a.topology.bonds[["atom1_index", "atom2_index"]].to_numpy(dtype=int)
    )
    bonds_b = np.array(
        molsys_b.topology.bonds[["atom1_index", "atom2_index"]].to_numpy(dtype=int)
    )
