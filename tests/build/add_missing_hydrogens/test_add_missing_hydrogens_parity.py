...
def avp_full():
    ...
    return msm.build.build_peptide(
        "AlaValPro", engine="MolSysMT", to_form="molsysmt.MolSys"
    )
...
def test_parity_protein_h_counts_within_tolerance(trpcage_heavy):
    ...
    n_h_native = msm.get(
        r_native, element="atom", selection='atom_type=="H"', n_atoms=True
    )
...
def test_parity_protein_heavy_count_unchanged(trpcage_heavy):
    ...
    n_heavy_before = msm.get(
        trpcage_heavy, element="atom", selection='atom_type!="H"', n_atoms=True
    )
    for eng in ("MolSysMT", "PDBFixer"):
        r = msm.build.add_missing_hydrogens(trpcage_heavy, engine=eng)
        n_heavy_after = msm.get(
            r, element="atom", selection='atom_type!="H"', n_atoms=True
        )
