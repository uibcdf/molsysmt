...
class TestAddMissingHydrogensEngMolSysMT:
    ...
    def test_round_trip_h_count(self, avp_heavy, avp_full):
        ...
        restored = msm.build.add_missing_hydrogens(avp_heavy, engine="MolSysMT")
        n_h_restored = msm.get(
            restored, element="atom", selection='atom_type=="H"', n_atoms=True
        )
        n_h_orig = msm.get(
            avp_full, element="atom", selection='atom_type=="H"', n_atoms=True
        )
    ...
    def test_partial_h_only_missing_added(self, avp_full):
        ...
        no_hb = msm.remove(
            avp_full,
            selection='atom_name in ["HB1", "HB2", "HB3"] and group_name=="ALA"',
        )
...
def capped_ala_full():
    ...
    mol = msm.build.build_peptide("ALA", engine="MolSysMT", to_form="molsysmt.MolSys")
    return msm.build.add_missing_terminal_cappings(
        mol, N_terminal="ACE", C_terminal="NME", engine="MolSysMT"
    )
