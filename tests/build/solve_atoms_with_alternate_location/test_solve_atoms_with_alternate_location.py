...
def test_solve_atoms_with_alternate_location_molsysmt_MolSys_1():
    ...
    aux_coors = puw.quantity(
        np.array([[3.2742, 2.2579, 0.1536], [3.2757, 2.2571, 0.1533]]), "nm"
    )
    ...
    aux_coors = puw.quantity(
        np.array([[3.1412, 2.241, 0.1076], [3.3396, 2.192, 0.2619]]), "nm"
    )
...
def test_solve_atoms_with_alternate_location_molsysmt_MolSys_2():
    ...
    atom_id, b_factor, coordinates = msm.get(
        molecular_system,
        element="atom",
        selection=[2686, 2687],
        atom_id=True,
    ...
    aux_coors = puw.quantity(
        np.array([[[3.2757, 2.2571, 0.1533], [3.3396, 2.192, 0.2619]]]), "nm"
    )
...
def test_solve_atoms_with_alternate_location_molsysmt_MolSys_3():
    ...
    msm.build.solve_atoms_with_alternate_location(
        molecular_system, selection=[2686, 2687], location_id=["A", "B"]
    )
    atom_id, b_factor, coordinates = msm.get(
        molecular_system,
        element="atom",
    ...
    aux_coors = puw.quantity(
        np.array([[[3.2742, 2.2579, 0.1536], [3.3396, 2.192, 0.2619]]]), "nm"
    )
    assert puw.are_close(coordinates, aux_coors)
...
def test_solve_atoms_with_alternate_location_molsysmt_MolSys_4():
    ...
    msm.build.solve_atoms_with_alternate_location(
        molecular_system, selection=[2686, 2687], location_id="occupancy"
    )
    atom_id, b_factor, coordinates = msm.get(
        molecular_system,
        element="atom",
    ...
    aux_coors = puw.quantity(
        np.array([[[3.2742, 2.2579, 0.1536], [3.1412, 2.241, 0.1076]]]), "nm"
    )
    assert puw.are_close(coordinates, aux_coors)
