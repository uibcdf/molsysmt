...
def test_pipeline_heavy_count_unchanged_by_hydrogens(trpcage_heavy):
    ...
    n_heavy_after = msm.get(
        mol, element="atom", selection='atom_type!="H"', n_atoms=True
    )
...
def test_pipeline_neutralization_correct(prepared):
    ...
    n_ions = msm.get(
        prepared, element="molecule", selection='molecule_type=="ion"', n_molecules=True
    )
    ion_names = msm.get(
        prepared, element="group", selection='group_type=="ion"', group_name=True
    )
    assert n_ions == 1
    assert ion_names == ["CL"]
