...
def test_add_missing_terminal_cappings_molsysmt_MolSys_1():
    ...
    molsys = msm.build.add_missing_terminal_cappings(
        molsys, N_terminal=None, C_terminal=None
    )
    ...
    check_before = n_atoms_before == 40 and np.all(
        np.array(["ALA", "VAL", "PRO"], dtype=object) == groups_before
    )
    check_after = n_atoms_after == 43 and np.all(
        np.array(["ALA", "VAL", "PRO"], dtype=object) == groups_after
    )
...
def test_add_missing_terminal_cappings_molsysmt_MolSys_2():
    ...
    check_after = n_atoms_after == 52 and np.all(
        np.array(["ACE", "ALA", "VAL", "PRO", "NME"], dtype=object) == groups_after
    )
