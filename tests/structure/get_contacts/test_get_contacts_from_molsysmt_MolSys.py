...
def test_get_contacts_from_molsysmt_MolSys_1():
    ...
    contact_map = msm.structure.get_contacts(
        molsys, selection=CA_atoms, threshold="1.2 nm"
    )
    check_shape_1 = (1, 497, 497) == contact_map.shape
    check_value_1 = not contact_map[0, 100, 120]
    check_value_2 = contact_map[0, 200, 0]
    check_value_3 = contact_map[0, 490, 493]
    assert check_shape_1 and check_value_1 and check_value_2 and check_value_3


def test_get_contacts_from_molsysmt_MolSys_2():
    molsys = msm.convert(systems["TcTIM"]["1tcd.h5msm"], to_form="molsysmt.MolSys")
    CA_atoms_chain_0 = msm.get(
        molsys,
        element="atom",
        selection="atom_name=='CA' and chain_index==0",
        atom_index=True,
    )
    CA_atoms_chain_1 = msm.get(
        molsys,
        element="atom",
        selection="atom_name=='CA' and chain_index==1",
        atom_index=True,
    )
    contact_map = msm.structure.get_contacts(
        molsys,
        selection=CA_atoms_chain_0,
        selection_2=CA_atoms_chain_1,
        threshold=1.2 * puw.unit("nm"),
    )
    check_shape_1 = (1, 248, 249) == contact_map.shape
    check_value_1 = np.all(
        np.array([False, False, False, False, True, False, False, True, False, False])
