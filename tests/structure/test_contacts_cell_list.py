...
def test_contacts_cell_list_vacuum():
    ...
    with context(cell_list=False):
        contacts_all_pairs = msm.structure.get_contacts(
            system, threshold=threshold, pbc=False
        )

    # Candidate: O(N) cell list
    with context(cell_list=True):
        contacts_cell_list = msm.structure.get_contacts(
            system, threshold=threshold, pbc=False
        )
...
def test_contacts_cell_list_orthogonal_pbc():
    ...
    with context(cell_list=False):
        contacts_all_pairs = msm.structure.get_contacts(
            system, threshold=threshold, pbc=True
        )
