...
def test_get_radius_of_gyration_trajectory():
    ...
    rg = msm.structure.get_radius_of_gyration(
        molsys, selection='atom_type!="H"', structure_indices=[0, 100]
    )
    rg_val = msm.pyunitwizard.get_value(rg, to_unit="nm")
...
def test_get_radius_of_gyration_uniform_vs_mass_weighted_differ():
    ...
    rg_geom = msm.structure.get_radius_of_gyration(
        molsys, selection='atom_type!="H"', weights=None
    )
    rg_mass = msm.structure.get_radius_of_gyration(
        molsys, selection='atom_type!="H"', weights="masses"
    )
