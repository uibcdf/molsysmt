...
def test_get_center_groups_of_atoms_shape(pentalanine):
    ...
    list_groups = msm.get(
        pentalanine, element="group", selection="all", atom_index=True
    )
...
def test_get_center_groups_of_atoms_values(pentalanine):
    ...
    center = msm.structure.get_center(
        pentalanine, selection=list_groups, structure_indices=0
    )
    # Shape: (1, n_groups, 3)
...
def test_get_center_groups_of_atoms_with_weights(pentalanine):
    ...
    msm.get(pentalanine, element="system", n_structures=True)
    len(list_groups)
    # Build equal weights per group (same as unweighted, just exercises the path)
    weights = [np.ones(len(g)) for g in list_groups]
    center_weighted = msm.structure.get_center(
        pentalanine, selection=list_groups, weights=weights
    )
...
def test_get_center_flat_with_weights(pentalanine):
    ...
    center_w = msm.structure.get_center(
        pentalanine, selection=ca_indices, weights=weights, structure_indices=0
    )
    center_p = msm.structure.get_center(
        pentalanine, selection=ca_indices, structure_indices=0
    )
...
def test_get_center_single_structure_index(pentalanine):
    ...
    center = msm.structure.get_center(
        pentalanine, selection='atom_name=="CA"', structure_indices=0
    )
    assert center.shape[0] == 1
    assert center.shape[2] == 3
...
def test_get_center_multiple_structure_indices(pentalanine):
    ...
    center = msm.structure.get_center(
        pentalanine, selection='atom_name=="CA"', structure_indices=[0, 1, 2]
    )
