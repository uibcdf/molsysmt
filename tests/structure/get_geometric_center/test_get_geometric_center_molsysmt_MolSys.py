...
def test_get_geometric_center_molsysmt_MolSys_1():
    ...
    check_values = np.allclose(
        [0.800907, 1.0867966, -0.02075539],
        msm.pyunitwizard.get_value(center[1000, 0, :], to_unit="nm"),
    )
...
def test_get_geometric_center_molsysmt_MolSys_2():
    ...
    distance_groups = msm.structure.get_distances(
        center_group_0, molecular_system_2=center_group_1
    )
    distance_groups_2 = msm.structure.get_distances(
        molsys,
...
def test_get_geometric_center_molsysmt_MolSys_3():
    ...
    check_value = np.allclose(
        true_distance,
        msm.pyunitwizard.get_value(distance_groups[1000:1010, 0, 0], to_unit="nm"),
    )
    assert check_value
