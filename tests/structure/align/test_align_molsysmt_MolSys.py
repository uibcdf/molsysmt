...
def test_get_structure_alignment_molsysmt_MolSys_2():
    ...
    molsys_2 = msm.convert(
        systems["T4 lysozyme L99A"]["1l17.h5msm"], to_form="molsysmt.MolSys"
    )
    molsys_2 = msm.convert(
        molsys_2, to_form="molsysmt.MolSys", selection='molecule_type=="protein"'
    )
    molsys_2on1 = msm.structure.least_rmsd_align(
        molsys_2,
        selection="backbone",
        reference_molecular_system=molsys_1,
        reference_selection="backbone",
    ...
    check = np.isclose(
        0.021333623779354657, msm.pyunitwizard.get_value(rmsd, to_unit="nm")[0]
    )
