...
def test_get_neighbors_from_molsysmt_MolSys_1():
    ...
    neighbors, distances = msm.structure.get_neighbors(
        molsys, selection=CA_atoms_list, n_neighbors=3
    )
...
def test_get_neighbors_from_molsysmt_MolSys_2():
    ...
    CA_atoms_list = msm.select(molsys, selection='atom_name=="CA"')
    neighbors, distances = msm.structure.get_neighbors(
        molsys, selection=CA_atoms_list, selection_2="all", n_neighbors=4
    )
...
def test_get_neighbors_from_molsysmt_MolSys_3():
    ...
    neighbors, distances = msm.structure.get_neighbors(
        molsys, selection=atoms_in_residues_chain_0, n_neighbors=8
    )
...
def test_get_neighbors_from_molsysmt_MolSys_4():
    ...
    check_distance = np.isclose(
        puw.get_value(distances[0, 0, 7], to_unit="nm"), 3.539767
    )
    assert check_neighbors
    assert check_distance
...
def test_get_neighbors_from_molsysmt_MolSys_5():
    ...
    neighbors, distances = msm.structure.get_neighbors(
        molsys, selection=100, selection_2=atoms_in_residues_chain_1, n_neighbors=4
    )
...
def test_get_neighbors_from_molsysmt_MolSys_6():
    ...
    neighbors, distances = msm.structure.get_neighbors(
        molsys, selection=CA_atoms, threshold="8 angstroms"
    )
...
def test_get_neighbors_from_molsysmt_MolSys_8():
    ...
    pairs_mutual, distances_mutual = msm.structure.get_neighbors(
        molsys,
    ...
    pairs, distances = msm.structure.get_neighbors(
        molsys,
        selection='atom_type=="S"',
        n_neighbors=1,
        output_type="pairs",
    ...
    assert puw.are_close(
        distances_mutual[0],
        puw.quantity([0.68968444, 0.21195179, 0.33566942, 0.20483012], "nanometer"),
    )
    assert pairs == [
        [
            [26, 1692],
