...
def test_get_dihedral_angles_from_molsysmt_MolSys_1(met_enkephalin_pdb_molsys):
    ...
    covalent_paths = msm.topology.get_covalent_paths(
        molsys,
        path=['atom_name=="C"', 'atom_name=="N"', 'atom_name=="CA"', 'atom_name=="C"'],
    )
    dihedral_angles = msm.structure.get_dihedral_angles(
        molsys, dihedral_quartets=covalent_paths[2]
    )
    check1 = np.allclose(
        np.array([[-180.0]]),
        msm.pyunitwizard.get_value(dihedral_angles, to_unit="degrees"),
    )
    check2 = np.allclose(
        np.array([[180.0]]),
        msm.pyunitwizard.get_value(dihedral_angles, to_unit="degrees"),
...
def test_get_dihedral_angles_from_molsysmt_MolSys_2(met_enkephalin_pdb_molsys):
    ...
    dihedral_angles = msm.structure.get_dihedral_angles(
        molsys, dihedral_quartets=covalent_paths
    )
...
def test_get_dihedral_angles_from_molsysmt_MolSys_3(traj_pentalanine_h5_molsys):
    molsys = traj_pentalanine_h5_molsys
    phi_angles, psi_angles = msm.structure.get_dihedral_angles(
        molsys, selection="group_index==[3,4]", phi=True, psi=True
    )
    dihedral_angles = np.hstack([phi_angles, psi_angles])
    true_shape = (5000, 2)
    true_values_1 = np.array(
        [
            [-72.00828191, 135.54164973],
    ...
    true_values_2 = np.array(
        [
            [-144.99215202, 133.44230978],
    ...
    true_values_3 = np.array(
        [
            [-115.83193832, -32.86894649],
    ...
    check_value_1 = np.allclose(
        true_values_1,
        msm.pyunitwizard.get_value(dihedral_angles[0:5], to_unit="degrees"),
    )
    check_value_2 = np.allclose(
        true_values_2,
        msm.pyunitwizard.get_value(dihedral_angles[1000:1005], to_unit="degrees"),
    )
    check_value_3 = np.allclose(
        true_values_3,
        msm.pyunitwizard.get_value(dihedral_angles[2000:2005], to_unit="degrees"),
    )
    assert check_shape
...
def test_get_dihedral_angles_from_molsysmt_MolSys_4(traj_pentalanine_h5_molsys):
    molsys = traj_pentalanine_h5_molsys
