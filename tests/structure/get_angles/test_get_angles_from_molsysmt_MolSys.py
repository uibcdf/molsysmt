...
def test_get_angles_from_molsysmt_MolSys_1():
    ...
    check2 = np.allclose(
        good_angles, msm.pyunitwizard.get_value(angles[:5, 0], to_unit="degrees")
    )
    assert check1
    assert check2
...
def test_get_angles_from_molsysmt_MolSys_2():
    molsys = msm.systems["pentalanine"]["traj_pentalanine.h5"]
    molsys = msm.convert(molsys)
    angles = msm.structure.get_angles(
        molsys, [0, 1, 2], structure_indices=[0, 1, 2, 3, 4], pbc=True
    )
    ...
    check2 = np.allclose(
        good_angles, msm.pyunitwizard.get_value(angles[:, 0], to_unit="degrees")
    )
    assert check1
    assert check2
