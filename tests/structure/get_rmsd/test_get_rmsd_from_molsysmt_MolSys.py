...
def test_get_rmsd_molsysmt_MolSys_1():
    ...
    rmsd = msm.structure.get_rmsd(
        molsys, selection="backbone", structure_indices=100, reference_structure_index=0
    )
    true_value_1 = [0.7381704258064243]
    check_value_1 = np.allclose(
        true_value_1, msm.pyunitwizard.get_value(rmsd, to_unit="nm")
    )
...
def test_get_rmsd_molsysmt_MolSys_2():
    ...
    check_value_1 = np.allclose(
        true_value_1, msm.pyunitwizard.get_value(rmsd[1000:1010], to_unit="nm")
    )
    assert check_value_1
