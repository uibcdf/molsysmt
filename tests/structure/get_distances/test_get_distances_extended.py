...
def pentalanine():
    return msm.convert(
        systems["pentalanine"]["pentalanine.h5msm"], to_form="molsysmt.MolSys"
    )
...
def test_pairs_as_two_element_list(tctim):
    ...
    dist = msm.structure.get_distances(
        tctim, selection=["atom_index==0", "atom_index==1"], pairs=True
    )
...
def test_two_systems_no_selection2(tctim):
    ...
    dist = msm.structure.get_distances(
        tctim, selection="group_index==0", molecular_system_2=tctim
    )
    assert dist.ndim == 3
...
def test_pbc_false(tctim):
    ...
    dist = msm.structure.get_distances(
        tctim, selection="group_index==0", selection_2="group_index==1", pbc=False
    )
    assert dist.ndim == 3
    assert np.all(puw.get_value(dist, to_unit="nm") >= 0)
