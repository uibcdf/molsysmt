...
def test_get_distances_from_molsysmt_MolSys_1():
    ...
    distances = msm.structure.get_distances(
        molsys, selection="group_index==0", selection_2="group_index==1"
    )
...
def test_get_distances_from_molsysmt_MolSys_groups_2():
    ...
    check_distance = np.isclose(
        puw.get_value(distances[0, 0, 0], to_unit="nm"), 0.43703472
    )
    assert check_shape and check_distance
...
def test_get_distances_from_molsysmt_MolSys_groups_3():
    molsys = msm.convert(systems["TcTIM"]["1tcd.h5msm"], to_form="molsysmt.MolSys")
    list_groups_2 = msm.get(
        molsys, element="group", selection="group_index in [4,5,6,7,8]", atom_index=True
    )
    distances = msm.structure.get_distances(
        molsys,
        selection="group_index==0",
        selection_2=list_groups_2,
        center_of_atoms_2=True,
    )
    check_shape = (1, 9, 5) == distances.shape
    check_distance = np.isclose(
        puw.get_value(distances[0, 3, 3], to_unit="nm"), 2.070343
    )
    assert check_shape and check_distance
...
def test_get_distances_from_molsysmt_MolSys_groups_4():
    molsys = msm.convert(systems["TcTIM"]["1tcd.h5msm"], to_form="molsysmt.MolSys")
    msm.get(
        molsys, element="group", selection="group_index in [0,1,2,3]", atom_index=True
    )
    list_groups_2 = msm.get(
        molsys, element="group", selection="group_index in [4,5,6,7,8]", atom_index=True
    )
    ...
    check_distance = np.isclose(
        puw.get_value(distances[0, 0, 4], to_unit="nm"), 2.291778
    )
    assert check_shape and check_distance
...
def test_get_distances_from_molsysmt_MolSys_groups_6():
    ...
    distances = msm.structure.get_distances(
        molsys, selection=list_groups_1, center_of_atoms=True
    )
    check_shape = (1, 4, 4) == distances.shape
    check_distance = np.isclose(
        puw.get_value(distances[0, 2, 3], to_unit="nm"), 0.386833
    )
    assert check_shape and check_distance
...
def test_get_distances_from_molsysmt_MolSys_groups_7():
    ...
    check_distance = np.isclose(
        puw.get_value(distances[1000, 0, 0], to_unit="nm"), 0.818624
    )
    assert check_shape and check_distance
...
def test_get_distances_from_molsysmt_MolSys_groups_8():
    ...
    distances = msm.structure.get_distances(
        molsys, selection=list_groups_1, center_of_atoms=True, structure_indices=3000
    )
...
def test_get_distances_from_molsysmt_MolSys_groups_9():
    ...
    check_shape = (1, 1, 1) == distances.shape
    check_distance = np.isclose(
        puw.get_value(distances[0, 0, 0], to_unit="nm"), 0.5978502
    )
    assert check_shape and check_distance
...
def test_get_distances_from_molsysmt_MolSys_groups_12():
    ...
    list_atom_groups = msm.get(
        molsys, element="group", selection="all", atom_index=True
    )
    distances = msm.structure.get_distances(
        molsys, selection=list_atom_groups, center_of_atoms=True
    )
...
def test_get_distances_from_molsysmt_MolSys_groups_13():
    ...
    distances = msm.structure.get_distances(
        molsys,
        selection=list_atom_groups_1,
        center_of_atoms=True,
        selection_2=list_atom_groups_2,
        center_of_atoms_2=True,
        pairs=True,
    )
