...
def test_get_distances_from_XYZ_1():
    molsys = msm.convert(
        systems["particles 4"]["traj_particles_4.xyznpy"], to_form="XYZ"
    )
    distances = msm.structure.get_distances(molsys)
    check_shape = (3, 4, 4) == distances.shape
    check_distance = np.isclose(
        puw.get_value(distances[1, 0, 2], to_unit="nm"), 3.741657
    )
...
def test_get_distances_from_XYZ_2():
    ...
    check_distance = np.isclose(
        puw.get_value(distances[2, 0, 0], to_unit="nm"), 1.7320508
    )
    assert check_shape and check_distance
...
def test_get_distances_from_XYZ_3():
    ...
    check_distance = np.isclose(
        puw.get_value(distances[0, 0, 1], to_unit="nm"), 1.41421356
    )
    assert check_shape and check_distance
...
def test_get_distances_from_XYZ_4():
    ...
    distances = msm.structure.get_distances(
        molsys, selection=1, selection_2=[0, 2], structure_indices=[1, 2]
    )
    check_distance = np.isclose(
        puw.get_value(distances[1, 0, 1], to_unit="nm"), 1.73205080
    )
...
def test_get_distances_from_XYZ_5():
    ...
    check_distance = np.isclose(
        puw.get_value(distances[1, 1, 0], to_unit="nm"), 2.2360679
    )
    assert check_distance
...
def test_get_distances_from_XYZ_6():
    ...
    distances = msm.structure.get_distances(
        molsys, selection=1, structure_indices=[0, 1], structure_indices_2=[1, 2]
    )
...
def test_get_distances_from_XYZ_8():
    ...
    distances = msm.structure.get_distances(
        molsys, selection=[[0, 1], [0, 2], [1, 2]], structure_indices=[1, 2], pairs=True
    )
    good_distances = np.array(
        [[3.46410162, 3.74165739, 1.41421356], [2.82842712, 1.73205081, 1.73205081]]
    )
...
def test_get_distances_from_XYZ_9():
    ...
    indices1, indices2, distances = msm.structure.get_distances(
        molsys,
        selection=[0, 0, 2],
        selection_2=[1, 1, 3],
        structure_indices=[1, 2],
...
def test_get_distances_from_XYZ_11():
    ...
    ind1, ind2, sind1, sind2, distances = msm.structure.get_distances(
        molsys,
        selection=[0, 0, 2],
        selection_2=[1, 1, 3],
        structure_indices=[0, 1],
    ...
    good_distances = np.array(
        [
            [[3.0, 3.0, 2.23606798], [3.0, 3.0, 2.23606798], [1.0, 1.0, 2.23606798]],
            [
                [3.0, 3.0, 3.16227766],
                [3.0, 3.0, 3.16227766],
...
def test_get_distances_from_XYZ_16():
    ...
    assert np.allclose(
        puw.get_value(distances[0][1]), np.array([2.44948974, 3.46410162])
    )
