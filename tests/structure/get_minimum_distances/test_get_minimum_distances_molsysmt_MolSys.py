...
def test_get_minimum_distances_from_molsysmt_MolSys_1():
    ...
    atoms_groups_component_0 = msm.get(
        molsys, element="group", selection="component_index==0", atom_index=True
    )
    atoms_groups_component_1 = msm.get(
        molsys, element="group", selection="component_index==1", atom_index=True
    )
    min_pairs, min_distances = msm.structure.get_minimum_distances(
        molsys, selection=atoms_groups_component_0, selection_2=atoms_groups_component_1
    )
    ...
    check_distance = np.isclose(
        puw.get_value(min_distances[0], to_unit="nm"), 0.38221311
    )
