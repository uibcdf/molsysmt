...
def test_rdkit_rich_atom_and_bond_fields_remain_independent():
    ...
    assert any(
        ...
        for value in msm.get(topology, element="bond", bond_stereochemistry=True)
    )
...
def test_rdkit_native_conversion_report_is_equivalent_for_supported_subset():
    ...
    _, report = msm.convert(mol, to_form="molsysmt.Topology", return_report=True)
...
def test_rdkit_isotope_is_preserved_and_no_longer_reported_as_loss():
    ...
    topology, report = msm.convert(mol, to_form="molsysmt.Topology", return_report=True)
...
def test_rdkit_native_roundtrip_preserves_supported_chemical_semantics():
    ...
    recovered_charges = msm.get(recovered, element="atom", partial_charge=True)
...
def test_rdkit_native_roundtrip_preserves_namespaced_atom_identity():
    ...
    assert second_native.topology.atoms["atom_id"].tolist() == ["carbon-alpha", "1"]
