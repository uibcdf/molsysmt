...
from molsysmt.form.mmcif_PdbxContainers_DataContainer import (
    get_structural_attributes as gsa,
)
...
def test_n_structures_explicit_indices(data_container):
    ...
    n = gsa.get_n_structures_from_system(
        data_container, structure_indices=[0], skip_digestion=True
    )
    assert n == 1
...
def test_coordinates_subset(data_container):
    ...
    xyz = gsa.get_coordinates_from_atom(
        data_container, indices=list(range(10)), skip_digestion=True
    )
    val = puw.get_value(xyz)
