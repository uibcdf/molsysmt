...
def _build_synthetic_h5msm(path):
    ...
    with h5py.File(path, "w") as f:
        ...
        atoms.create_dataset(
            "atom_id",
            data=np.array([f"{i}" for i in range(n_atoms)], dtype=h5py.string_dtype()),
        )
        atoms.create_dataset(
            "atom_name", data=np.array(["C"] * n_atoms, dtype=h5py.string_dtype())
        )
        atoms.create_dataset(
            "atom_type", data=np.array(["C"] * n_atoms, dtype=h5py.string_dtype())
        )
        ...
        coords = np.arange(n_structures * n_atoms * 3, dtype=np.float32).reshape(
