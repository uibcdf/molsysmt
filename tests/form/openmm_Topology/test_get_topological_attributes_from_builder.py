"""
Tests for openmm_Topology/get_topological_attributes.py using a synthetic
...
Differences from mdtraj_Topology adapter
-----------------------------------------
- get_n_bonds_from_atom, get_n_inner_bonds_from_atom → scalar (total unique
  bonds touching the queried atoms), not a per-atom list.
- get_n_components_from_group, get_n_chains_from_group → scalar unique count,
  not a per-group list.
- get_n_chains_from_component → scalar unique count, not a per-component list.
- All get_chain_name_from_* return None (openmm.Topology has no chain names).
- Bond-related queries from group/component/molecule/entity/chain raise
  NotImplementedMethodError (same as mdtraj_Topology).
"""
...
def topo():
    ...
    molsys = b.build()
    return msm.convert(molsys, to_form="openmm.Topology")
...
# Note: get_n_bonds_from_atom and get_n_inner_bonds_from_atom return a scalar
#       in openmm.Topology (total unique bonds), not a per-atom list.
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm.Topology these return a scalar (total unique bonds), not a list
...
# Note: get_n_components_from_group, get_n_chains_from_group return scalars
#       (unique counts) in openmm.Topology, not per-group lists.
# Note: bond-related from_group functions raise NotImplementedMethodError
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm these are already unique-count scalars, total_ delegates:
...
# Note: bond-related from_molecule functions raise NotImplementedMethodError
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm.Topology, n_chains and type-count functions return scalars:
        # get_n_chains_from_molecule, get_n_amino_acids_from_molecule, etc.
        # are tested in test_molecule_total_count below.
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm, n_chains and type counts return scalars (not per-molecule lists):
...
# Note: bond-related from_entity functions raise NotImplementedMethodError
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm.Topology, n_chains and type-count functions return scalars:
        # get_n_chains_from_entity, get_n_amino_acids_from_entity, etc.
        # are tested in test_entity_total_count below.
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm, n_chains and type counts return scalars (not per-entity lists):
...
# Note: get_n_chains_from_component returns a scalar unique count in
#       openmm.Topology, not a per-component list.
# Note: bond-related from_component functions raise NotImplementedMethodError
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm.Topology, n_chains and type-count functions return scalars:
        # get_n_chains_from_component, get_n_amino_acids_from_component, etc.
        # are tested in test_component_total_count below.
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm n_chains and type counts return scalars (not per-component lists):
...
# Note: bond-related from_chain functions raise NotImplementedMethodError
# Note: get_chain_name_from_chain returns None (openmm has no chain names)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm.Topology, type-count functions return scalars:
        # get_n_amino_acids_from_chain, get_n_peptides_from_chain, etc.
        # are tested in test_chain_total_count below.
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # In openmm, type counts return scalars (not per-chain lists):
...
def test_n_atoms_from_atom_scalar(topo):
    assert aux.get_n_atoms_from_atom(topo) == N_ATOMS


def test_n_groups_from_atom_scalar(topo):
...
def test_chain_name_from_atom_is_none(topo):
    assert aux.get_chain_name_from_atom(topo) is None
