...
def yaml_file(builder_pdb_molsys, tmp_path):
    ...
    msm.convert(
        builder_pdb_molsys.structures,
        to_form="file:structures_yaml",
...
def test_parity_n_atoms(yaml_file, source_structures):
    assert (
        msm.get(yaml_file, element="system", n_atoms=True) == source_structures.n_atoms
    )
...
def test_parity_n_structures(yaml_file, source_structures):
    assert (
        msm.get(yaml_file, element="system", n_structures=True)
        == source_structures.n_structures
    )
