...
def test_structures_yaml_roundtrip(tmp_path):
    structures = msm.convert(
        msm.systems["T4 lysozyme L99A"]["181l.h5msm"], to_form="molsysmt.Structures"
    )
...
def test_structures_yaml_preserves_declared_builder_structure(
    builder_structures, tmp_path
):
    output = tmp_path / "builder_structures.yaml"

    msm.convert(
        builder_structures, to_form="file:structures_yaml", output_filename=str(output)
    )
