...
if find_spec("openff") is None or find_spec("openff.toolkit") is None:
    pytest.skip("OpenFF Toolkit is an optional backend", allow_module_level=True)
...
def test_openff_preserves_atom_charge_aromaticity_and_stereochemistry():
    ...
    assert any(
        value in {"R", "S", "r", "s"}
        for value in msm.get(topology, element="atom", atom_stereochemistry=True)
...
def test_openff_keeps_formal_fractional_order_and_aromaticity_independent():
    ...
    assert msm.get(topology, element="bond", fractional_bond_order=True)[
        0
    ] == pytest.approx(1.25)
...
def test_openff_ez_stereo_and_reference_atoms_are_preserved():
    ...
    stereo_references = msm.get(topology, element="bond", bond_stereo_atom_indices=True)
    stereo_index = next(
        index
        for index, value in enumerate(stereo)
        if not pd.isna(value) and value == "E"
    )
    assert all(not pd.isna(value) for value in stereo_references[stereo_index])
