...
from molsysmt.form.molsysmt_MolecularMechanicsDict import (
    get_mechanical_attributes as gma,
)
...
def test_formal_charge_subset(full_mmdict):
    fc = gma.get_formal_charge_from_atom(
        full_mmdict, indices=[0, 1], skip_digestion=True
    )
    assert len(fc) == 2
...
def test_partial_charge_subset(full_mmdict):
    pc = gma.get_partial_charge_from_atom(
        full_mmdict, indices=[2, 4], skip_digestion=True
    )
    assert len(pc) == 2
...
def test_get_non_bonded_method(full_mmdict):
    assert (
        gma.get_non_bonded_method_from_system(full_mmdict, skip_digestion=True) == "PME"
    )


def test_get_cutoff_distance(full_mmdict):
    assert (
        gma.get_cutoff_distance_from_system(full_mmdict, skip_digestion=True)
        is not None
    )
...
def test_get_switch_distance(full_mmdict):
    assert (
        gma.get_switch_distance_from_system(full_mmdict, skip_digestion=True)
        is not None
    )
...
def test_get_dispersion_correction(full_mmdict):
    assert (
        gma.get_dispersion_correction_from_system(full_mmdict, skip_digestion=True)
        is True
    )
...
def test_get_ewald_error_tolerance(full_mmdict):
    assert (
        gma.get_ewald_error_tolerance_from_system(full_mmdict, skip_digestion=True)
        == 1e-4
    )
...
def test_get_hydrogen_mass(full_mmdict):
    assert (
        gma.get_hydrogen_mass_from_system(full_mmdict, skip_digestion=True) is not None
    )
...
def test_get_flexible_constraints(full_mmdict):
    assert (
        gma.get_flexible_constraints_from_system(full_mmdict, skip_digestion=True)
        is False
    )
...
def test_get_water_model(full_mmdict):
    assert (
        gma.get_water_model_from_system(full_mmdict, skip_digestion=True) == "TIP3P-FB"
    )
...
def test_get_solute_dielectric(full_mmdict):
    assert (
        gma.get_solute_dielectric_from_system(full_mmdict, skip_digestion=True) == 1.0
    )


def test_get_solvent_dielectric(full_mmdict):
    assert (
        gma.get_solvent_dielectric_from_system(full_mmdict, skip_digestion=True) == 78.5
    )


def test_get_salt_concentration(full_mmdict):
    assert (
        gma.get_salt_concentration_from_system(full_mmdict, skip_digestion=True)
        is not None
    )
...
def test_non_bonded_method_none_when_missing(empty_mmdict):
    assert (
        gma.get_non_bonded_method_from_system(empty_mmdict, skip_digestion=True) is None
    )
