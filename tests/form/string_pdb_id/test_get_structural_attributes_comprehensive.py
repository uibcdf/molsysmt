"""Comprehensive tests for string_pdb_id/get_structural_attributes.py.

All getters delegate to molsysmt_MolSys after downloading the structure from
RCSB, so every test here requires a live network connection.

Oracle: 1AKI (chicken egg-white lysozyme)
  - 1 model/structure
  - 1079 heavy atoms
  - 129 residues
  - No periodic box (single-crystal structure)
  - B-factors present
  - Occupancy present
"""
...
import molsysmt.form.string_pdb_id.get_structural_attributes as aux
...
class TestGetNStructuresFromSystem:
    ...
    def test_subset_two_returns_two(self):
        # 1AKI has only 1 structure, but the getter counts len(structure_indices)
        # when structure_indices is not 'all', regardless of validity.
...
class TestGetCoordinatesFromAtom:
    ...
    def test_all_returns_correct_shape(self):
        ...
        assert puw.get_value(result).shape == (N_STRUCTURES, N_ATOMS, 3)
    ...
    def test_structure_subset_shape(self):
        ...
        assert puw.get_value(result).shape == (1, N_ATOMS, 3)
...
class TestGetBoxFromSystem:
    ...
    @pytest.mark.network
    def test_returns_none_for_xray_structure(self):
        # 1AKI is an X-ray crystal structure; periodic box is not set in the
        # MolSys representation (no simulation box).
        result = aux.get_box_from_system(ITEM)
        # PDB structures may or may not have a unit cell stored as a box;
        # the key requirement is the call does not raise.
        assert result is None or hasattr(result, '__class__')

    @pytest.mark.network
    def test_structure_subset_does_not_raise(self):
        # Just verify no exception is raised for a structure subset
...
class TestGetTimeFromSystem:
    ...
    @pytest.mark.network
    def test_pdb_has_no_time(self):
        # Static PDB structures carry no simulation time.
...
class TestGetStructureIdFromSystem:
    ...
    def test_returns_value_or_none(self):
        # PDB structures typically have no explicit structure_id;
        # this should return None or an array of length 1.
...
class TestGetBFactorFromAtom:
    ...
    @pytest.mark.network
    def test_returns_non_none_for_xray(self):
        # 1AKI is an X-ray structure; all atoms have B-factors.
        result = aux.get_b_factor_from_atom(ITEM)
        # B-factors may or may not be loaded depending on the BCIF pipeline;
        # the getter must not raise.
        assert result is None or hasattr(result, '__class__')
...
class TestGetOccupancyFromAtom:
    ...
    def test_does_not_raise(self):
        # Occupancy is present in PDB records; the getter must not raise.
        try:
            result = aux.get_occupancy_from_atom(ITEM)
            # If returned, it must be None or array-like
            assert result is None or hasattr(result, '__class__')
