"""
Tests for openmm_Simulation/get_topological_attributes.py using a synthetic
...
The openmm.Simulation adapter delegates all topological queries to openmm_Topology
through item.topology. Return-type characteristics are therefore identical to the
openmm_Topology and openmm_Modeller adapters.

Fixture: the Simulation is built by converting the MolSys to openmm.Topology, then
creating a minimal openmm.System (unit masses, no force fields) and a Langevin
integrator. No positions need to be set for topology-only tests.

Note: full openmm.Simulation creation via msm.convert(..., to_form='openmm.Simulation')
requires a forcefield and GPU platform and is not suitable for CI tests.
"""
...
def simulation():
    ...
    ala_atoms = [
        b.add_atom(atom_name=n, atom_type=t)
    ...
    g_ala = b.add_group(
        ala_atoms, group_id="1", group_name="ALA", group_type="amino acid"
    )
    g_gly = b.add_group(
        gly_atoms, group_id="2", group_name="GLY", group_type="amino acid"
    )
    ...
    b.add_chain(
        [g_wat1, g_wat2, g_ion], chain_id="B", chain_name="B", chain_type="solvent"
    )

    mol_pep = b.add_molecule(
        [g_ala, g_gly],
        molecule_id="0",
        molecule_name="peptide 0",
        molecule_type="peptide",
    )
    mol_wat1 = b.add_molecule(
        [g_wat1], molecule_id="1", molecule_name="water 0", molecule_type="water"
    )
    mol_wat2 = b.add_molecule(
        [g_wat2], molecule_id="2", molecule_name="water 1", molecule_type="water"
    )
    mol_ion = b.add_molecule(
        [g_ion], molecule_id="3", molecule_name="ion 0", molecule_type="ion"
    )

    b.add_entity(
        [mol_pep], entity_id="0", entity_name="peptide 0", entity_type="peptide"
    )
    b.add_entity(
        [mol_wat1, mol_wat2], entity_id="1", entity_name="water", entity_type="water"
    )
    ...
    system = mm.System()
    for _ in range(topo.getNumAtoms()):
    ...
    return Simulation(topo, system, integrator)
...
def test_system_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
...
def test_system_bond_pairs_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
...
def test_system_bonded_atoms_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_atom_array_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_atom_bond_pairs_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
    assert len(result) == N_BONDS


def test_inner_bonded_atoms_from_atom_length(simulation):
    result = aux.get_inner_bonded_atoms_from_atom(simulation)
    assert isinstance(result, list)
    assert len(result) == N_BONDED_ATOMS
...
def test_atom_total_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
    assert result == expected
...
def test_group_array_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_group_total_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
    assert result == expected
...
def test_molecule_array_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_molecule_total_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
    assert result == expected
...
def test_entity_array_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_entity_total_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
    assert result == expected
...
def test_component_array_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_component_total_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
    assert result == expected
...
def test_chain_array_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
    assert isinstance(result, list)
...
def test_chain_total_count(simulation, func_name, expected):
    result = getattr(aux, func_name)(simulation)
    assert result == expected
...
def test_bond_list_length(simulation, func_name):
    result = getattr(aux, func_name)(simulation)
...
def test_bonded_atoms_from_bond(simulation):
    result = aux.get_bonded_atoms_from_bond(simulation)
    assert isinstance(result, list)
    assert len(result) == N_BONDED_ATOMS
...
def test_atom_names_ala(simulation):
    names = aux.get_atom_name_from_atom(simulation)
    assert names[:5] == ["N", "CA", "C", "O", "CB"]


def test_atom_names_gly(simulation):
    names = aux.get_atom_name_from_atom(simulation)
    assert names[5:10] == ["N", "CA", "C", "O", "OXT"]


def test_group_names(simulation):
    names = aux.get_group_name_from_group(simulation)
    assert list(names) == ["ALA", "GLY", "HOH", "HOH", "NA"]


def test_chain_ids(simulation):
    ids = aux.get_chain_id_from_chain(simulation)
    assert ids == ["A", "B"]


def test_molecule_types(simulation):
    types = aux.get_molecule_type_from_molecule(simulation)
    assert types == ["peptide", "water", "water", "ion"]


def test_entity_types(simulation):
    types = aux.get_entity_type_from_entity(simulation)
    assert types == ["peptide", "water", "ion"]


def test_atoms_per_group(simulation):
    counts = aux.get_n_atoms_from_group(simulation)
...
def test_chain_name_from_atom_is_none(simulation):
    assert aux.get_chain_name_from_atom(simulation) is None


def test_chain_name_from_group_is_none(simulation):
    assert aux.get_chain_name_from_group(simulation) is None


def test_chain_name_from_molecule_is_none(simulation):
    assert aux.get_chain_name_from_molecule(simulation) is None


def test_chain_name_from_entity_is_none(simulation):
    assert aux.get_chain_name_from_entity(simulation) is None
