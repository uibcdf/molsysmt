...
def test_molsys_yaml_roundtrip_from_molsys(tmp_path):
    ...
    assert (
        msm.get(str(output), element="system", n_atoms=True) == molsys.topology.n_atoms
    )
    assert (
...
def test_molsys_yaml_preserves_declared_builder_truth(builder_pdb_molsys, tmp_path):
    ...
    msm.convert(
        builder_pdb_molsys, to_form="file:molsys_yaml", output_filename=str(output)
    )
    ...
    assert msm.get(molsys_dict, element="molecule", molecule_name=True) == [
        "protein 0",
        "water",
    ]
    assert rebuilt.topology.bonds[["atom1_index", "atom2_index"]].values.tolist() == (
        builder_pdb_molsys.topology.bonds[
