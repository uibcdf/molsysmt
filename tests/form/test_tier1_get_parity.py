...
def tier1_forms(request):
    ...
    forms = {
        "molsysmt.MolSys": msm.convert(
            base_file, to_form="molsysmt.MolSys", skip_digestion=True
        ),
    }

    try:
        forms["openmm.Topology"] = msm.convert(
            base_file, to_form="openmm.Topology", skip_digestion=True
        )
    except Exception:
        pass

    try:
        forms["mdtraj.Trajectory"] = msm.convert(
            base_file, to_form="mdtraj.Trajectory", skip_digestion=True
        )
    except Exception:
        pass
...
def test_get_atom_name_parity(tier1_forms):
    ...
    for form_name, item in tier1_forms.items():
        results[form_name] = msm.get(
            item, element="atom", atom_name=True, skip_digestion=True
        )

    native_names = results["molsysmt.MolSys"]
    for form_name, names in results.items():
        assert list(names) == list(native_names), (
            f"Atom name discrepancy in {form_name}"
        )
...
def test_get_n_atoms_parity(tier1_forms):
    ...
    for form_name, item in tier1_forms.items():
        results[form_name] = msm.get(
            item, element="system", n_atoms=True, skip_digestion=True
        )

    native_val = results["molsysmt.MolSys"]
    for form_name, val in results.items():
        assert val == native_val, (
            f"n_atoms discrepancy in {form_name}: expected {native_val}, got {val}"
        )
...
def test_get_n_groups_parity(tier1_forms):
    ...
    for form_name, item in tier1_forms.items():
        results[form_name] = msm.get(
            item, element="system", n_groups=True, skip_digestion=True
        )

    native_val = results["molsysmt.MolSys"]
    for form_name, val in results.items():
        assert val == native_val, (
            f"n_groups discrepancy in {form_name}: expected {native_val}, got {val}"
        )
