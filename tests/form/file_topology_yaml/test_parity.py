...
def yaml_file(builder_pdb_molsys, tmp_path):
    ...
    msm.convert(
        builder_pdb_molsys.topology, to_form="file:topology_yaml", output_filename=path
    )
    return path
...
def test_parity_n_groups(yaml_file, source_topology):
    assert (
        msm.get(yaml_file, element="system", n_groups=True) == source_topology.n_groups
    )


def test_parity_n_chains(yaml_file, source_topology):
    assert (
        msm.get(yaml_file, element="system", n_chains=True) == source_topology.n_chains
    )
...
def test_parity_atom_names(yaml_file, source_topology):
    assert (
        msm.get(yaml_file, element="atom", atom_name=True)
...
def test_parity_group_names(yaml_file, source_topology):
    assert (
        msm.get(yaml_file, element="group", group_name=True)
...
def test_parity_chain_ids(yaml_file, source_topology):
    assert (
        msm.get(yaml_file, element="chain", chain_id=True)
