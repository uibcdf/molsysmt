...
def test_molsys_to_molsysdict_roundtrip_and_queries():
    ...
    assert (
        msm.get(molsys_dict, element="system", n_atoms=True) == molsys.topology.n_atoms
    )
    assert (
        msm.get(molsys_dict, element="atom", atom_name=True)
        == molsys.topology.atoms["atom_name"].tolist()
    )
    assert (
        msm.get(molsys_dict, element="group", group_name=True)
        == molsys.topology.groups["group_name"].tolist()
    )
    assert (
        msm.get(molsys_dict, element="chain", chain_id=True)
...
def test_molsysdict_preserves_declared_builder_truth(builder_pdb_molsys):
    ...
    assert (
        ...
        == builder_pdb_molsys.topology.groups["group_id"].tolist()
    )
    assert (
        rebuilt.topology.molecules["molecule_id"].tolist()
        == builder_pdb_molsys.topology.molecules["molecule_id"].tolist()
    )
