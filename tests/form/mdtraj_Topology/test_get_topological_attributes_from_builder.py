"""
Tests for mdtraj_Topology/get_topological_attributes.py using a synthetic
...
--------------------
Bond-related queries *from group/component/molecule/entity/chain* (e.g.,
get_bond_index_from_group, get_bonded_atoms_from_molecule, …) raise
NotImplementedMethodError in the current mdtraj_Topology adapter because
mdtraj does not store per-residue or per-chain bond bookkeeping.
These functions are intentionally excluded from the parametrized tests below
to reflect the form's documented capability boundary.

get_chain_name_from_chain returns None (mdtraj does not store chain names).
"""
...
# Note: get_chain_name_from_chain returns None (mdtraj has no chain names)
# ---------------------------------------------------------------------------
...
# Note: get_bond_order_from_bond, get_bond_type_from_bond not implemented
# ---------------------------------------------------------------------------
...
def test_chain_name_is_none(topo):
    # mdtraj does not store chain names — function returns None by design
    assert aux.get_chain_name_from_chain(topo) is None


def test_chain_name_from_atom_is_none(topo):
    # mdtraj does not store chain names; from_atom variant also returns None
