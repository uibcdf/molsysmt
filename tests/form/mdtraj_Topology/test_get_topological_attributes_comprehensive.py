"""
Comprehensive tests for mdtraj_Topology/get_topological_attributes.py
...
System facts (1vii.pdb, model 1 only, no solvent):
  ...
  N_ENTITIES     = 1
  N_AMINO_ACIDS  = 36
  N_BONDED_ATOMS = 596  (every atom is in at least one bond for a protein)

mdtraj.Topology adapter notes (inherited from builder tests):
  - get_n_bonds_from_atom / get_n_inner_bonds_from_atom → list of length N_ATOMS
    (per-atom bond counts, unlike openmm.Topology which returns a scalar)
  - get_chain_name_from_* → None (mdtraj.Topology has no chain names)
  - get_n_components_from_group / get_n_chains_from_group → list of length N_GROUPS
    (per-group unique counts, unlike openmm.Topology which returns a scalar)
  - get_n_chains_from_component → list of length N_COMPONENTS
...
def test_chain_name_from_group_is_none(topo):
    # mdtraj does not store chain names
...
def test_chain_type_from_chain(topo):
    ...
    # mdtraj.Topology uses 'system' for chains that cannot be narrowly classified;
    # a single peptide chain is labelled 'system'.
