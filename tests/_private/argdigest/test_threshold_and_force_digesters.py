...
from molsysmt._private.argdigest.argument.distance_threshold import (
    digest_distance_threshold,
)
...
def test_angle_distance_and_threshold_digesters_cover_supported_callers():
    ...
    expected_angle = msm.pyunitwizard.standardize(
        msm.pyunitwizard.quantity(30.0, "degrees")
    )
    assert msm.pyunitwizard.get_value(angle) == pytest.approx(
        msm.pyunitwizard.get_value(expected_angle)
    )
...
def test_cutoff_distance_radius_and_force_digesters_accept_valid_quantities():
    ...
    assert msm.pyunitwizard.get_value(
        force, to_unit="kilojoule/(nanometer mol)"
    ) == pytest.approx(2.0)
...
def test_force_constant_supports_scalar_and_list_outputs_for_supported_callers():
    scalar = digest_force_constant(
        "5 kilojoule/(nanometer**2 mol)", caller=FORCE_CONSTANT_CALLER
    )
    assert len(scalar) == 1
    assert msm.pyunitwizard.get_value(
        scalar[0], to_unit="kilojoule/(nanometer**2 mol)"
    ) == pytest.approx(5.0)
    ...
    assert [
        msm.pyunitwizard.get_value(item, to_unit="kilojoule/(nanometer**2 mol)")
        for item in sequence
    ] == [pytest.approx(5.0), pytest.approx(7.0)]

    single = digest_force_constant("5 kilojoule/(nanometer**2 mol)")
    assert msm.pyunitwizard.get_value(
        single, to_unit="kilojoule/(nanometer**2 mol)"
    ) == pytest.approx(5.0)

    with pytest.raises(ArgumentError):
        digest_force_constant(
            [msm.pyunitwizard.quantity(5.0, "nanometers")], caller=FORCE_CONSTANT_CALLER
        )
