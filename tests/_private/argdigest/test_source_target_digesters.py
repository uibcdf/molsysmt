...
def test_item_source_target_digesters(builder_pdb_molsys, tmp_path):
    ...
    assert (
        digest_from_item(builder_pdb_molsys, form="molsysmt.MolSys")
        is builder_pdb_molsys
    )
    assert (
        digest_to_item(builder_pdb_molsys, form="molsysmt.MolSys") is builder_pdb_molsys
    )
    ...
    assert (
        digest_from_element(
            None, caller="molsysmt.topology.bonds_are_required_to_get_attribute"
        )
        is None
    )
    ...
    assert digest_output_form(["MOLSYSMT.MOLSYS", "molsysmt.Topology"]) == [
        "molsysmt.MolSys",
        "molsysmt.Topology",
    ]
    assert (
        digest_output_indices(
            "atom", caller="molsysmt.structure.get_distances.get_distances"
        )
        == "atom"
    )
    assert (
        digest_output_structure_indices(
            "structure", caller="molsysmt.structure.get_neighbors.get_neighbors"
        )
        == "structure"
    )
    ...
    with pytest.raises(ArgumentError):
        digest_output_indices(
            "bad", caller="molsysmt.structure.get_distances.get_distances"
        )
    with pytest.raises(ArgumentError):
        digest_output_structure_indices(
            "bad", caller="molsysmt.structure.get_neighbors.get_neighbors"
        )
