...
def test_hbond_acceptor_and_donor_digesters():
    ...
    assert np.array_equal(
        digest_acceptors([1, 2], syntax="OpenMM"), np.array([1, 2], dtype="int64")
    )
    ...
    assert np.array_equal(
        digest_donors([6, 7], syntax="OpenMM"), np.array([6, 7], dtype="int64")
    )
...
def test_hbond_auxiliary_digesters():
    ...
    assert (
        digest_disulfide_bonds(True, caller="molsysmt.build.get_missing_bonds") is True
    )
