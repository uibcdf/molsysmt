...
def test_value_digesters_dispatch_to_expected_setter_digesters():
    assert (
        digest_value("CA", caller="molsysmt.form.molsysmt_MolSys.set_atom_name_to_atom")
        == "CA"
    )
    assert (
        digest_value("12", caller="molsysmt.form.molsysmt_MolSys.set_group_id_to_atom")
        == "12"
    )
    assert (
        digest_value(
            "protein", caller="molsysmt.form.molsysmt_MolSys.set_chain_type_to_group"
        )
        == "protein"
    )
    with pytest.raises(ArgumentError):
        digest_value(
            "bad", caller="molsysmt.form.molsysmt_MolSys.set_structure_id_to_system"
        )
