...
def test_digest_non_bonded_method_and_constraints_accept_form_callers_and_strings():
    assert (
        digest_non_bonded_method(
            "PME", caller="molsysmt.form.openmm_Topology.to_openmm_System"
        )
        == "PME"
    )
    ...
    assert (
        digest_constraints(
            "hbonds", caller="molsysmt.form.openmm_Topology.to_openmm_System"
        )
        == "hbonds"
    )
...
def test_the_minimization_default_survives_its_own_digester():
    ...
    target = getattr(
        potential_energy_minimization, "__wrapped__", potential_energy_minimization
    )
