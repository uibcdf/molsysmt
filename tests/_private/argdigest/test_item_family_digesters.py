...
def test_digest_attribute_type_accepts_supported_values():
    ...
    assert (
        digest_attribute_type("all", caller="molsysmt.basic.compare.compare") == "all"
    )
    assert (
        digest_attribute_type("Topological", caller="molsysmt.basic.compare.compare")
        == "topological"
    )
    assert (
        digest_attribute_type(
            "structural", caller="molsysmt.basic.get_attributes.get_attributes"
        )
        == "structural"
    )
    assert (
        digest_attribute_type(
            "mechanical", caller="molsysmt.basic.get_attributes.get_attributes"
        )
        == "mechanical"
    )
...
def test_digest_item_accepts_items_and_form_checks():
    ...
    assert (
        digest_item(None, caller="molsysmt.form.molsysmt_MolSys.append_structures")
        is None
    )
    with pytest.raises(ArgumentError):
...
def test_digest_keys_accepts_mutate_synonyms():
    assert (
        digest_keys("group_index", caller="molsysmt.build.mutate.mutate")
        == "group_index"
    )
    assert (
        digest_keys("residue_indices", caller="molsysmt.build.mutate.mutate")
        == "group_index"
    )
    assert digest_keys("group_id", caller="molsysmt.build.mutate.mutate") == "group_id"
    assert (
        digest_keys("residue_name", caller="molsysmt.build.mutate.mutate")
        == "group_name"
    )
    with pytest.raises(ArgumentError):
