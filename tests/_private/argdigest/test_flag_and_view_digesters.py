...
from molsysmt._private.argdigest.argument.constant_id_step import (
    digest_constant_id_step,
)
from molsysmt._private.argdigest.argument.constant_time_step import (
    digest_constant_time_step,
)
...
def test_boolean_flag_digesters_accept_only_bool():
    ...
    assert digest_allow_reassign(True) is True
    assert digest_allow_reassign(False) is False
    with pytest.raises(ArgumentError):
        digest_progress_bar("yes")
    with pytest.raises(ArgumentError):
        digest_allow_reassign("yes")
...
def test_view_digester_accepts_supported_view_objects():
    if find_spec("nglview") is None:
        pytest.skip("nglview is an optional viewer backend")
