...
from molsysmt._private.argdigest.argument.alternate_location import (
    digest_alternate_location,
)
...
from molsysmt._private.argdigest.argument.compression_opts import (
    digest_compression_opts,
)
...
def test_digest_attribute_accepts_known_attributes_and_aggregates():
    ...
    assert (
        digest_attribute(
            "Topological", caller="molsysmt.basic.has_attribute.has_attribute"
        )
        == "topological"
    )
...
def test_digest_center_accepts_convert_and_align_cases():
    assert (
        digest_center("atom_index==0", caller="molsysmt.basic.convert.convert")
        == "atom_index==0"
    )
    assert np.array_equal(
        digest_center(3, caller="molsysmt.basic.convert.convert"),
    ...
    assert (
        digest_center(
            True, caller="molsysmt.structure.align_principal_axes.align_principal_axes"
        )
        is True
    )
    with pytest.raises(ArgumentError):
...
def test_digest_comparison_and_h5msm_compression_variants():
    assert (
        digest_comparison(
            "equal", rule="equal", caller="molsysmt.basic.compare.compare.compare"
        )
        == "equal"
    )
    assert (
        digest_comparison(
            "in", rule="in", caller="molsysmt.basic.compare.compare.compare"
        )
        == "in"
    )
    with pytest.raises(ArgumentError):
        digest_comparison(
            "equal", rule="in", caller="molsysmt.basic.compare.compare.compare"
        )
...
def test_digest_bioassembly_accepts_boolean_and_declared_formats():
    ...
    assert np.array_equal(
        digested_mapping["bio1"]["chain_indices"][0], np.array([0], dtype="int64")
    )
    ...
    payload = {
        0: {
            ...
            "coordinates": puw.quantity(
                [[[0.0, 0.0, 0.0], [0.1, 0.1, 0.1]]], "nanometers"
            ),
        }
    }
