...
from molsysmt._private.argdigest.argument.bonded_atom_pairs import (
    digest_bonded_atom_pairs,
)
...
def test_angles_and_distance_digesters_normalize_quantities():
    ...
    matrix = digest_angles(
        msm.pyunitwizard.quantity([[90.0, 120.0], [60.0, 180.0]], "degrees")
    )
...
def test_atom_pair_and_atom_pairs_digesters_support_supported_callers():
    ...
    assert digest_atom_pair(np.array([1, 2]), caller=HBOND_CALLER) == [
    ...
    assert np.array_equal(
        digest_atom_pairs([1, 2], caller=CONTACTS_CALLER), np.array([[1, 2]])
    )
    assert np.array_equal(
...
def test_bond_id_index_and_type_digesters_cover_public_callers():
    ...
    assert digest_bond_type(["single", "double"], caller=SELECT_CALLER) == [
        "single",
        "double",
    ]
    assert (
        digest_bond_type(
            None, caller="molsysmt.native.molsys_builder.MolSysBuilder.add_bond"
        )
        is None
    )
