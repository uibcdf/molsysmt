...
def test_id_digesters_normalize_scalars_and_lists():
    assert (
        digest_atom_id(
            7, caller="molsysmt.native.molsys_builder.MolSysBuilder.add_atom"
        )
        == 7
    )
    assert (
        digest_group_id(
            "A", caller="molsysmt.native.molsys_builder.MolSysBuilder.add_group"
        )
        == "A"
    )
    assert (
        digest_chain_id(
            2, caller="molsysmt.native.molsys_builder.MolSysBuilder.add_chain"
        )
        == 2
    )
    assert (
        digest_molecule_id(
            "mol", caller="molsysmt.native.molsys_builder.MolSysBuilder.add_molecule"
        )
        == "mol"
    )
    assert (
        digest_entity_id(
            "ent", caller="molsysmt.native.molsys_builder.MolSysBuilder.add_entity"
        )
        == "ent"
    )
