...
from molsysmt._private.argdigest.argument.flexible_constraints import (
    digest_flexible_constraints,
)
...
from molsysmt._private.argdigest.argument.group_behavior_2 import (
    digest_group_behavior_2,
)
from molsysmt._private.argdigest.argument.groups_of_atoms import digest_groups_of_atoms
from molsysmt._private.argdigest.argument.groups_of_atoms_2 import (
    digest_groups_of_atoms_2,
)
...
def test_constraints_precision_exclusion_decomposition_and_fit_digesters():
    ...
    assert (
        digest_float_precision(
            "single", caller="molsysmt.form.molsysmt_MolSys.to_file_h5msm"
        )
        == "single"
    )
    assert (
        digest_float_precision(
            "double", caller="molsysmt.form.molsysmt_Structures.to_file_h5msm"
        )
        == "double"
    )
    with pytest.raises(ArgumentError):
    ...
    assert (
        digest_decomposition(
            True,
            caller="molsysmt.molecular_mechanics.get_potential_energy.get_potential_energy",
        )
        is True
    )
    with pytest.raises(ArgumentError):
    ...
    np.testing.assert_array_equal(
        digest_fit(3, caller=convert_caller), np.array([3], dtype="int64")
    )
    np.testing.assert_array_equal(
        digest_fit([0, 2], caller=convert_caller), np.array([0, 2], dtype="int64")
    )
    with pytest.raises(ArgumentError):
        digest_fit('name == "CA"')
