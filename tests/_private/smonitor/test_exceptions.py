...
class TestArgumentChoiceError:
    ...
    def test_with_caller(self):
        exc = ArgumentChoiceError(
            argument="engine", value="x", choices=["A", "B"], caller="msm.get"
        )
        assert isinstance(exc, Exception)
...
class TestArgumentLengthError:
    ...
    def test_with_caller(self):
        exc = ArgumentLengthError(
            argument="values", expected=3, actual=5, caller="msm.set"
        )
        assert isinstance(exc, Exception)
...
class TestArgumentConflictError:
    def test_basic(self):
        exc = ArgumentConflictError(
            arg1="selection", arg2="atom_indices", reason="cannot use both"
        )
        assert isinstance(exc, Exception)

    def test_with_caller(self):
        exc = ArgumentConflictError(
            arg1="a", arg2="b", reason="conflict", caller="msm.get"
        )
        assert isinstance(exc, Exception)
...
class TestNotCompatibleConversionError:
    def test_basic(self):
        exc = NotCompatibleConversionError(
            from_form="molsysmt.MolSys",
            to_form="rdkit.Mol",
    ...
    def test_with_caller(self):
        exc = NotCompatibleConversionError(
            from_form="A", to_form="B", missing_arguments={"x"}, caller="convert"
        )
        assert isinstance(exc, Exception)
...
class TestNotImplementedConversionError:
    def test_basic(self):
        exc = NotImplementedConversionError(
            from_form="molsysmt.MolSys", to_form="unknown.Form"
        )
        assert isinstance(exc, Exception)

    def test_with_caller(self):
        exc = NotImplementedConversionError(
            from_form="A", to_form="B", caller="convert"
        )
        assert isinstance(exc, Exception)
...
class TestNotWithThisFormError:
    ...
    def test_with_form_and_attribute(self):
        exc = NotWithThisFormError(
            caller="get", form="file:xyz", requested_attribute="velocities"
        )
        assert isinstance(exc, Exception)
...
class TestFileContentError:
    ...
    def test_with_all_optional(self):
        exc = FileContentError(
            reason="bad line", caller="parser", record="ATOM", filename="1abc.pdb"
        )
        assert isinstance(exc, Exception)
...
class TestUnsupportedHeavyOperationError:
    def test_basic(self):
        exc = UnsupportedHeavyOperationError(
...
class TestHeavyOutputFailureError:
    ...
    def test_with_optional_bytes(self):
        exc = HeavyOutputFailureError(
            reason="oom", predicted_bytes=1000000, available_bytes=500000
        )
        assert isinstance(exc, Exception)
