...
def test_extract_with_output_filename(t4_h5msm_molsys, tmp_path):
    ...
    msm.extract(
        t4_h5msm_molsys, selection='molecule_type=="protein"', output_filename=out
    )
...
def test_extract_molecular_mechanics_subsets_per_atom_parameters():
    ...
    np.testing.assert_allclose(
        np.asarray(result.partial_charge, dtype=float), [0.3, 0.1]
    )
...
def test_extract_sequence_uses_residue_positions():
    ...
    assert msm.extract("amino_acids_3:AlaValPro", selection=[1]) == "amino_acids_3:Val"
