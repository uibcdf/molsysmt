...
def test_append_structures_with_molsysmt_MolSys_1(proline_molsys):
    ...
    molsys_B = msm.structure.translate(
        molsys_A, translation="[0.1, 0.1, 0.1] nanometers"
    )
    molsys_C = msm.structure.translate(
        molsys_A, translation="[0.2, 0.2, 0.2] nanometers"
    )
    n_atoms_A, n_structures_A = msm.get(
...
def test_append_structures_with_molsysmt_MolSys_2(proline_molsys):
    ...
    n_atoms_B, n_structures_B = msm.get(
        molsys_B, element="system", n_atoms=True, n_structures=True
    )
    n_atoms_C, n_structures_C = msm.get(
        molsys_C, element="system", n_atoms=True, n_structures=True
    )
    assert "molsysmt.MolSys" == msm.get_form(molsys_A)
...
def test_append_structures_keeps_single_state_association_implicit():
    ...
    target.structures.coordinates = msm.pyunitwizard.quantity(np.zeros((1, 1, 3)), "nm")
    source = Structures(
