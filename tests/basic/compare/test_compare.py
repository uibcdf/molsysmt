...
def test_compare_all_eq_1(t4_h5msm_molsys):
    ...
    output = msm.compare(
        molsys_A, molsys_B, attributes_type="topological", coordinates=True, box=True
    )
    assert output
...
def test_compare_all_eq_3():
    molsys_A = msm.convert(
        systems["T4 lysozyme L99A"]["181l.h5msm"], to_form="openmm.Modeller"
    )
...
def test_compare_all_eq_4():
    ...
    molsys_A = msm.convert(
        systems["T4 lysozyme L99A"]["181l.pdb"], to_form="openmm.Modeller"
    )
    molsys_B = msm.convert(molsys_A, to_form="molsysmt.MolSys")
    ...
    assert not msm.compare(molsys_A, molsys_C)
    assert msm.compare(molsys_A, molsys_B, coordinates=True, box=True, n_groups=True)
    assert not msm.compare(
        molsys_B, molsys_C, coordinates=True, box=True, n_groups=True
    )
    assert msm.compare(molsys_B, molsys_C, coordinates=False, box=True, n_groups=False)
