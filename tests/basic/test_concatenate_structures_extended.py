...
def test_concatenate_selections_length_mismatch(
    traj_pentalanine_h5_molsys, proline_molsys
):
    ...
    with pytest.raises(ArgumentLengthError):
        msm.concatenate_structures(
            [traj_pentalanine_h5_molsys, proline_molsys], selections=["all"]
        )


def test_concatenate_structure_indices_length_mismatch(
    traj_pentalanine_h5_molsys, proline_molsys
):
    """structure_indices list with wrong length raises ArgumentLengthError."""
    with pytest.raises(ArgumentLengthError):
        msm.concatenate_structures(
            [traj_pentalanine_h5_molsys, proline_molsys], structure_indices=["all"]
        )
