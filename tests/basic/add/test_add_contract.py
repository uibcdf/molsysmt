...
def test_a_one_sided_atom_aligned_attribute_is_dropped_with_a_warning(
    ...
    expected_atoms = (
        target.structures.coordinates.shape[1] + source.structures.coordinates.shape[1]
    )
...
def test_an_attribute_present_on_both_sides_is_concatenated(
...
def test_a_structure_count_mismatch_leaves_the_target_untouched(
    proline_molsys, valine_molsys
):
    ...
    source.structures.coordinates = puw.quantity(
        np.repeat(coordinates, 2, axis=0), "nm"
    )
...
def test_in_place_true_returns_none_and_mutates_the_target(
    proline_molsys, valine_molsys
):
    expected = msm.get(proline_molsys, n_atoms=True) + msm.get(
        valine_molsys, n_atoms=True
    )
...
def test_the_source_selection_is_applied_before_concatenating(
    proline_molsys, valine_molsys
):
...
def test_a_string_selection_applies_to_the_assembled_composite_source(proline_molsys):
    ...
    result = msm.add(
        proline_molsys, [prmtop, inpcrd], selection='atom_type=="C"', in_place=False
    )
...
def test_structure_indices_select_structures_of_the_source(
    proline_molsys, valine_molsys
):
    source = valine_molsys
    coordinates = puw.get_value(source.structures.coordinates, to_unit="nm")
    source.structures.coordinates = puw.quantity(
        np.repeat(coordinates, 3, axis=0), "nm"
    )
    n_target = msm.get(proline_molsys, n_atoms=True)
