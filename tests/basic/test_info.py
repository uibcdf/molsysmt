...
def test_info_2(tctim_h5msm_molsys):
    ...
    true_dict = {
        "index": {0: 45, 1: 46, 2: 47, 3: 48, 4: 49},
...
def test_info_4(tctim_h5msm_molsys):
    ...
    true_dict = {
        ...
        "entity name": {0: "TRIOSEPHOSPHATE ISOMERASE", 1: "TRIOSEPHOSPHATE ISOMERASE"},
    }
    true_df = DataFrame(true_dict)
    assert df.data.equals(true_df)
...
def test_info_10(tctim_h5msm_molsys):
    ...
    df = msm.info(molsys)
    true_dict = {
        "form": {0: ["molsysmt.Topology", "molsysmt.Structures"]},
        "n_atoms": {0: 3983},
        "n_groups": {0: 662},
        "n_components": {0: 167},
        "n_chains": {0: 4},
...
def test_info_12(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    group_index_in_component_0 = msm.get(
        molsys, element="group", selection="component_index==0", index=True
    )[69]
    group_index_in_component_1 = msm.get(
        molsys, element="group", selection="component_index==1", index=True
    )[12]
    df = msm.info(
        molsys,
        element="group",
        selection=[group_index_in_component_0, group_index_in_component_1],
    )
    true_dict = {
        "index": {0: 69, 1: 260},
        "id": {0: "73", 1: "15"},
...
def test_info_styler_table_carries_the_dataframe_class(tctim_h5msm_molsys):
    ...
    for element, styler in stylers.items():
        ...
        assert 'class="dataframe"' in html, (
            f"info(element={element!r}) lost the dataframe class"
        )
