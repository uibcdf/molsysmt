...
def test_get_nglview_NGLWidget_1():
    molsys = msm.convert(
        [msm.systems["nglview"]["md_1u19.gro"], msm.systems["nglview"]["md_1u19.xtc"]],
        to_form="molsysmt.MolSys",
    )
    view = msm.convert(molsys, to_form="nglview.NGLWidget")
    check_comparison = msm.compare(
        molsys,
        view,
...
def test_get_nglview_NGLWidget_preserves_chain_and_entity_semantics_for_t4():
    ...
    assert msm.get(view, element="chain", chain_type=True) == msm.get(
        molsys, element="chain", chain_type=True
    )
    assert msm.get(view, element="molecule", molecule_type=True) == msm.get(
        molsys, element="molecule", molecule_type=True
    )
    assert msm.get(view, element="entity", entity_type=True) == msm.get(
        molsys, element="entity", entity_type=True
    )
