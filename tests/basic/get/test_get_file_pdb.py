...
def test_get_file_pdb_system_counts(t4_pdb_file, t4_pdb_molsys):
    n_atoms, n_groups = msm.get(
        t4_pdb_file, element="system", n_atoms=True, n_groups=True
    )
...
def test_get_file_pdb_single_attribute_through_topology_pipe(
    t4_pdb_file, t4_pdb_molsys
):
    atom_types = msm.get(
        t4_pdb_file, element="atom", selection=[0, 1, 2], atom_type=True
    )
    expected = msm.get(
        t4_pdb_molsys, element="atom", selection=[0, 1, 2], atom_type=True
    )
...
def test_get_file_pdb_multimodel_counts(md_1u19_pdb_file, md_1u19_pdb_molsys):
    n_atoms, n_structures = msm.get(
        md_1u19_pdb_file, element="system", n_atoms=True, n_structures=True
    )
