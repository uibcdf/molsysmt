...
def test_select_8(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, 'atom_type=="C" and not atom_name=="CA" and group_index==[2,3]'
    )
...
def test_select_13(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, 'molecule_type=="protein" and atom_type!="H" and group_index==3'
    )
    true_output = np.array([25, 26, 27, 28, 29, 30, 31])
    assert np.all(output == true_output)
...
def test_select_14(tctim_h5msm_molsys):
    ...
    output = msm.select(
        molsys,
        'group_name==["GLY","ALA","VAL"] and chain_name=="A" and group_index==[2,3,4,5,6]',
    )
...
def test_select_15(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    # Selection expressions resolve these local names at runtime.
    indices = [10, 11, 12]  # noqa: F841
...
def test_select_16(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    indices = list(range(10, 20))  # noqa: F841
    atoms = ["CA", "C", "O", "N"]  # noqa: F841
...
def test_select_17(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    indices = list(range(10, 30))  # noqa: F841
...
def test_select_20(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, 'group_name=="ALA" and group_index==[2,3,4,5,6,7]', element="group"
    )
...
def test_select_22(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, 'chain_name==["A","C"] and molecule_type!="water"', element="group"
    )
...
def test_select_31(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, 'chain_id=="A" within 0.3 nm without pbc of chain_id=="B"'
    )
    true_output = np.array([89, 480, 527, 547, 550, 552, 554, 566, 723, 734])
    assert np.all(output == true_output)
...
def test_select_33(tctim_h5msm_molsys):
    ...
    output = msm.select(
        molsys,
        '(atom_name=="N" and chain_id=="A") within 3 angstroms of (atom_type=="O" and molecule_type=="water")',
    )
...
def test_select_34(tctim_h5msm_molsys):
    ...
    output = msm.select(
        molsys,
        '(atom_name=="CA" and chain_id=="A") within 0.5 nm of (atom_name=="CA" and chain_name=="B")',
        element="group",
    )
...
def test_select_37(tctim_h5msm_molsys):
    ...
    output = msm.select(
        molsys,
        '((atom_type=="O" and chain_name=="A") bonded to (atom_type=="C" and chain_name=="A")) and group_index==[3,4]',
    )
...
def test_select_38(tctim_h5msm_molsys):
    ...
    output = msm.select(
        molsys,
        '((atom_name=="N" and chain_name=="A") bonded to atom_type=="C") within 3 angstroms of (atom_type=="O" and molecule_type=="water")',
    )
    true_output = np.array([119, 213, 473, 531, 654, 696, 799, 1049])
    assert np.all(output == true_output)
...
def test_select_41(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, element="group", selection="group_index==[3,4,5]", to_syntax="NGLView"
    )
...
def test_select_42(tctim_h5msm_molsys):
    molsys = tctim_h5msm_molsys
    output = msm.select(
        molsys, element="group", selection="group_index==[3,4,5]", to_syntax="MDTraj"
    )
