...
barnase_barstar.topology.reset_chains(n_chains=1)
msm.set(barnase_barstar, element='atom', selection='all', chain_index=[0], skip_digestion=True)
msm.set(barnase_barstar, element='chain', selection='all', chain_id=['0'], chain_name=['A'], skip_digestion=True)
barnase_barstar.topology.rebuild_chains(redefine_indices=False, redefine_ids=False, redefine_types=True, redefine_names=False)
