...
#### Protein Data Bank ####
# This generator loads the CIF handler after its local helper definitions.
from molsysmt.native import CIFFileHandler  # noqa: E402
...
for value in data:
    ...
    tmp_dict["topology"] = []
    tmp_dict["topology"].append(
        {"atoms": key["_chem_comp_atom"]["atom_id"], "bonds": []}
    )
    tmp_dict["topology"].append(
        {"atoms": key["_chem_comp_atom"]["alt_atom_id"], "bonds": []}
    )
    if "_chem_comp_bond" in key:
        for atom1, atom2 in zip(
            key["_chem_comp_bond"]["atom_id_1"], key["_chem_comp_bond"]["atom_id_2"]
        ):
