...
def to_molsysmt_Topology(item, atom_indices="all", skip_digestion=False):
    ...
    from molsysmt.form.molsysmt_H5MSMFileHandler.to_molsysmt_Topology import (
        to_molsysmt_Topology as molsysmt_H5MSMFileHandler_to_molsysmt_Topology,
    )

    handler = to_molsysmt_H5MSMFileHandler(item, skip_digestion=True)
    tmp_item = molsysmt_H5MSMFileHandler_to_molsysmt_Topology(
        handler, atom_indices=atom_indices, skip_digestion=True
    )
    handler.close()
