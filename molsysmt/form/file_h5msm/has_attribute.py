...
def has_attribute(molecular_system, attribute, include_none=False, skip_digestion=True):
    """
    Checking if form file:h5msm supports a specific attribute.
    ...
    skip_digestion : bool, default=True
    ...
    if not include_none:
        if attribute in {
            ...
            output = ("b_factor" in tmp_item.file["structures"]) and (
                tmp_item.file["structures"]["b_factor"].shape[0] > 0
            )
            tmp_item.close()
