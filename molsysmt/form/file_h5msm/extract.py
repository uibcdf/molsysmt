...
def extract(
    ...
    """
    Extracting a subset of elements or structures from form file:h5msm.
    ...
    output_file_handler = H5MSMFileHandler(
        output_filename,
        io_mode="w",
        int_precision=int_precision,
        float_precision=float_precision,
    ...
    if is_all(atom_indices):
        ...
        if n_groups:
            ...
            topo_out["groups"]["group_id"][:] = topo_in["groups"]["group_id"][
                group_indices
            ]
            topo_out["groups"]["group_name"][:] = topo_in["groups"]["group_name"][
                group_indices
            ]
            topo_out["groups"]["group_type"][:] = topo_in["groups"]["group_type"][
                group_indices
            ]
            ...
            topo_out["groups"]["molecule_index"][:] = new_molecule_index.astype(
                int_type
            )
        else:
        ...
        if n_molecules:
            ...
            topo_out["molecules"]["molecule_id"][:] = topo_in["molecules"][
                "molecule_id"
            ][molecule_indices]
            topo_out["molecules"]["molecule_name"][:] = topo_in["molecules"][
                "molecule_name"
            ][molecule_indices]
            topo_out["molecules"]["molecule_type"][:] = topo_in["molecules"][
                "molecule_type"
            ][molecule_indices]

            entity_indices, new_entity_index = np.unique(
                topo_in["molecules"]["entity_index"][molecule_indices],
                return_inverse=True,
            )
        ...
        if n_bonds:
            ...
            topo_out["bonds"]["atom1_index"][:] = np.searchsorted(
                atom_indices,
                topo_in["bonds"]["atom1_index"][mask].astype(np.int64),
                side="left",
            )
            topo_out["bonds"]["atom2_index"][:] = np.searchsorted(
                atom_indices,
                topo_in["bonds"]["atom2_index"][mask].astype(np.int64),
                side="left",
            )
        ...
        if structs_in["id"].shape[0]:
            if structs_in.attrs["constant_id_step"]:
                if equispaced:
                    ...
                    structs_out["id"][0] = (
                        structs_in["id"][0]
                        + structure_indices[0] * structs_in.attrs["id_step"]
                    )
                else:
                    structs_out["id"].resize((n_structures,))
                    structs_out["id"][:] = (
                        structs_in["id"][0]
                        + structure_indices * structs_in.attrs["id_step"]
                    )
            else:
                structs_out["id"].resize((n_structures,))
        ...
        if structs_in["time"].shape[0]:
            if structs_in.attrs["constant_time_step"]:
                if equispaced:
                    ...
                    structs_out["time"][0] = (
                        structs_in["time"][0]
                        + structure_indices[0] * structs_in.attrs["time_step"]
                    )
                else:
                    structs_out["time"].resize((n_structures,))
                    structs_out["time"][:] = (
                        structs_in["time"][0]
                        + structure_indices * structs_in.attrs["time_step"]
                    )
            else:
                structs_out["time"].resize((n_structures,))
        ...
        if is_all(atom_indices):
            ...
            if structs_in["coordinates"].shape[0]:
                ...
                for count, ii in enumerate(structure_indices):
                    structs_out["coordinates"][count, :, :] = structs_in["coordinates"][
                        ii, atom_indices, :
                    ]
            if structs_in["velocities"].shape[0]:
                structs_out["velocities"].resize((n_structures, n_at, 3))
                for count, ii in enumerate(structure_indices):
                    structs_out["velocities"][count, :, :] = structs_in["velocities"][
                        ii, atom_indices, :
                    ]
