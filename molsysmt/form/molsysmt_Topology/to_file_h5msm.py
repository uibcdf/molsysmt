...
def dump_topology_to_h5msm(item, file, atom_indices="all"):
    ...
    atoms["group_index"][:] = (
        atoms_df["group_index"].fillna(-1).to_numpy(dtype=int_type)
    )
    atoms["chain_index"][:] = (
        atoms_df["chain_index"].fillna(-1).to_numpy(dtype=int_type)
    )
    ...
    if n_groups > 0:
        ...
        groups["molecule_index"][:] = (
            groups_df["molecule_index"].fillna(-1).to_numpy(dtype=int_type)
        )
    ...
    if n_molecules > 0:
        ...
        molecules["molecule_name"][:] = molecules_df["molecule_name"].to_numpy(
            dtype="S"
        )
        molecules["molecule_type"][:] = molecules_df["molecule_type"].to_numpy(
            dtype="S"
        )
        molecules["entity_index"][:] = (
            molecules_df["entity_index"].fillna(-1).to_numpy(dtype=int_type)
        )
