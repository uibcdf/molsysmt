...
def to_mdtraj_Topology(item, atom_indices="all", skip_digestion=False):
    ...
    import pandas as pd
    from mdtraj import Aromatic, Double, Single, Topology, Triple
    ...
    group_chain_mapping = (
        item.atoms.groupby("group_index")["chain_index"].agg("first").to_dict()
    )

    for group in item.groups.itertuples(index=True):
        tmp_residue = tmp_item.add_residue(
            group.group_name,
            list_new_chains[group_chain_mapping[group.Index]],
    ...
    for atom in item.atoms.itertuples(index=True):
        ...
        tmp_atom = tmp_item.add_atom(
            atom.atom_name,
            elem,
            list_new_residues[atom.group_index],
            atom.atom_id,
    ...
    for bond in bonds.itertuples(index=False):
        ...
        if (
            hasattr(bond, "is_aromatic")
            and not pd.isna(bond.is_aromatic)
            and bond.is_aromatic
        ):
            bond_type = Aromatic
        tmp_item.add_bond(
            list_new_atoms[bond.atom1_index],
            list_new_atoms[bond.atom2_index],
            type=bond_type,
            order=order,
        )
