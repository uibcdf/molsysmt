...
def _group_records(item):
    records = []
    previous_key = None
    for segment_id, residue_id, residue_name in zip(
        item.segid, item.resno, item.resname
    ):
        key = (segment_id, residue_id)
        if key != previous_key:
...
def get_group_type_from_atom(item, indices="all", skip_digestion=False):
    ...
    values = [
        get_group_type_from_group_name(name, skip_digestion=True)
        for name in item.resname
    ]
    return _select(values, indices)
