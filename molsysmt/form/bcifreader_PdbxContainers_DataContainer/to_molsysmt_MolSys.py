...
def to_molsysmt_MolSys(item, atom_indices='all', structure_indices='all', skip_digestion=False):
    ...
    for chain_index, group_indices in chain_index_to_group_indices.items():
        ...
        if aux_entity_type in ['protein', 'peptide', 'polysaccharide', 'DNA', 'RNA']:
            molecule_index+=1
    ...
    for chain_index, group_indices in chain_index_to_group_indices.items():
        ...
        print(aux_entity_index, aux_entity_type, aux_entity_name)

        if aux_entity_type in ['protein', 'peptide', 'polysaccharide', 'DNA', 'RNA']:
            if aux_entity_polymer_type[aux_entity_index]['poly_type'] not in ['polypeptide(L)']:
                former_group_id = -10
    ...
    if len(alt_atom_indices):
        ...
        b_factor = b_factor[0,atom_indices_to_be_kept]
    ...
    if len(atoms_without_bonds):
        ...
        if len(missing_bonds):
            ...
            bond_atom1_index = np.concatenate([bond_atom1_index, missing_bonds_array[:,0]])
            bond_atom2_index = np.concatenate([bond_atom2_index, missing_bonds_array[:,1]])
