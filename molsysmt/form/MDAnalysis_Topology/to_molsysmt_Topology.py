...
def to_molsysmt_Topology(item, atom_indices="all", skip_digestion=False):
    """
    Converting from MDAnalysis.Topology to molsysmt.Topology.
    ...
    from molsysmt.form.molsysmt_Topology.extract import (
        extract as extract_molsysmt_Topology,
    )
    ...
    # Atoms
    atom_name = (
        list(item.names.values)
        if hasattr(item, "names")
        else [str(i) for i in range(n_atoms)]
    )
    atom_id = (
        list(item.ids.values.astype(str))
        if hasattr(item, "ids")
        else [str(i) for i in range(n_atoms)]
    )
    atom_type = (
        list(item.types.values)
        if hasattr(item, "types")
        else ["" for _ in range(n_atoms)]
    )
    ...
    # Groups
    group_name = (
        list(item.resnames.values)
        if hasattr(item, "resnames")
        else ["" for _ in range(n_groups)]
    )
    group_id = (
        list(item.resids.values.astype(str))
        if hasattr(item, "resids")
        else [str(i) for i in range(n_groups)]
    )
    ...
    # Chains
    segids = (
        list(item.segids.values)
        if hasattr(item, "segids")
        else [str(i) for i in range(n_chains)]
    )
    ...
    if not is_all(atom_indices):
        tmp_item = extract_molsysmt_Topology(
