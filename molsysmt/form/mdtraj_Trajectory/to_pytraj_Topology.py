from molsysmt._private.argdigest import arg_digest


@arg_digest(form="mdtraj.Trajectory")
def to_pytraj_Topology(item, atom_indices="all", skip_digestion=False):
    """
    Converting from mdtraj.Trajectory to pytraj.Topology.


    Parameters
    ----------
    item : molecular system
        Argument item.
    atom_indices : int, list, tuple, or numpy.ndarray, default='all'
        Atom indices (0-based) to include.
    skip_digestion : bool, default=False
        Whether to skip MolSysMT's internal argument digestion mechanism.

    Returns
    -------
    pytraj.Topology
        Resulting object in pytraj.Topology form.


    .. versionadded:: 1.0.0
    """

    from molsysmt.form.molsysmt_MolSys.to_pytraj_Topology import (
        to_pytraj_Topology as molsysmt_MolSys_to_pytraj_Topology,
    )

    from .to_molsysmt_MolSys import to_molsysmt_MolSys

    tmp_item = to_molsysmt_MolSys(item, atom_indices=atom_indices, skip_digestion=True)
    tmp_item = molsysmt_MolSys_to_pytraj_Topology(tmp_item, skip_digestion=True)

    return tmp_item
