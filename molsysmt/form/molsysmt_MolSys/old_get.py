...
def get_n_aminoacids_from_atom (item, indices='all'):
    ...
    from molsysmt.form.molsysmt_Topology import get_n_aminoacids_from_atom as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_n_peptides_from_atomNEW (item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_atom as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_proteins_from_atomNEW (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_atom as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_molecule_type_from_group (item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_molecule_type_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_index_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_index_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_id_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_id_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_name_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_name_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_type_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_type_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_atoms_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_atoms_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_groups_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_groups_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_components_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_components_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_molecules_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_molecules_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_chains_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_chains_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_entities_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_entities_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_bonds_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_aminoacids_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_aminoacids_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_nucleotides_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_nucleotides_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_ions_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_ions_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_waters_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_waters_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_small_molecules_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_small_molecules_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_peptides_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_proteins_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_dnas_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_dnas_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_rnas_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_rnas_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_lipids_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_lipids_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_polysaccharides_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_polysaccharides_from_group as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_saccharides_from_group (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_saccharides_from_group as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_atom_index_from_component(item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_atom_index_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_id_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_id_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_name_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_name_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_type_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_type_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_index_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_index_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_id_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_id_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_name_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_name_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_type_from_component(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_type_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_name_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_name_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_index_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_index_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_id_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_id_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_type_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_type_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_name_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_name_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_index_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_index_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_id_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_id_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_type_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_type_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_index_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_index_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_id_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_id_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_name_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_name_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_type_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_type_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_index_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_index_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_id_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_id_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_name_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_name_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_type_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_type_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_atoms_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_atoms_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_groups_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_groups_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_components_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_components_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_molecules_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_molecules_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_chains_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_chains_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_entities_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_entities_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_bonds_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_aminoacids_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_aminoacids_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_nucleotides_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_nucleotides_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_ions_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_ions_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_waters_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_waters_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_small_molecules_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_small_molecules_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_peptides_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_proteins_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_dnas_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_dnas_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_rnas_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_rnas_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_lipids_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_lipids_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_polysaccharides_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_polysaccharides_from_component as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_saccharides_from_component (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_saccharides_from_component as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_atom_index_from_molecule(item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_atom_index_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_id_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_id_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_name_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_name_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_type_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_type_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_index_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_index_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_id_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_id_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_name_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_name_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_type_from_molecule(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_type_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_name_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_name_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_index_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_index_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_id_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_id_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_type_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_type_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_name_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_name_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_index_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_index_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_id_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_id_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_type_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_type_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_index_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_index_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_id_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_id_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_name_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_name_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_type_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_type_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_index_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_index_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_id_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_id_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_name_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_name_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_type_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_type_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_atoms_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_atoms_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_groups_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_groups_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_components_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_components_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_molecules_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_molecules_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_chains_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_chains_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_entities_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_entities_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_bonds_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_aminoacids_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_aminoacids_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_nucleotides_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_nucleotides_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_ions_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_ions_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_waters_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_waters_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_small_molecules_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_small_molecules_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_peptides_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_proteins_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_dnas_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_dnas_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_rnas_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_rnas_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_lipids_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_lipids_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_polysaccharides_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_polysaccharides_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_saccharides_from_molecule (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_saccharides_from_molecule as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_atom_index_from_chain(item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_atom_index_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_id_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_id_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_name_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_name_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_type_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_type_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_index_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_index_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_id_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_id_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_name_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_name_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_type_from_chain(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_type_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_name_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_name_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_index_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_index_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_id_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_id_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_type_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_type_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_name_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_name_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_index_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_index_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_id_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_id_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_type_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_type_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_index_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_index_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_id_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_id_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_name_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_name_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_type_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_type_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_index_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_index_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_id_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_id_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_name_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_name_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_type_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_type_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_atoms_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_atoms_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_groups_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_groups_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_components_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_components_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_molecules_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_molecules_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_chains_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_chains_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_entities_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_entities_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_bonds_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_aminoacids_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_aminoacids_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_nucleotides_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_nucleotides_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_ions_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_ions_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_waters_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_waters_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_small_molecules_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_small_molecules_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_peptides_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_proteins_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_dnas_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_dnas_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_rnas_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_rnas_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_lipids_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_lipids_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_polysaccharides_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_polysaccharides_from_chain as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_saccharides_from_chain (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_saccharides_from_chain as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_atom_index_from_entity(item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_atom_index_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_id_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_id_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_name_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_name_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_atom_type_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_atom_type_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_index_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_index_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_id_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_id_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_name_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_name_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_group_type_from_entity(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_group_type_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_name_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_name_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_index_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_index_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_id_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_id_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_component_type_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_component_type_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_name_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_name_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_index_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_index_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_id_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_id_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_chain_type_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_chain_type_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_index_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_index_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_id_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_id_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_name_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_name_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_molecule_type_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_molecule_type_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_index_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_index_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_id_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_id_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_name_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_name_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_entity_type_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_entity_type_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_atoms_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_atoms_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_groups_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_groups_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_components_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_components_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_molecules_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_molecules_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_chains_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_chains_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_entities_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_entities_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_bonds_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_aminoacids_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_aminoacids_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_nucleotides_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_nucleotides_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_ions_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_ions_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_waters_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_waters_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_small_molecules_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_small_molecules_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_peptides_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_proteins_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_dnas_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_dnas_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_rnas_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_rnas_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_lipids_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_lipids_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_polysaccharides_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_polysaccharides_from_entity as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_saccharides_from_entity (item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_saccharides_from_entity as aux_get
    return aux_get(item.topology, indices=indices)
...
def get_n_atoms_from_system(item):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_n_atoms_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_groups_from_system(item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_groups_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_components_from_system(item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_components_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_chains_from_system(item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_chains_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_molecules_from_system(item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_molecules_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_entities_from_system(item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_entities_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_bonds_from_system(item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_aminoacids_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_aminoacids_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_nucleotides_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_nucleotides_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_ions_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_ions_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_waters_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_waters_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_small_molecules_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_small_molecules_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_peptides_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_peptides_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_proteins_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_proteins_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_dnas_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_dnas_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_rnas_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_rnas_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_lipids_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_lipids_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_polysaccharides_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_polysaccharides_from_system as aux_get
    return aux_get(item.topology)

@arg_digest(form=form)
def get_n_saccharides_from_system (item):

    from molsysmt.form.molsysmt_TopologyNew import get_n_saccharides_from_system as aux_get
    return aux_get(item.topology)
...
def get_bonded_atoms_from_system(item):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_bonded_atoms_from_system as aux_get
    return aux_get(item.topology)
...
def get_bond_index_from_bond(item, indices='all'):
    ...
    from molsysmt.form.molsysmt_TopologyNew import get_bond_index_from_bond as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_bond_order_from_bond(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_bond_order_from_bond as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_bond_type_from_bond(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_bond_type_from_bond as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_bonded_atoms_from_bond(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_bonded_atoms_from_bond as aux_get
    return aux_get(item.topology, indices=indices)

@arg_digest(form=form)
def get_n_bonds_from_bond(item, indices='all'):

    from molsysmt.form.molsysmt_TopologyNew import get_n_bonds_from_bond as aux_get
    return aux_get(item.topology, indices=indices)
