...
def to_molsysmt_MolSys(
    ...
    from molsysmt.element.group import (
    ...
    for atom_index, atom_record in enumerate(item.getObj("atom_site").data):
        ...
        if atom_record[index_att["pdbx_PDB_ins_code"]] not in ["?", "."]:
            aux_group_id = (
                str(aux_group_id) + atom_record[index_att["pdbx_PDB_ins_code"]]
            )
    ...
    if item.exists("entity_poly"):
        index_att = {
            jj: ii
            for ii, jj in enumerate(item.getObj("entity_poly").getAttributeList())
        }
        ...
        index_att = {
            jj: ii
            for ii, jj in enumerate(item.getObj("entity_poly_seq").getAttributeList())
        }
        ...
        for record in item.getObj("entity_poly_seq").data:
            ...
            aux_entity_polymer_type[entity_index]["seq"].append(
                [record[index_att["num"]], record[index_att["mon_id"]]]
            )
    ...
    index_att = {
        jj: ii for ii, jj in enumerate(item.getObj("chem_comp").getAttributeList())
    }
    ...
    if item.exists("pdbx_entity_nonpoly"):
        index_att = {
            ...
            for ii, jj in enumerate(
                item.getObj("pdbx_entity_nonpoly").getAttributeList()
            )
        }
    ...
    index_att = {
        jj: ii for ii, jj in enumerate(item.getObj("entity").getAttributeList())
    }
    ...
    for entity_index, record in enumerate(item.getObj("entity").data):
        if record[index_att["type"]] == "branched":
            ...
            aux_dict["group_indices_per_molecule"] = [
                aux_group_indices[ii : ii + num_of_groups_per_molecule]
                for ii in range(0, num_of_groups, num_of_groups_per_molecule)
            ]
            aux_dict["seq"] = list(
                group_name[chain_index_to_group_indices[entity_index]]
            )
        ...
        match record[index_att["type"]]:
            case "polymer":
                match aux_entity_polymer_type[entity_index]["poly_type"]:
                    case "polypeptide(L)":
                        if (
                            len(aux_entity_polymer_type[entity_index]["seq"])
                            >= min_length_protein
                        ):
            ...
            case "non-polymer":
                match aux_entity_non_polymer_type[entity_index]["comp_type"]:
                    ...
                    case "organic compound":
                        aux_entity_type = "small molecule"
                    case _:
                        aux_entity_type = get_group_type_from_group_name(
                            aux_entity_non_polymer_type[entity_index]["comp_id"]
                        )
            case "branched":
                if all(
                    [
    ...
    if item.exists("chem_comp_bond"):
        ...
        for aux_group_index, aux_atom_indices in group_index_to_atom_indices.items():
            ...
            if aux_group_name in bonds_intra_group:
                ...
                for at1, at2, metadata in bonds_intra_group[aux_group_name]:
                    try:
                        aux_bond_records.append(
                            (sorted([dict_aux[at1], dict_aux[at2]]), metadata)
                        )
                        dict_mask[at1] = True
                        dict_mask[at2] = True
                    except Exception:
                ...
                if len(remains):
                    if set(remains) == set(["H1", "H3"]):
                        ...
                        aux_atom_pairs_bonded = get_bonded_atom_pairs(
                            aux_group_name,
                            aux_atom_names,
                            aux_atom_indices,
                            sorted=False,
        ...
        for aux_group_index, aux_atom_indices in group_index_to_atom_indices.items():
            ...
            aux_group_name = group_name[aux_group_index]
            aux_atom_pairs_bonded = get_bonded_atom_pairs(
                aux_group_name, aux_atom_names, aux_atom_indices
            )
            if aux_atom_pairs_bonded is None:
                atoms_without_bonds += aux_atom_indices
            else:
    ...
    for chain_index, group_indices in chain_index_to_group_indices.items():
        ...
        if aux_entity_type in ["protein", "peptide"]:
            if (
                aux_entity_polymer_type[aux_entity_index]["poly_type"]
                == "polypeptide(L)"
            ):
                former_group_id = -10
                ...
                for aux_group_index in chain_index_to_group_indices[chain_index]:
                    ...
                    if not with_insertions:
                        if former_group_id + 1 == aux_group_id:
                            pair_groups_peptidic_bonds.append(
                                [former_group_index, aux_group_index]
                            )
                    else:
                        if isinstance(former_group_id, int) and isinstance(
                            aux_group_id, int
                        ):
                            if former_group_id + 1 == aux_group_id:
                                pair_groups_peptidic_bonds.append(
                        ...
                        elif isinstance(former_group_id, int) and isinstance(
                            aux_group_id, str
                        ):
                        ...
                        elif isinstance(former_group_id, str) and isinstance(
                            aux_group_id, str
                        ):
                            if (
                                former_group_id[:-1] == aux_group_id[:-1]
                                and ord(aux_group_id[-1]) - ord(former_group_id[-1])
                                == 1
                            ):
                                pair_groups_peptidic_bonds.append(
                                    [former_group_index, aux_group_index]
    ...
    if item.exists("struct_conn"):
        index_att = {
            jj: ii
            for ii, jj in enumerate(item.getObj("struct_conn").getAttributeList())
        }
        ...
        for record in item.getObj("struct_conn").data:
            if record[index_att["conn_type_id"]] == "covale":
                ...
                group_index_1 = [
                    ii for ii in group_indices_1 if group_id[ii] == group_id_1
                ]
                group_index_1 = group_index_1[0]
                atom_indices_1 = group_index_to_atom_indices[group_index_1]
                atom_index_1 = [
                    ii for ii in atom_indices_1 if atom_name[ii] == atom_name_1
                ]
                ...
                group_index_2 = [
                    ii for ii in group_indices_2 if group_id[ii] == group_id_2
                ]
                group_index_2 = group_index_2[0]
                atom_indices_2 = group_index_to_atom_indices[group_index_2]
                atom_index_2 = [
                    ii for ii in atom_indices_2 if atom_name[ii] == atom_name_2
                ]
                atom_index_2 = atom_index_2[0]

                if [atom_index_1, atom_index_2] not in bond_accumulator:
                    atom_pairs_bonded_by_struct_conn.append(
                        sorted([atom_index_1, atom_index_2])
                    )
                bond_accumulator.add(
    ...
    if len(alt_atom_indices):
        ...
        aux_dict = {}
        for aux_atom_index, aux_atom_name, aux_group_id, aux_chain_id in zip(
            alt_atom_indices, alt_atom_names, alt_group_ids, alt_chain_ids
        ):
        ...
        atom_indices_to_be_kept = np.setdiff1d(
            np.arange(n_atoms), atoms_to_be_removed_with_alt_loc
        )
        dict_old_to_new_atom_indices = {
            jj: ii for ii, jj in enumerate(atom_indices_to_be_kept)
        }

        if len(atoms_without_bonds):
            atoms_without_bonds = np.setdiff1d(
                atoms_without_bonds, atoms_to_be_removed_with_alt_loc
            )
            atoms_without_bonds = [
                dict_old_to_new_atom_indices[ii] for ii in atoms_without_bonds
            ]
        ...
        for chosen, same_atoms in zip(chosen_with_alt_loc, aux_dict.values()):
            ...
            aux_coordinates = puw.quantity(
                coordinates[0, same_atoms, :], unit="angstroms", standardized=True
            )
            aux_b_factor = puw.quantity(
                b_factor[0, same_atoms], unit="angstroms**2", standardized=True
            )
            aux_dict = {
        ...
        if len(atom_pairs_bonded_by_struct_conn):
            ...
            for ii, jj in atom_pairs_bonded_by_struct_conn:
                ...
                if (
                    ii in dict_old_to_new_atom_indices
                    and jj in dict_old_to_new_atom_indices
                ):
                    aux_list.append(
                        [
    ...
    if item.exists("pdbx_struct_oper_list") and item.exists("pdbx_struct_assembly_gen"):
        ...
        index_att = {
            jj: ii
            for ii, jj in enumerate(
                item.getObj("pdbx_struct_oper_list").getAttributeList()
            )
        }
        for record in item.getObj("pdbx_struct_oper_list"):
            ...
            operators[str(record[index_att["id"]])] = {
                "matrix": matrix,
                "vector": vector,
            }
        ...
        for record in item.getObj("pdbx_struct_assembly_gen"):
            ...
            aux["chain_indices"] = [
                old_chain_id_to_chain_index[ii] for ii in old_chain_ids
            ]
            ...
            if parenCount == 2:
                ...
                oper.extend(
                    _parse_operation_expression(operation_expression[0 : temp + 1])
                )
                oper2.extend(
                    _parse_operation_expression(operation_expression[temp + 1 :])
                )
            if len(oper2) == 0:
                for ii in oper:
                ...
                for ii in oper:
                    for jj in oper2:
                        aux_trans, aux_rot = _compose_operation(
                            operators[str(ii)]["vector"],
                            operators[str(ii)]["matrix"],
                            operators[str(jj)]["vector"],
                            operators[str(jj)]["matrix"],
                        )
    ...
    if len(atoms_without_bonds):
        missing_bonds = get_missing_bonds(
            tmp_item, selection=atoms_without_bonds, skip_digestion=False
        )
    ...
    if n_models > 1:
        ...
        for model_index in models:
            ...
            item_per_model.append(
                tmp_item.extract(atom_indices=aux_atom_indices, skip_digestion=True)
            )

        comparison = []
        for ii in range(1, n_models):
            aux = item_per_model[0].topology.compare(
                item_per_model[ii].topology,
                atom_id=False,
                atom_name=True,
                atom_type=True,
                group_index=True,
        ...
        if all(comparison):
            for ii in range(1, n_models):
                item_per_model[0].structures.append_structures(
                    item_per_model[ii].structures
                )
