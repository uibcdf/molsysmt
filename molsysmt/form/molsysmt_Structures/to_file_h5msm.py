...
def to_file_h5msm(
    ...
    dump_structures_to_h5msm(
        item, handler, atom_indices=atom_indices, structure_indices=structure_indices
    )
...
def dump_structures_to_h5msm(item, file, atom_indices="all", structure_indices="all"):
    ...
    if not file_is_h5msm:
        raise InternalAlgorithmError(
            ...
            caller="molsysmt.form.molsysmt_Structures.to_file_h5msm",
        )
    ...
    if item.coordinates is not None:
        ...
        if puw.check(item.coordinates, unit=length_unit):
            ...
            aux = puw.get_value(item.coordinates, to_unit=length_unit).astype(
                float_precision
            )
    ...
    if item.velocities is not None:
        ...
        if puw.check(item.velocities, unit=velocity_unit):
            ...
            aux = puw.get_value(item.velocities, to_unit=velocity_unit).astype(
                float_precision
            )
    ...
    if item.b_factor is not None:
        ...
        if puw.check(item.b_factor, unit=b_factor_unit):
            ...
            aux = puw.get_value(item.b_factor, to_unit=b_factor_unit).astype(
                float_precision
            )
    ...
    if item.kinetic_energy is not None:
        ...
        if puw.check(item.kinetic_energy, unit=energy_unit):
            ...
            aux = puw.get_value(item.kinetic_energy, to_unit=energy_unit).astype(
                float_precision
            )
    ...
    if item.potential_energy is not None:
        ...
        if puw.check(item.potential_energy, unit=energy_unit):
            ...
            aux = puw.get_value(item.potential_energy, to_unit=energy_unit).astype(
                float_precision
            )
    ...
    if item.temperature is not None:
        ...
        if puw.check(item.temperature, unit=temperature_unit):
            ...
            aux = puw.get_value(item.temperature, to_unit=temperature_unit).astype(
                float_precision
            )
