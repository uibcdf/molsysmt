...
def merge(items, atom_indices="all", structure_indices="all", skip_digestion=False):
    """
    Merging multiple items into a single item of form molsysmt.Structures.
    ...
    if len(atom_indices) != n_items:
        raise ArgumentError(
            "atom_indices",
            value=atom_indices,
            caller="molsysmt.form.molsysmt_Structures.merge",
        )
    ...
    if is_all(first_structure_indices):
        ...
        output.box = (
            None
            if items[0].box is None
            else items[0].box[first_structure_indices, :, :]
        )
        output.structure_id = (
            None
            if items[0].structure_id is None
            else items[0].structure_id[first_structure_indices]
        )
        output.time = (
            None if items[0].time is None else items[0].time[first_structure_indices]
        )
    ...
    for aux_item, aux_atom_indices, aux_structure_indices in zip(
        items, atom_indices, structure_indices
    ):
        if is_all(aux_structure_indices):
            aux_n_atoms = (
                aux_item.n_atoms if is_all(aux_atom_indices) else len(aux_atom_indices)
            )

            if aux_n_atoms > 0:
                if n_structures != aux_item.n_structures:
                    raise StructuralInconsistencyError(
                        reason=f"Inconsistent number of structures: {n_structures} vs {aux_item.n_structures}",
                        caller="molsysmt.form.molsysmt_Structures.merge",
                    )
                ...
                if is_all(aux_atom_indices):
                    ...
                    list_coordinates.append(
                        aux_item.coordinates[:, aux_atom_indices, :]
                        if aux_item.coordinates is not None
                        else None
                    )
                    list_velocities.append(
                        aux_item.velocities[:, aux_atom_indices, :]
                        if aux_item.velocities is not None
                        else None
                    )
                    list_b_factor.append(
                        aux_item.b_factor[:, aux_atom_indices]
                    ...
                    for ii, alt_loc_dict in enumerate(aux_item.alternate_location):
                        if alt_loc_dict is not None:
                            for key, value in alt_loc_dict.items():
                                output.alternate_location[ii][key + count_n_atoms] = (
                                    value
                                )
        else:
            ...
            if aux_n_structure_indices > 0:
                if n_structures != aux_n_structure_indices:
                    raise StructuralInconsistencyError(
                        reason=f"Inconsistent number of structures: {n_structures} vs {aux_n_structure_indices}",
                        caller="molsysmt.form.molsysmt_Structures.merge",
                    )
                ...
                aux_n_atoms = (
                    aux_item.n_atoms
                    if is_all(aux_atom_indices)
                    else len(aux_atom_indices)
                )

                if aux_n_atoms > 0:
                    if is_all(aux_atom_indices):
                        list_coordinates.append(
                            aux_item.coordinates[aux_structure_indices, :, :]
                            if aux_item.coordinates is not None
                            else None
                        )
                        list_velocities.append(
                            aux_item.velocities[aux_structure_indices, :, :]
                            if aux_item.velocities is not None
                            else None
                        )
                        list_b_factor.append(
                            aux_item.b_factor[aux_structure_indices, :]
                            if aux_item.b_factor is not None
                            else None
                        )
                    else:
                        ...
                        list_coordinates.append(
                            tmp[:, aux_atom_indices, :] if tmp is not None else None
                        )
                        tmp = (
                            aux_item.velocities[aux_structure_indices, :, :]
                            if aux_item.velocities is not None
