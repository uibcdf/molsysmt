            ...
            if isinstance(indices, (list, tuple, np.ndarray)):
                for ii in indices:
                    ...
                    with silence_backend_stdout():
                        coordinates_aux, box_lengths_aux, box_angles_aux = (
                            self.molecular_system.read(1, 0, self._mdtraj_atom_indices)
                        )
            ...
            for argument in self.arguments:
                if argument == "coordinates":
                    self._output_dictionary["coordinates"] = puw.quantity(
                        np.array(coordinates), "angstroms", standardized=True
                    )
                    del coordinates
                    ...
                    if box_is_available:
                        box_lengths = puw.quantity(
                            np.array(box_lengths), "angstroms", standardized=True
                        )
                        box_angles = puw.quantity(
                            np.array(box_angles), "degrees", standardized=True
                        )
                        self._output_dictionary["box"] = (
                            get_box_from_lengths_and_angles(
                                box_lengths, box_angles, skip_digestion=True
                            )
                        )
