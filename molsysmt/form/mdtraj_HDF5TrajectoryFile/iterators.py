...
class StructuresIterator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._node = {}
        self._node["cell_angles"] = self.molecular_system._get_node(
            where="/", name="cell_angles"
        )
        self._node["cell_lengths"] = self.molecular_system._get_node(
            where="/", name="cell_lengths"
        )
        self._node["time"] = self.molecular_system._get_node(where="/", name="time")
        self._node["coordinates"] = self.molecular_system._get_node(
            where="/", name="coordinates"
        )
        self._node["temperature"] = self.molecular_system._get_node(
            where="/", name="temperature"
        )
        self._node["kinetic_energy"] = self.molecular_system._get_node(
            where="/", name="kineticEnergy"
        )
        self._node["potential_energy"] = self.molecular_system._get_node(
            where="/", name="potentialEnergy"
        )
            ...
            for argument in self.arguments:
                if argument == "coordinates":
                    ...
                    self._output_dictionary["coordinates"] = puw.quantity(
                        coordinates, "nm", standardized=True
                    )
                    del coordinates
                elif argument == "time":
                    self._output_dictionary["time"] = puw.quantity(
                        self._node["time"][indices], "ps", standardized=True
                    )
                elif argument == "structure_id":
                    self._output_dictionary["structure_id"] = indices
                elif argument == "box":
                    box_lengths = puw.quantity(
                        self._node["cell_lengths"][indices, :].astype(np.float64),
                        "nm",
                        standardized=True,
                    )
                    box_angles = puw.quantity(
                        self._node["cell_angles"][indices, :].astype(np.float64),
                        "degrees",
                        standardized=True,
                    )
                    ...
                    self._output_dictionary["box"] = get_box_from_lengths_and_angles(
                        box_lengths, box_angles
                    )
                    del (box_lengths, box_angles)
                elif argument == "temperature":
                    self._output_dictionary["temperature"] = puw.quantity(
                        self._node["temperature"][indices], "K", standardized=True
                    )
                elif argument == "potential_energy":
                    self._output_dictionary["potential_energy"] = puw.quantity(
                        self._node["potential_energy"][indices],
                        "kJ/mol",
                        standardized=True,
                    )
                elif argument == "kinetic_energy":
                    self._output_dictionary["kinetic_energy"] = puw.quantity(
                        self._node["kinetic_energy"][indices],
                        "kJ/mol",
                        standardized=True,
                    )
