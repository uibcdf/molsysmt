...
def extract(
    ...
    """
    Extracting a subset of elements or structures from form mdtraj.HDF5TrajectoryFile.
    ...
    try:
        ...
        if is_all(atom_indices) and is_all(structure_indices):
            if output_filename is not None:
                ...
                iterator = (
                    tqdm(range(n_structures)) if progress_bar else range(n_structures)
                )
            ...
            if output_filename is None:
                ...
                iterator = (
                    tqdm(structure_indices) if progress_bar else structure_indices
                )
