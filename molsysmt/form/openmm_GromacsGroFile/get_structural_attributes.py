...
def _get_positions_array(item, indices, structure_indices):
    ...
    frame_list = (
        list(range(n_frames)) if is_all(structure_indices) else list(structure_indices)
    )
...
def get_coordinates_from_atom(
    ...
    .. versionadded:: 1.0.0
    """
    output = _get_positions_array(item, indices, structure_indices) * puw.unit(
        "nanometer"
    )
    return output
...
def get_coordinates_from_system(item, structure_indices="all", skip_digestion=False):
    ...
    .. versionadded:: 1.0.0
    """
    output = _get_positions_array(item, "all", structure_indices) * puw.unit(
        "nanometer"
    )
    return output
