...
@arg_digest()
def get_dihedral_quartets(
    molecular_system, with_blocks=False, selection="all", syntax="MolSysMT", **kwargs
):
    """
    ...
    for dihedral_angle in dihedral_angles:
        ...
        quartets = get_covalent_paths(
            molecular_system, path=path, selection=selection, syntax=syntax
        )
        ...
        if with_blocks:
            ...
            for quartet_index in range(n_quartets):
                ...
                component_atom_indices = get(
                    molecular_system,
                    element="component",
                    selection=component_index,
                    atom_index=True,
                )[0]
