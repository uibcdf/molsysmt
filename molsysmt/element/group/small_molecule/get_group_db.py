...
def get_group_db(group_name):
    """
    Getting the internal reference database dictionary for small molecule groups.
    ...
    if group_name not in group_names:
        raise InternalAlgorithmError(
            ...
            caller="molsysmt.element.group.small_molecule.get_group_db",
        )
    with gzip.open(
        path("molsysmt.data.databases.small_molecules", group_name[0] + ".pkl.gz"), "rb"
    ) as fff:
        dbs = pickle.load(fff)
