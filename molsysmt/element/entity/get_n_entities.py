...
@arg_digest()
def get_n_entities(
    molecular_system, selection="all", redefine_entities=False, syntax="MolSysMT"
):
    """
    ...
    if isinstance(selection, str) and selection == "all":
        ...
        if isinstance(molecular_system, Topology):
            return (
                len(molecular_system.entities.index)
                if not redefine_entities
                else len(
                    get_entity_index(
                        molecular_system,
                        element="entity",
        ...
        if isinstance(molecular_system, MolSys):
            return (
                len(molecular_system.topology.entities.index)
                if not redefine_entities
                else len(
                    get_entity_index(
                        molecular_system,
