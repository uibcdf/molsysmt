...
def get_chain_type(
    ...
    if redefine_types:
        molecule_types_from_chain = get_molecule_type(
            molecular_system,
            element="chain",
            selection="all",
        ...
        n_molecules = get_n_molecules(
            molecular_system, redefine_molecules=redefine_molecule_indices
        )

        chain_types_from_chain = []

        if (
            len(molecule_types_from_chain) == 1
            and len(molecule_types_from_chain[0]) == n_molecules
        ):
        ...
        if element == "atom":
            ...
            for chains_in_entity in aux:
                output.append(
                    np.array(chain_types_from_chain, dtype=object)[
                        chains_in_entity
                    ].tolist()
                )
        else:
