...
def get_n_components(
    molecular_system, selection="all", redefine_components=False, syntax="MolSysMT"
):
    """
    ...
    if isinstance(selection, str) and selection == "all":
        ...
        if isinstance(molecular_system, Topology):
            return (
                len(molecular_system.components.index)
                if not redefine_components
                else len(
                    get_component_index(
                        molecular_system,
                        element="component",
        ...
        if isinstance(molecular_system, MolSys):
            return (
                len(molecular_system.topology.components.index)
                if not redefine_components
                else len(
                    get_component_index(
                        molecular_system,
