...
aux_dict["C"]["C"] = puw.quantity(
    1.54, "angstroms"
)  # 1.54 single bond, 1.34 double bond, 1.20 triple bond
aux_dict["C"]["N"] = puw.quantity(1.47, "angstroms")
aux_dict["C"]["O"] = puw.quantity(
    1.43, "angstroms"
)  # 1.43 single bond, 1.23 double bond
aux_dict["N"]["N"] = puw.quantity(1.47, "angstroms")
aux_dict["N"]["O"] = puw.quantity(
    1.45, "angstroms"
)  # 1.45 single bond, 1.25 double bond
aux_dict["O"]["H"] = puw.quantity(0.96, "angstroms")
aux_dict["H"]["H"] = None
for ii, kk in aux_dict.items():
    for jj in kk:
...
max_expected_bond_length["terminal capping"] = max_expected_bond_length[
    "amino acid"
].copy()
