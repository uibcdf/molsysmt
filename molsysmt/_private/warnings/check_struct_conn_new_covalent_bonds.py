...
def check_struct_conn_new_covalent_bonds(molecular_system, atom_pairs):
    """
    covalent_bonds: iterable of tuples or dicts con info de los enlaces.
                    Cada entrada debe contener al menos las cadenas
                    implicadas (chain1, chain2).
    """
    ...
    if len(label_pairs_reported) > 0:

        msg = (f"[MolSysMT Warning] {len(label_pairs_reported)} covalent bond(s) reported by the 'struct_conn' table "
        ...
        for label1, label2 in label_pairs_reported:
            msg += f"  - {label1}  <-->  {label2}\n"
