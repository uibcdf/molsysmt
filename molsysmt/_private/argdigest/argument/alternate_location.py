...
def digest_alternate_location(alternate_location, caller=None):
        ...
        for aux_alternate_location in alternate_location:
            for aux_atom_index, aux_dict in aux_alternate_location.items():
                if not isinstance(aux_atom_index, (int, np.int64)):
                    ...
                    break
                try:
                    aux_dict["location_id"] = digest_location_id(
                        aux_dict["location_id"]
                    )
                    ...
                    aux_dict["coordinates"] = digest_coordinates(
                        aux_dict["coordinates"]
                    )[0]
    ...
    raise ArgumentError(
        "alternate_location", value=alternate_location, caller=caller, message=None
    )
