...
from molsysmt._private.argdigest._scientific_arrays import normalize_lengths_to_nm_array
...
def digest_box_lengths(box_lengths, caller=None):
    ...
    if caller == "molsysmt.basic.get.get":
        if isinstance(box_lengths, bool):
            ...
            raise ArgumentError(
                "box_lengths", value=box_lengths, caller=caller, message=None
            )

    else:
        if caller in _RAW_NM_ARRAY_CALLERS:
            return normalize_lengths_to_nm_array(
                box_lengths, "box_lengths", caller=caller
            )
