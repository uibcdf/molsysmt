...
def color_is_rgb(color):
    ...
    if isinstance(color, (list, tuple, np.ndarray)):
        if len(color) == 3 and all(
            isinstance(ii, (int, np.int64, float)) for ii in color
        ):
            output = True
...
def get_list_of_colors_from_values(
    values,
    min_value=None,
    mid_value=None,
    max_value=None,
        ...
        if mid_value is not None:
            if not symmetrical:
                ...
                norm = mcolors.TwoSlopeNorm(
                    vmin=min_value, vcenter=mid_value, vmax=max_value
                )
                ...
                if (max_value is None) and (min_value is None):
                    ...
                    max_value = max(values)
                    halfrange = max(
                        abs(max_value - mid_value), abs(min_value - mid_value)
                    )
