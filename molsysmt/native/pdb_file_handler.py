...
def parse_format33(file):
    ...
    from .pdb_atomic_coordinate_entry import (
        AtomRecord,
        AuthorRecord,
        CaveatRecord,
        CompndRecord,
    ...
    while counter < n_lines:
        ...
        ### COMPND
            ...
            while record == "COMPND":
                ...
                if aux.startswith("MOL_ID:"):  ### mol_id
                    ...
                    compnd.chain = [
                        ii.strip() for ii in aux[6:].rstrip(";").strip().split(",")
                    ]
                    ...
                    compnd.synonym = [
                        ii.strip() for ii in aux[8:].rstrip(";").strip().split(",")
                    ]
        ...
        ### JRNL
            ...
            while record == "JRNL  ":
                ...
                if aux == "AUTH":  ### AUTH
                    if line[16:18] == "  ":
                        jrnl.authorList = [
                            ii.strip() for ii in line[19:79].strip().split(",")
                        ]
                    else:
                        jrnl.authorList += [
                            ii.strip() for ii in line[19:79].strip().split(",")
        ...
        ### HETSYN
            ...
            while record == "HETSYN":
                ...
                hetsyn.hetSynonyms += [
                    ii.strip() for ii in line[15:70].split(";") if ii.strip()
                ]
            ...
            while record == "CONECT":
                if previous_serial_atom_number != int(line[6:11]):
                    ...
                    while line[position : position + 5].strip():
                        conect.bondedAtomsSerNum.append(
                            int(line[position : position + 5])
                        )
                        position += 5
                        if position >= 31:
                            break
