    ...
    * **Named backbone/side-chain dihedrals** — omit ``dihedral_quartets`` and pass
    ...
    if dihedral_quartets is None:
        ...
        for key in kwargs.keys():
            if kwargs[key]:
                aux_dihedral_quartets = get_dihedral_quartets(
                    molecular_system, selection=selection, syntax=syntax, **{key: True}
                )
    ...
    if pbc:
        ...
        if box is not None:
            if box[0] is not None:
                ...
                angles = _kernels.get_mic_dihedral_angles(
                    coordinates, box, dihedral_quartets
                )
