...
def least_rmsd_fit(molecular_system=None, selection='all', selection_fit='atom_type!="H"', structure_indices='all',
        ...
        to_form=None, in_place=False, syntax='MolSysMT', engine='MolSysMT', parallel=None, num_threads=None,
        use_gpu=None, gpu_backend=None, precision=None, skip_digestion=False):
    ...
    if engine=='MolSysMT':
        ...
        from molsysmt.lib.structure._kernel_inputs import align_coordinates_values_and_unit, extract_coordinates_value_and_unit
        ...
        if _use_gpu:
            ...
            selection_coordinates = get(molecular_system, element='atom', selection=selection,
                                        structure_indices=structure_indices, syntax=syntax, coordinates=True)
            
            # Align coordinates to move to the fit length unit
            coords_dtype = np.float32 if config.precision == 'single' else np.float64
            coords_to_move = np.asarray(puw.get_value(selection_coordinates, to_unit=length_unit), dtype=coords_dtype)
            ...
            if fitted_coords is None:
                from molsysmt.lib.structure.get_least_rmsd_cuda import least_rmsd_fit as _kernel
                fitted_coords = _kernel(coords_to_move, fit_coords, ref_coords)

            # Wrap coordinates back to quantity
            fitted_coords = puw.quantity(fitted_coords, length_unit)
            del(coordinates, reference_coordinates, fit_coords, ref_coords, coords_to_move, selection_coordinates)

            if in_place:
                set(molecular_system, selection=selection, structure_indices=structure_indices,
                    syntax=syntax, coordinates=fitted_coords, skip_digestion=True)
                del(fitted_coords)
                gc.collect()
                return None
            else:
                ...
                if config.precision == 'single':
                    if puw.is_quantity(tmp_molecular_system):
                        tmp_molecular_system = puw.quantity(puw.get_value(tmp_molecular_system).astype(np.float32), puw.get_unit(tmp_molecular_system))
        ...
        if fitted_coords is None:
            ...
            if in_place:
                ...
                if config.precision == 'single':
                    coords = get(molecular_system, element='atom', selection=selection, structure_indices=structure_indices,
                                 syntax=syntax, coordinates=True, skip_digestion=True)
                    coords_val, coords_unit = puw.get_value_and_unit(coords)
                    set(molecular_system, selection=selection, structure_indices=structure_indices,
                        syntax=syntax, coordinates=puw.quantity(coords_val.astype(np.float32), coords_unit), skip_digestion=True)
                ...
                if config.precision == 'single':
                    if puw.is_quantity(tmp_molecular_system):
                        ...
                        coords = get(tmp_molecular_system, element='atom', selection=selection, structure_indices=structure_indices,
                                     syntax=syntax, coordinates=True, skip_digestion=True)
                        coords_val, coords_unit = puw.get_value_and_unit(coords)
                        set(tmp_molecular_system, selection=selection, structure_indices=structure_indices,
                            syntax=syntax, coordinates=puw.quantity(coords_val.astype(np.float32), coords_unit), skip_digestion=True)
