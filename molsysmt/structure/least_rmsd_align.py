...
def least_rmsd_align(
    ...
    aux_atoms_list = select(
        molecular_system, element="atom", selection="group_index==@identical_groups"
    )
    selection_to_be_fitted = select(
        molecular_system, element="atom", selection=selection, mask=aux_atoms_list
    )
    components_selected = select(
        molecular_system, element="component", selection=selection
    )
    atoms_in_components_selected = select(
        molecular_system,
        element="atom",
        selection="component_index==@components_selected",
    )
    ...
    if engine_least_rmsd_fit == "MolSysMT":
        ...
        if in_place:
            least_rmsd_fit(
                ...
                selection=atoms_in_components_selected,
            ...
            del (
                atoms_in_components_selected,
                selection_to_be_fitted,
                reference_selection_to_be_fitted,
            )
            del (structure_indices, reference_structure_index)

        else:
            output = least_rmsd_fit(
                molecular_system=molecular_system,
                selection=atoms_in_components_selected,
                selection_fit=selection_to_be_fitted,
                structure_indices=structure_indices,
