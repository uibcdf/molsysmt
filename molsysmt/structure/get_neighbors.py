...
def _threshold_neighbors_via_cell_list(molecular_system, selection, selection_2,
    ...
    kernel over the flattened (structure, atom) space. Returns either object arrays
    ``(neighs, dists)`` (``output_type='numpy.ndarray'``) assembled by cheap
    slicing, or the flat CSR ``(offsets, indices, distances)`` (``'csr'``). Returns
...
def get_neighbors(molecular_system, selection="all", structure_indices="all", center_of_atoms=False, weights=None,
    ...
    * **Fixed-count mode** (``n_neighbors`` is set, ``threshold=None``): returns the
    ...
    output_type : {'numpy.ndarray', 'pairs', 'csr'}, default 'numpy.ndarray'
        ...
        * ``'csr'``: return the flat CSR ``(offsets, indices, distances)`` over all
          structures; the neighbours of query atom ``ii`` in structure ``s`` are the
          slice ``[offsets[s*n_query+ii] : offsets[s*n_query+ii+1]]``. Only available
          on the cell-list fast path (threshold mode, native engine, plain atom
          selections); otherwise ``NotImplementedMethodError`` is raised.
