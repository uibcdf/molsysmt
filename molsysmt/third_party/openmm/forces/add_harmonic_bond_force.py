...
@arg_digest()
def add_harmonic_bond_force(
    molecular_system=None,
    atom_pair=None,
    force_constant="1000 kilojoules_per_mole/nm**2",
    bond_length=None,
    pbc=False,
    return_force=False,
    syntax="MolSysMT",
    ...
    if bond_length is None:
        bond_length = get_distances(
            molecular_system, element="atom", selection=atom_pairs, pairs=True, pbc=pbc
        )[0]
    ...
    for ii, atom_pair in enumerate(atom_pairs):
        ...
        force.addBond(
            int(atom_pair[0]), int(atom_pair[1]), aux_bond_length, aux_force_constant
        )
