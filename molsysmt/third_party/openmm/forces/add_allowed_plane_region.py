...
def add_allowed_plane_region(
    ...
    if np.allclose(normal_vector, [1.0, 0.0, 0.0]):
        if pbc:
            potential = (
                "0.5*Ka*q^2; q = max(0, d-wa); d = periodicdistance(x, 0, 0, px, 0, 0)"
            )
        else:
        ...
        if pbc:
            potential = (
                "0.5*Ka*q^2; q = max(0, d-wa); d = periodicdistance(0, y, 0, 0, py, 0)"
            )
        else:
        ...
        if pbc:
            ...
            potential = (
                "0.5*Ka*q^2; q = max(0, d-wa); d = abs((x-px)*vx+(y-py)*vy+(z-pz)*vz);"
            )
