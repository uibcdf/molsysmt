...
@arg_digest()
def add_forbidden_plane_region(
    ...
    from molsysmt import get_form, select
    ...
    if np.allclose(normal_vector, [1.0, 0.0, 0.0]):
        ...
        if pbc:
            potential = (
                "0.5*Kf*q^2; q = min(0, d-wf); d = periodicdistance(0, y, 0, 0, py, 0)"
            )
        else:
        ...
        if pbc:
            potential = (
                "0.5*Kf*q^2; q = min(0, d-wf); d = periodicdistance(0, 0, z, 0, 0, pz)"
            )
        else:
