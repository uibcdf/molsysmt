...
@arg_digest()
def add_point_harmonic_restraint(
    molecular_system,
    selection="all",
    ...
    if point is None:
        if form_in == "openmm.Context":
            raise ArgumentError(
                argument="point",
                caller="molsysmt.third_party.openmm.forces.add_point_harmonic_restraint",
    ...
    force_constant = puw.convert(
        force_constant,
        to_unit=u.kilojoule_per_mole / (u.nanometer**2),
        to_form="openmm.unit",
    )
