...
class H5MSMReporter(object):
    ...
    def __init__(
        ...
        self._file_handler = H5MSMFileHandler(
            file,
            io_mode="w",
            creator="OpenMM",
        ...
        if isinstance(topology, mm.app.Topology):
            if not is_all(self._selection):
                self._selection = select(
                    topology, selection=self._selection, syntax=self._syntax
                )
                self._selection_is_all = False
    ...
    def _initialize(self, simulation):
        ...
        if self._constant_report_interval:
            ...
            if self._constant_step_size:
                ...
                self._structures_sd.attrs["time_step"] = (
                    self._step_size.in_units_of(unit.picoseconds)._value
        ...
        if self._steps is not None:
            ...
            if self._coordinates:
                self._structures_sd["coordinates"].resize(
                    (self._n_structures, self._n_atoms, 3)
                )

            if self._velocities:
                self._structures_sd["velocities"].resize(
                    (self._n_structures, self._n_atoms, 3)
                )
    ...
    def describeNextReport(self, simulation):
        ...
        return (
            steps,
            self._needs_positions,
            self._needs_velocities,
            self._needs_forces,
            self._needs_energy,
        )
    ...
    def report(self, simulation, state):
        ...
        if self._box:
            self._structures_sd["box"][index, :, :] = state.getPeriodicBoxVectors(
                asNumpy=True
            )._value
            if self._constant_box:
                self._box = False
        ...
        if self._coordinates:
            if self._selection_is_all:
                self._structures_sd["coordinates"][index, :, :] = state.getPositions(
                    asNumpy=True
                )._value
            else:
                self._structures_sd["coordinates"][index, :, :] = state.getPositions(
                    asNumpy=True
                )[self._selection, :]._value

        if self._velocities:
            if self._selection_is_all:
                self._structures_sd["velocities"][index, :, :] = state.getVelocities(
                    asNumpy=True
                )._value
            else:
                self._structures_sd["velocities"][index, :, :] = state.getVelocities(
        ...
        if self._potential_energy:
            self._structures_sd["potential_energy"][index] = (
                state.getPotentialEnergy()._value
            )

        if self._kinetic_energy:
            self._structures_sd["kinetic_energy"][index] = (
