...
def standardize_view(view, atom_indices="all", structure_indices="all"):
    ...
    sel_cartoon = select(
        tmp_topology,
        selection='molecule_type in ["protein", "dna", "rna"]' + mask_string,
        to_syntax="NGLView",
    )
    sel_balls = select(
        tmp_topology,
        selection='molecule_type in ["ion"]' + mask_string,
        to_syntax="NGLView",
    )
    sel_licorice = select(
        tmp_topology,
        selection='molecule_type in ["lipid", "small molecule", "saccharide"]'
        + mask_string,
        to_syntax="NGLView",
    )

    peptide_indices = select(
        tmp_topology, selection='molecule_type=="peptide"', element="molecule"
    )
    peptides_to_cartoon = []
    peptides_to_licorice = []
    for peptide_index in peptide_indices:
        n_groups = get(
            tmp_topology, element="molecule", selection=peptide_index, n_groups=True
        )[0]
    ...
    sel_peptides_cartoon = select(
        tmp_topology,
        selection="molecule_index in @peptides_to_cartoon" + mask_string,
        to_syntax="NGLView",
    )
    sel_peptides_licorice = select(
        tmp_topology,
        selection="molecule_index in @peptides_to_licorice" + mask_string,
        to_syntax="NGLView",
    )
