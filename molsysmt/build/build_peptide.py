...
def _pick_topology_variant(topologies, prefer_oxt=None):
    ...
    if not topologies:
        raise InternalAlgorithmError(
            "No topology variants found in group database",
...
def _place_group_coordinates(group_anchor_x, atom_names):
    ...
    for atom_index, atom_name in enumerate(atom_names):
        if atom_name in backbone_map:
            ...
            angle = (
                atom_index * 2.399963229728653
            )  # irrational multiple to reduce overlaps
...
def _to_three_letter_peptide_sequence(molecular_system):
    ...
    if type(molecular_system) is str:
        sequence = molecular_system.removeprefix("amino_acids_1:").removeprefix(
            "amino_acids_3:"
        )
...
def _get_peptide_builder_templates():
    ...
    try:
        path = files("molsysmt.data.databases.peptide_builder").joinpath(
            "amber14sb.json.gz"
        )
...
def _minimum_nonbonded_heavy_distance(
    ...
    for existing_index in existing_heavy:
        for candidate_local_index in candidate_heavy:
            ...
            if (
                tuple(sorted((int(existing_index), int(candidate_global_index))))
                in existing_bonds_set
            ):
                continue
            distance = np.linalg.norm(
                existing_coordinates[existing_index, :]
                - candidate_coordinates[candidate_local_index, :]
            )
...
def _minimum_nonbonded_heavy_distance_between_groups(
    left_coordinates,
    left_atom_names,
    left_atom_types,
    right_coordinates,
    ...
    for left_index, left_atom_type in enumerate(left_atom_types):
        ...
        for right_index, right_atom_type in enumerate(right_atom_types):
            ...
            if skip_pair_names is not None:
                if (
                    left_atom_names[left_index],
                    right_atom_names[right_index],
                ) == skip_pair_names:
                    continue
            distance = np.linalg.norm(
                left_coordinates[left_index, :] - right_coordinates[right_index, :]
            )
            if distance < min_distance:
                min_distance = distance
    return min_distance
...
def _minimum_nonbonded_heavy_distance_full(
    coordinates, atom_types=None, bonds=None, heavy_mask=None, bonded_matrix=None
):
    ...
    if heavy_mask is None or bonded_matrix is None:
        ...
        heavy_mask, bonded_matrix = _prepare_nonbonded_heavy_distance_full(
            atom_types, bonds
        )
...
def _optimize_peptide_torsions(
    coordinates,
    atom_types,
    bonds,
    groups_data,
    ...
    for _ in range(n_rounds):
        ...
        for group_index in range(len(group_start_indices) - 1):
            ...
            axis = _normalize(
                optimized_coordinates[right_head_index, :]
                - optimized_coordinates[left_tail_index, :]
            )
            if axis is None:
                continue
...
def _get_connected_component(adjacency, start_index, blocked_bond):
    ...
    while stack:
        ...
        for neighbor in adjacency.get(current, []):
            if (current == i_block and neighbor == j_block) or (
                current == j_block and neighbor == i_block
            ):
                continue
...
def _optimize_sidechain_torsions(
    coordinates,
    atom_types,
    atom_names,
    bonds,
    ...
    for _ in range(n_rounds):
        ...
        for group_index, group_data in enumerate(groups_data):
            ...
            for atom1_index, atom2_index in bonds:
                ...
                component_1 = _get_connected_component(
                    adjacency, atom1_index, (atom1_index, atom2_index)
                )
                if atom2_index in component_1:
                    # Ring bond or alternate path exists; avoid rotating constrained cycles.
                    continue
                component_2 = _get_connected_component(
                    adjacency, atom2_index, (atom1_index, atom2_index)
                )
                ...
                backbone_count_1 = sum(
                    atom_names[ii] in backbone_names for ii in component_1
                )
                backbone_count_2 = sum(
                    atom_names[ii] in backbone_names for ii in component_2
                )
                ...
                if len(rotating_indices) <= 1 or len(rotating_indices) >= len(
                    group_indices
                ):
                    continue
                ...
                axis = _normalize(
                    optimized_coordinates[atom2_index, :]
                    - optimized_coordinates[atom1_index, :]
                )
                if axis is None:
                    continue
...
def _align_capping_junction_geometry(
    coordinates, atom_types, groups_data, group_start_indices
):
    ...
    for group_index in range(len(group_start_indices) - 1):
        ...
        for local_i, local_j in previous_group_data["bonded_atom_indices"]:
            ...
            if (
                neighbor_local_index is None
                or neighbor_local_index in oxygen_local_candidates
            ):
                continue
        ...
        direction_from_oxygen = _normalize(
            oxygen_coordinates - carbonyl_carbon_coordinates
        )
        direction_from_substituent = _normalize(
            substituent_coordinates - carbonyl_carbon_coordinates
        )
        current_direction = _normalize(
            next_nitrogen_coordinates - carbonyl_carbon_coordinates
        )
        if (
            direction_from_oxygen is None
            or direction_from_substituent is None
        ...
        dot_value = float(
            np.clip(np.dot(current_direction, desired_direction), -1.0, 1.0)
        )
        ...
        rotation_matrix = _rotation_matrix_from_axis_angle(
...
def _optimize_backbone_torsions(
    coordinates,
    atom_types,
    atom_names,
    bonds,
...
def build_peptide(molecular_system, to_form="molsysmt.MolSys", engine="LEaP"):
    ...
    if engine == "LEaP":
        ...
        from molsysmt.element.group.amino_acid import (
            get_group_db as get_amino_acid_group_db,
        )
        from molsysmt.element.group.amino_acid import (
            group_names as amino_acid_group_names,
        )
        from molsysmt.element.group.terminal_capping import (
            get_group_db as get_terminal_capping_group_db,
        )
        from molsysmt.element.group.terminal_capping import (
            group_names as terminal_capping_group_names,
        )
        ...
        if sequence == "":
            raise ArgumentError(
                "sequence", value="empty", caller="molsysmt.build.build_peptide"
            )
        ...
        for group_name in group_names:
            if (
                group_name not in amino_acid_group_names
                and group_name not in terminal_capping_group_names
            ):
                raise ArgumentChoiceError(
                    argument="group_names",
                    value=group_name,
                    choices=sorted(
                        amino_acid_group_names | terminal_capping_group_names
                    ),
                    caller="molsysmt.build.build_peptide",
                )
        ...
        amino_group_indices = [
            ...
            if group_name in amino_acid_group_names
        ]
        last_amino_group_index = (
            amino_group_indices[-1] if amino_group_indices else None
        )
        ...
        for group_index, group_name in enumerate(group_names):
            ...
            resolved_template_name = _resolve_template_alias(
                template_name, peptide_builder_templates
            )
            if resolved_template_name is not None:
                ...
                atom_types = []
                for atom_name, atomic_number in zip(
                    atom_names, template_data["atom_elements"]
                ):
                ...
                groups_data.append(
                    {
                        ...
                        "atom_ff_types": list(
                            template_data.get("atom_types", atom_types)
                        ),
                        "bonded_atom_indices": [
                            tuple(pair) for pair in template_data["bonds"]
                        ],
                        "coordinates_nm": coordinates_nm,
                        "connect": template_data.get("connect", [None, None]),
                        "atom_name_to_local_index": {
                            name: idx for idx, name in enumerate(atom_names)
                        },
                    }
                )
            else:
                if group_name in terminal_capping_group_names:
                    ...
                    if template_name == group_name:
                        if group_index == last_amino_group_index:
                            topology = _pick_topology_variant(
                                group_db["topology"], prefer_oxt=True
                            )
                        else:
                            topology = _pick_topology_variant(
                                group_db["topology"], prefer_oxt=False
                            )
                    else:
                        topology = _pick_topology_variant(group_db["topology"])
                ...
                local_atom_indices = {
                    atom_name: ii for ii, atom_name in enumerate(atom_names)
                }
                bonded_atom_indices = []
                for atom_name_1, atom_name_2 in list(topology["bonds"]):
                    if (
                        atom_name_1 in local_atom_indices
                        and atom_name_2 in local_atom_indices
                    ):
        ...
        for group_index, group_data in enumerate(groups_data):
            ...
            group_data["bonded_atom_indices"]
            ...
            if group_index == 0:
                ...
                if previous_tail_coordinates is not None and head_index is not None:
                    ...
                    previous_name_to_index = previous_group_data[
                        "atom_name_to_local_index"
                    ]
                    previous_tail_local_index = previous_group_data["connect"][1]
                    ...
                    if ca_local_index is not None and oxygen_local_index is not None:
                        ca_coordinates = coordinates[
                            previous_group_start + ca_local_index, :
                        ]
                        oxygen_coordinates = coordinates[
                            previous_group_start + oxygen_local_index, :
                        ]
                        direction_from_ca = _normalize(
                            ca_coordinates - previous_tail_coordinates
                        )
                        direction_from_oxygen = _normalize(
                            oxygen_coordinates - previous_tail_coordinates
                        )
                        if (
                            direction_from_ca is not None
                            and direction_from_oxygen is not None
                        ):
                        ...
                        oxygen_local_index is not None
                        and previous_tail_local_index is not None
                    ):
                        oxygen_coordinates = coordinates[
                            previous_group_start + oxygen_local_index, :
                        ]
                        direction_from_oxygen = _normalize(
                            previous_tail_coordinates - oxygen_coordinates
                        )
                        ...
                        substituent_local_index = None
                        oxygen_local_candidates = {oxygen_local_index}
                        for local_i, local_j in previous_group_data[
                            "bonded_atom_indices"
                        ]:
                            neighbor_local_index = None
                            if local_i == previous_tail_local_index:
                            ...
                            if (
                                ...
                                continue
                            atom_name_neighbor = previous_group_data["atom_names"][
                                neighbor_local_index
                            ]
                            atom_type_neighbor = previous_group_data["atom_types"][
                                neighbor_local_index
                            ]
                            if (
                                atom_name_neighbor.startswith("H")
                                or atom_type_neighbor == "H"
                            ):
                                continue
                            substituent_local_index = neighbor_local_index
                            break
                        ...
                        if substituent_local_index is not None:
                            substituent_coordinates = coordinates[
                                previous_group_start + substituent_local_index, :
                            ]
                            direction_from_substituent = _normalize(
                                substituent_coordinates - previous_tail_coordinates
                            )
                            if (
                                direction_from_substituent is not None
                                and direction_from_oxygen is not None
                            ):
                                trigonal_sum = (
                                    direction_from_substituent + direction_from_oxygen
                                )
                                if np.linalg.norm(trigonal_sum) > 1.0e-3:
                                    trigonal_direction = _normalize(-trigonal_sum)
                                    if trigonal_direction is not None:
                        ...
                        if candidate_directions:
                            ...
                            for candidate_direction in candidate_directions:
                                target_coordinates = (
                                    ...
                                    + peptide_bond_length * candidate_direction
                                )
                                translation = (
                                    target_coordinates - local_coordinates[head_index]
                                )
                                candidate_coordinates = local_coordinates + translation
                                candidate_n_coordinates = candidate_coordinates[
                                    head_index, :
                                ]
                                ...
                                if (
                                    np.linalg.norm(
                                        candidate_n_coordinates - oxygen_coordinates
                                    )
                                    < 0.12
                                ):
                                    continue
                                ...
                                local_junction_score = np.linalg.norm(
                                    candidate_n_coordinates - oxygen_coordinates
                                )
                                if substituent_coordinates is not None:
                                ...
                                if (
                                    local_junction_score
                                    > best_local_junction_score + 1.0e-10
                                ):
                                    ...
                                    abs(
                                        local_junction_score - best_local_junction_score
                                    )
                                    <= 1.0e-10
                                ):
                                    ...
                                    local_junction_score
                                    >= best_local_junction_score - 0.01
                                ):
                    ...
                    target_coordinates = (
                        previous_tail_coordinates + peptide_bond_length * bond_direction
                    )
                    ...
                    if current_group_is_proline:
                        previous_group_start = group_start_indices[group_index - 1]
                        previous_group_size = len(
                            groups_data[group_index - 1]["atom_names"]
                        )
                        ...
                        previous_group_names = groups_data[group_index - 1][
                            "atom_names"
                        ]
                        previous_group_types = groups_data[group_index - 1][
                            "atom_types"
                        ]

                        previous_name_to_index = groups_data[group_index - 1][
                            "atom_name_to_local_index"
                        ]
                        oxygen_local_index = previous_name_to_index.get("O", None)
                        if oxygen_local_index is None:
                        ...
                        for local_i, local_j in groups_data[group_index - 1][
                            "bonded_atom_indices"
                        ]:
                            neighbor_local_index = None
                            ...
                            if (
                                neighbor_local_index is None
                                or neighbor_local_index == oxygen_local_index
                            ):
                                continue
                            atom_name_neighbor = groups_data[group_index - 1][
                                "atom_names"
                            ][neighbor_local_index]
                            atom_type_neighbor = groups_data[group_index - 1][
                                "atom_types"
                            ][neighbor_local_index]
                            if (
                                atom_name_neighbor.startswith("H")
                        ...
                        if (
                            oxygen_coordinates is not None
                            and capping_substituent_coordinates is not None
                        ):
                            direction_from_oxygen = _normalize(
                                oxygen_coordinates - previous_tail_coordinates
                            )
                            direction_from_substituent = _normalize(
                                capping_substituent_coordinates
                                - previous_tail_coordinates
                            )
                            if (
                                direction_from_oxygen is not None
                                and direction_from_substituent is not None
                            ):
                                direction_trigonal = _normalize(
                                    -(
                                        direction_from_oxygen
                                        + direction_from_substituent
                                    )
                                )
                        if direction_trigonal is not None:
                            direction_candidates.insert(0, direction_trigonal)

                        proline_ca_local_index = group_data[
                            "atom_name_to_local_index"
                        ].get("CA", None)
                        proline_cd_local_index = group_data[
                            "atom_name_to_local_index"
                        ].get("CD", None)
                        previous_alpha_local_index = previous_name_to_index.get(
                            "CA", None
                        )
                        ...
                        for direction in direction_candidates:
                            ...
                            if angle_oxygen is not None and (
                                angle_oxygen < 105.0 or angle_oxygen > 135.0
                            ):
                                continue
                            if angle_substituent is not None and (
                                angle_substituent < 105.0 or angle_substituent > 135.0
                            ):
                                continue

                            candidate_translation = (
                                candidate_target - local_coordinates[head_index]
                            )
                            candidate_coordinates = (
                                local_coordinates + candidate_translation
                            )
                            for rotation_matrix in orientation_matrices:
                                ...
                                local_score = (
                                    _minimum_nonbonded_heavy_distance_between_groups(
                                ...
                                if angle_substituent is not None:
                                    geometry_penalty += 0.5 * abs(
                                        angle_substituent - 117.0
                                    )
                                ...
                                if proline_cd_local_index is not None:
                                    ...
                                    if angle_c_n_cd is not None:
                                        geometry_penalty += 0.2 * abs(
                                            angle_c_n_cd - 122.0
                                        )

                                if (
                                    oxygen_coordinates is not None
                                    and proline_ca_local_index is not None
                                ):
                                    ...
                                    if dihedral_ocnca is not None:
                                        geometry_penalty += 0.3 * abs(
                                            _wrap_degrees(dihedral_ocnca)
                                        )

                                if (
                                    previous_alpha_coordinates is not None
                                    and proline_ca_local_index is not None
                                ):
                                ...
                                objective_score = (
                                    local_score - 0.0006 * geometry_penalty
                                )
                                if objective_score > best_objective_score + 1.0e-10:
                                    ...
                                    abs(objective_score - best_objective_score)
                                    <= 1.0e-10
                                ):
                                    if local_score > best_local_score + 1.0e-10:
                                        ...
                                        if (
                                            geometry_penalty
                                            < best_geometry_penalty - 1.0e-10
                                        ):
                                            best_geometry_penalty = geometry_penalty
                                            best_candidate_coordinates = (
                                                rotated_coordinates
                                            )
                                elif abs(local_score - best_local_score) <= 1.0e-10:
                                    if (
                    ...
                    if atom_index > 0 and (not skip_local_cn_scan):
                        axis = _normalize(
                            target_coordinates - previous_tail_coordinates
                        )
                        if axis is not None:
                            ...
                            for angle in np.linspace(
                                0.0, 2.0 * np.pi, num=24, endpoint=False
                            ):
                                rotation = _rotation_matrix_from_axis_angle(axis, angle)
                                trial_coordinates = _apply_rotation(
                                    group_coordinates, rotation, target_coordinates
                                )
                            ...
                            if best_score < 0.22:
                                ...
                                tilt_angles = np.deg2rad(
                                    np.array(
                                        [-50.0, -35.0, -20.0, 0.0, 20.0, 35.0, 50.0],
                                        dtype=float,
                                    )
                                )
                                for angle_u in tilt_angles:
                                    rotation_u = _rotation_matrix_from_axis_angle(
                                        axis_u, angle_u
                                    )
                                    tilted_u = _apply_rotation(
                                        best_coordinates, rotation_u, target_coordinates
                                    )
                                    for angle_v in tilt_angles:
                                        rotation_v = _rotation_matrix_from_axis_angle(
                                            axis_v, angle_v
                                        )
                                        tilted_uv = _apply_rotation(
                                            tilted_u, rotation_v, target_coordinates
                                        )
                                        score = _minimum_nonbonded_heavy_distance(
                    ...
                    group_coordinates = local_coordinates + np.array(
                        [fallback_anchor_x, 0.0, 0.0], dtype=float
                    )

            # Keep peptide-bond geometry intact; clashes are handled by torsion scan + warning.

            fallback_anchor_x = max(
                fallback_anchor_x, float(np.max(group_coordinates[:, 0])) + 0.25
            )
            previous_tail_coordinates = None
            ...
            for atom_index_1_local, atom_index_2_local in group_data[
                "bonded_atom_indices"
            ]:
        ...
        temp_item.topology.groups["group_name"] = [
            group_data["group_name"] for group_data in groups_data
        ]
        temp_item.topology.groups["group_type"] = [
            group_data["group_type"] for group_data in groups_data
        ]
        temp_item.topology.groups["molecule_index"] = 0

        temp_item.topology.components["component_id"] = np.arange(n_groups)
        temp_item.topology.components["component_name"] = [
            group_data["group_name"] for group_data in groups_data
        ]
        temp_item.topology.components["component_type"] = [
            group_data["group_type"] for group_data in groups_data
        ]
        ...
        temp_item.topology.rebuild_components(
        ...
        heavy_mask_final, bonded_matrix_final = _prepare_nonbonded_heavy_distance_full(
            atom_types, all_bonds
        )
        ...
        temp_item.structures.coordinates = puw.standardize(
            puw.quantity(coordinates[np.newaxis, :, :], "nm")
        )
