...
def add_missing_hydrogens(
    ...
    if engine == "OpenMM":
        ...
        temp_molecular_system = convert(
            molecular_system, to_form="openmm.Modeller", skip_digestion=True
        )
        temp_molecular_system.addHydrogens(pH=pH)
        ...
        if form_out == "molsysmt.MolSys":
            output_molecular_system.topology.rebuild_entities(
            ...
            output_molecular_system.rebuild_entities(
                redefine_indices=True,
                redefine_ids=True,
                redefine_names=True,
                redefine_types=True,
        ...
        temp_molecular_system = convert(
            molecular_system, to_form="pdbfixer.PDBFixer", skip_digestion=True
        )
        ...
        for g in range(n_groups):
            g_atoms = topo.atoms[topo.atoms["group_index"] == g]
            group_name_to_idx[g] = dict(
                zip(g_atoms["atom_name"], g_atoms.index.tolist())
            )
        ...
        for g in range(n_groups):
            ...
            if gname in ("ACE", "NME"):
                ...
                present_hs = {
                    a
                    for a in g_atoms["atom_name"]
                    if a.startswith("H")
                    or (len(a) >= 2 and a[0].isdigit() and a[1] == "H")
                }
                name_to_idx = group_name_to_idx[g]
                # Build parent → [H, ...] map from template bonds
                capping_parent_to_hs = {}
                for b1, b2 in template["bonds"]:
                    b1h = b1.startswith("H") or (
                        len(b1) >= 2 and b1[0].isdigit() and b1[1] == "H"
                    )
                    b2h = b2.startswith("H") or (
                        len(b2) >= 2 and b2[0].isdigit() and b2[1] == "H"
                    )
                    if b1h and not b2h:
                ...
                for par_name, h_names in capping_parent_to_hs.items():
                    ...
                    bond_len = (
                        h_bond_length("C")
                        if par_name in ("CH3", "C")
                        else h_bond_length(par_name[0])
                    )
            ...
            present_hs = {
                a
                for a in present_names
                if a.startswith("H") or (len(a) >= 2 and a[0].isdigit() and a[1] == "H")
            }
            ...
            expected_hs = get_expected_hydrogens(
                ...
                pH=pH,
                is_n_terminal=is_n_term,
                is_c_terminal=is_c_term,
            ...
            for variant in db["topology"]:
                ...
                if present_heavy_set <= variant_heavy:
                    ...
                    if (
                        best_extra is None
                        or h_overlap > best_h_overlap
                        or (h_overlap == best_h_overlap and extra < best_extra)
                    ):
                        best_extra = extra
            ...
            for b1, b2 in best_variant["bonds"]:
                h, par = None, None
                b1_is_h = b1.startswith("H") or (
                    len(b1) >= 2 and b1[0].isdigit() and b1[1] == "H"
                )
                b2_is_h = b2.startswith("H") or (
                    len(b2) >= 2 and b2[0].isdigit() and b2[1] == "H"
                )
            ...
            for par_name, h_names in parent_to_hs.items():
                ...
                hybridization = _infer_hybridization(
                    par_name, variant_heavy_neigh, lookup
                )
        ...
        if new_atom_info:
            native_out = append_atoms_to_molsys(
                native_ms, new_atom_info, new_bonds_info
            )
        else:
