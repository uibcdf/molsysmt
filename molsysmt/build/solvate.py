...
from molsysmt._private.smonitor import ArgumentError, InternalAlgorithmError
...
def _build_tiled_water(water_tile, n_x, n_y, n_z, tile_nm):
    ...
    template_xyz = puw.get_value(water_tile.structures.coordinates, to_unit="nm")[
        0
    ]  # (n_atoms_tile, 3)
    tiled_xyz = (
        template_xyz[np.newaxis] + offsets[:, np.newaxis]
    )  # (n_tiles, n_atoms_tile, 3)
    ...
    grp_offsets = np.repeat(
        np.arange(n_tiles, dtype=np.int64) * n_groups_tile, n_atoms_tile
    )
    ...
    mol_offsets = np.repeat(
        np.arange(n_tiles, dtype=np.int64) * n_groups_tile, n_groups_tile
    )
    ...
    bond_offsets = np.repeat(
        np.arange(n_tiles, dtype=np.int64) * n_atoms_tile, n_bonds_tile
    )
...
def _build_ions(ion_name, atom_name, atom_type, positions):
    ...
    new_topo.atoms["atom_id"] = pd.array(
        [str(i + 1) for i in range(n_ions)], dtype="string"
    )
    ...
    new_topo.atoms["group_index"] = pd.array(
        np.arange(n_ions, dtype=np.int64), dtype="Int64"
    )
    new_topo._set_component_indices(np.arange(n_ions, dtype=np.int64))
    new_topo.atoms["chain_index"] = pd.array(
        np.zeros(n_ions, dtype=np.int64), dtype="Int64"
    )

    new_topo.groups["group_id"] = pd.array(
        [str(i + 1) for i in range(n_ions)], dtype="string"
    )
    ...
    new_topo.groups["molecule_index"] = pd.array(
        np.arange(n_ions, dtype=np.int64), dtype="Int64"
    )
...
def _place_in_box_origin_convention(item):
    ...
    coordinates = np.asarray(
        puw.get_value(
            get(item, element="system", coordinates=True, skip_digestion=True),
            to_unit="nm",
        ),
        dtype=float,
    )
...
def solvate(
    ...
    if engine == "OpenMM":
        ...
        component_indices, component_names = get(
            molecular_system,
            element="component",
            component_index=True,
            component_name=True,
        )
        molecule_indices, molecule_names = get(
            molecular_system,
            element="molecule",
            molecule_index=True,
            molecule_name=True,
        )
        chain_indices, chain_ids, chain_names = get(
            molecular_system,
            element="chain",
            chain_index=True,
            chain_id=True,
        ...
        aux_water_model, aux_forcefield = get(
            molecular_system, water_model=True, forcefield=True
        )
        ...
        if box_shape == "truncated octahedral":
            ...
            vectors = (
                Vec3(1.0, 0, 0),
                Vec3(1.0 / 3.0, 2.0 * np.sqrt(2.0) / 3.0, 0.0),
                Vec3(-1.0 / 3.0, np.sqrt(2.0) / 3.0, np.sqrt(6.0) / 3.0),
            )
        ...
        assign_selection_to_new_chain(
            tmp_item, selection='group_type in ["water","ion"]'
        )
        ...
        max_size = max(
            ...
            - min((pos[i] for pos in pdbfixer.positions))
            for i in range(3)
        )
        ...
        if cation not in _ion_info:
            raise NotImplementedError(
                f"cation={cation!r} is not supported with engine='MolSysMT'."
            )
        if anion not in _ion_info:
            raise NotImplementedError(
                f"anion={anion!r} is not supported with engine='MolSysMT'."
            )
        ...
        solute = translate(
            solute, translation=puw.quantity(shift, "nm"), in_place=False
        )
        ...
        if np.any(outside):
            ...
            # The removal selection resolves this local via @outside_mol_indices.
            outside_mol_indices = np.unique(  # noqa: F841
                get(
                    all_water,
                    element="atom",
                    selection=outside_atom_indices,
                    molecule_index=True,
                    skip_digestion=True,
                )
            ).tolist()
            all_water = remove(
                all_water,
                selection="molecule_index in @outside_mol_indices",
                skip_digestion=True,
            )
        ...
        if n_ions_total > 0:
            ...
            ion_ion_cutoff_nm = (
                0.05  # 0.5 Å — avoid placing two ions on top of each other
            )
            ...
            solvated = remove(
                solvated,
                selection="molecule_index in @accepted_mol_indices",
                skip_digestion=True,
            )
