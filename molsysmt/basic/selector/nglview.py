...
def indices_to_selection(molecular_system, indices, element="atom"):
    ...
    if element == "atom":
        ...
        if np.all(np.isin(np.unique(chain_names), [" ", None])):
            ...
            output_string = " ".join(
                [str(ii) + ":" + str(jj) for ii, jj in zip(group_ids, chain_names)]
            )
    elif element == "chain":
        from molsysmt import get

        chain_names = get(
            molecular_system, element="chain", selection=indices, chain_id=True
        )
        output_string = " ".join([":" + ii for ii in chain_names])
    else:
        raise NotImplementedMethodError(
            caller="molsysmt.basic.selector.nglview.indices_to_selection"
        )
