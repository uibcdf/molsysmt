...
packages = sort_packages(aggregate_by_package(rows, root=args.root, depth=args.depth), mode="coverage")

print("\nCoverage by package\n")
print(f"{'Cover':>7}  {'Stmts':>7}  {'Miss':>7}  {'Files':>7}  Package")
print("-" * 110)
for row in packages:
    print(f"{row['percent']:6.1f}%  {row['statements']:7d}  {row['missing']:7d}  {row['files']:7d}  {row['package']}")
