...
def test_exact_public_record_is_accepted(monkeypatch, tmp_path):
    _set_distribution(monkeypatch, tmp_path, editable=False)
    _write_conda_record(
        tmp_path,
    ...
    validate_conda_staging._require_conda_install(
        "molsysmt", "0.22.0", require_public_provenance=True
    )


def test_staging_record_is_rejected_from_public_gate(monkeypatch, tmp_path):
    _set_distribution(monkeypatch, tmp_path, editable=False)
    _write_conda_record(
        tmp_path,
        channel="https://conda.anaconda.org/uibcdf/label/staging/linux-64",
    ...
    with pytest.raises(RuntimeError, match="incomplete public artifact identity"):
        validate_conda_staging._require_conda_install(
            "molsysmt", "0.22.0", require_public_provenance=True
        )


def test_public_gate_rejects_mismatched_channel_alias(monkeypatch, tmp_path):
    _set_distribution(monkeypatch, tmp_path, editable=False)
    _write_conda_record(
        tmp_path,
        channel="https://conda.anaconda.org/uibcdf/linux-64",
    ...
    with pytest.raises(RuntimeError, match="non-public channel"):
        validate_conda_staging._require_conda_install(
            "molsysmt", "0.22.0", require_public_provenance=True
        )
