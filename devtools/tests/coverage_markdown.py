...
parser = argparse.ArgumentParser(
    description="Generate Markdown and JSON coverage summaries."
)
...
rows_sorted = sorted(rows, key=lambda x: (-x["missing"], x["percent"], x["path"]))
packages_top = sort_packages(
    aggregate_by_package(rows, root=args.root, depth=1), mode="coverage"
)
packages = sort_packages(
    aggregate_by_package(rows, root=args.root, depth=args.package_depth),
    mode="coverage",
)
...
hotspot_rows = [
    [r["path"], f"{r['percent']:.1f}%", str(r["statements"]), str(r["missing"])]
...
content.append(
    markdown_table(
        ["Package", "Coverage", "Map", "Grade", "Files", "Statements", "Missing"],
        package_rows_top,
    )
)
content.append("\n\n## Coverage by package\n")
...
content.append(
    markdown_table(["File", "Coverage", "Statements", "Missing"], hotspot_rows)
)
content.append("")
