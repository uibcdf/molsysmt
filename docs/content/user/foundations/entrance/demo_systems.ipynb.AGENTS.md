...
1. **Format Policy**:
   - MUST remain a Jupyter Notebook (`.ipynb`) containing Python code cells that dynamically query `molsysmt.systems.categories` and `molsysmt.systems.info` to render category subsections, system subsubsections, and file-level explanation tables.
...
5. **Inviolable Catalog Architecture**:
   ...
   - **System Subsubsections (H4)**: Title, italic summary explanation, and `Keyword` specification (e.g. `msm.systems['Trp-Cage']`).
   - **File Explanation Tables**: 2-column HTML tables (`File String`, `Contents & Usage Explanation`) detailing exact contents and purpose for every file.
