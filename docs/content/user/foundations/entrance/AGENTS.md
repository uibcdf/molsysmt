# Section: The Entrance Directives (`AGENTS.md`)

This guide governs all content under `docs/content/user/foundations/entrance/`.

## 🧭 Subdirectory Purpose & Scope
Cover the first contact topics: mission statement, installation instructions, documentation navigation guide, toolbox overview, and bundled demo systems catalog.

## 📄 Pages List & Paired Micro-`AGENTS.md` Files
- `index.md` ➔ `index.AGENTS.md`: Group landing page with `toctree`.
- `what_is_molsysmt.md` ➔ `what_is_molsysmt.md.AGENTS.md`: What is MolSysMT, form-agnostic philosophy.
- `installation.md` ➔ `installation.AGENTS.md`: Installation via Conda and PyPI.
- `navigating_documentation.md` ➔ `navigating_documentation.md.AGENTS.md`: Guide to navigating the documentation and learning paths.
- `toolbox_overview.md` ➔ `toolbox_overview.md.AGENTS.md`: Overview of the API functional modules.
- `demo_systems.ipynb` ➔ `demo_systems.ipynb.AGENTS.md`: `molsysmt.systems` dynamic catalog auto-compiled from `molsysmt/systems.py`.

