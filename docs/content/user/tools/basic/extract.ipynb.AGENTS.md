# extract.ipynb Governance Guide

This document defines micro-governance rules for the `extract.ipynb` tutorial unit (`msm.extract`).
...
- **Cell Structure**:
  ...
  2. Intro block: Title `# Extract`, `versionadded 1.0.0`, collapsible `API documentation` box dropdown.
...
- **See Also**: Collapsible dropdown listing referenced tools in exact order of appearance (`convert`, `info`).
