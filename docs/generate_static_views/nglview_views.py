...
def get_repr_dict_from_msgs(msg_archive):
    ...
    for msg in msg_archive:
        ...
        if method == "loadFile":
            ...
            repr_dict[comp_idx][r_idx] = {"type": repr_type, "params": kwargs}
        ...
        for k, v in data.get("state", {}).items():
            if v.get("model_name") == "NGLModel":
                ...
                for msg in msgs:
                    msg["fire_embed"] = True
                    if (
                        msg.get("methodName") == "loadFile"
                        and msg.get("args")
                        and isinstance(msg["args"][0], dict)
                    ):
                        if not msg["args"][0].get("ext"):
                            msg["args"][0]["ext"] = msg.get("kwargs", {}).get(
                                "ext", "pdb"
                            )
...
molsys_181 = msm.convert(
    msm.systems["T4 lysozyme L99A"]["181l.h5msm"], selection='molecule_type=="protein"'
)
charge_groups = msm.physchem.get_charge(
    molsys_181, element="group", selection='molecule_type=="protein"'
)
v4 = msm.view(molsys_181, viewer="nglview")
msm.third_party.nglview.set_color_by_value(
    v4, values=charge_groups, selection='molecule_type=="protein"', cmap="bwr"
)
...
try:
    ...
    traj_molsys = msm.convert(
        [nv.datafiles.GRO, nv.datafiles.XTC], to_form="molsysmt.MolSys"
    )
    contact_map = msm.structure.get_contacts(
        traj_molsys,
        selection='molecule_type=="protein" and atom_name=="CA"',
    ...
    fig_plotly.write_html(
        str(static_ngl / "nglview_contact_map.html"), include_plotlyjs="cdn"
    )
...
vq3 = msm.view(
    molsys_181,
    selection="all within 0.7 nm of molecule_name=='BENZENE'",
    viewer="nglview",
)
...
view = msm.convert(molsys, to_form="nglview.NGLWidget")
msm.third_party.nglview.add_contacts(
    view,
    atom_pairs=[[10, 50], [20, 80], [30, 100]],
    color="#3498DB",
    radius="0.3 angstroms",
)
...
view = msm.convert(molsys, to_form="nglview.NGLWidget")
msm.third_party.nglview.add_cylinders(
    view,
    bottom="atom_index==0",
    top="atom_index==10",
    color="#2ECC71",
...
view = msm.convert(molsys, to_form="nglview.NGLWidget")
msm.third_party.nglview.set_color(
    view, color="#E67E22", selection='molecule_type=="protein"'
)
...
msm.third_party.nglview.set_color_by_value(
    view, values=values, selection='molecule_type=="protein"', cmap="bwr_r"
)
...
view = msm.convert(molsys, to_form="nglview.NGLWidget")
msm.third_party.nglview.show_as_surface(
    view, selection='molecule_type=="protein"', opacity=0.7
)
