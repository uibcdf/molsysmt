...
protein = msm.convert(
    msm.systems["chicken villin HP35"]["chicken_villin_HP35.h5msm"],
    to_form="molsysmt.MolSys",
)
protein_aligned = msm.structure.align_principal_axes(
    protein, axes=[[1, 0, 0], [0, 1, 0], [0, 0, 1]]
)
v1 = msm.view(protein_aligned)
v1.export.html(
    str(views_dir / "tools_structure_align_principal_axes_1.html"),
...
molsys_popc = msm.structure.align_principal_axes(
    molsys_popc, axes=[[0, 1, 0], [0, 0, 1], [1, 0, 0]]
)
molsys_popc = msm.structure.center(
    molsys_popc, selection="all", center_of_selection='atom_name=="P"'
)
...
molsys_villin = msm.convert(
    msm.systems["chicken villin HP35"]["chicken_villin_HP35.h5msm"],
    to_form="molsysmt.MolSys",
)
v4 = msm.view(molsys_villin)
v4.export.html(
    str(views_dir / "tools_structure_rotate_1.html"), background="transparent"
)
...
v5.export.html(
    str(views_dir / "tools_structure_rotate_2.html"), background="transparent"
)
...
v6.export.html(
    str(views_dir / "tools_structure_set_dihedral_angles_1.html"),
    background="transparent",
)
print(" -> Saved tools_structure_set_dihedral_angles_1.html")

print("7/9: Generating tools_structure_set_dihedral_angles_2.html...")
molsys_set = msm.structure.set_dihedral_angles(
    molsys_dih, dihedral_quartets=phi_quartets[1], angles="60.0 degrees"
)
v7 = msm.view(molsys_set)
v7.export.html(
    str(views_dir / "tools_structure_set_dihedral_angles_2.html"),
    background="transparent",
)
...
v8.export.html(
    str(views_dir / "tools_structure_shift_dihedral_angles_1.html"),
    background="transparent",
)
...
molsys_shifted = msm.structure.shift_dihedral_angles(
    molsys_dih, dihedral_quartets=phi_quartets[1], shifts="90.0 degrees"
)
v9 = msm.view(molsys_shifted)
v9.export.html(
    str(views_dir / "tools_structure_shift_dihedral_angles_2.html"),
    background="transparent",
)
