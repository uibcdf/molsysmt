...
The statement needs four things, none of which is prose about the software:
...
**4. The reproducibility package.** Which release, which DOI, which environment, and
...
report is the strongest available contribution, and that an Application Note is too
short. Both are judgements from one reading of one repository by one reviewer who is not
...
Covers the argument, the claimed-API list, the venue, and the reproducibility package.
...
1. A document in `devguide/` states the contribution in one sentence, the claimed APIs,
   the venue, and the reproducibility package.
