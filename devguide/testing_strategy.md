...
MolSysMT now has one CPU implementation: the bundled Rust extension. The
temporary `--molsysmt-kernel` migration selector and two-backend parity suite
were removed after the final oracle artifact was preserved. Scientific-truth
tests remain independent of implementation and are the maintained correctness
authority.
