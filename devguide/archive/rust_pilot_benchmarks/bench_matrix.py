...
print(
    f"{'kernel (profile)':38s} {'N':>6s}  parity   {'COLD nb':>9s} {'COLD rs':>9s}   {'WARM nb':>8s} {'WARM rs':>8s}"
)
print("-" * 100)
...
def row(label, n, parity, cnb, crs, wnb, wrs):
    ...
    cold_rust_total += crs
    print(
        f"{label:38s} {n:6d}  {str(parity):5s}   {cnb * 1000:8.1f} {crs * 1000:8.1f}   {wnb * 1000:7.2f} {wrs * 1000:7.2f}"
    )
...
for n in (500, 2000, 4000):
    ...
    # regular arithmetic
    if first:
        cnb = cold(pairwise_nb, c)
        crs = cold(rust.pairwise_sqdistances, c)
    else:
        cnb = crs = 0.0
    p = np.allclose(pairwise_nb(c), rust.pairwise_sqdistances(c), atol=1e-9)
    row(
        "pairwise_sqdist (regular)",
        n,
        p,
        cnb,
        crs,
        best(pairwise_nb, c),
        best(rust.pairwise_sqdistances, c),
    )
    # transcendental
    if first:
        cnb = cold(coulomb_nb, c, q)
        crs = cold(rust.coulomb_potential, c, q)
    else:
        cnb = crs = 0.0
    p = np.allclose(coulomb_nb(c, q), rust.coulomb_potential(c, q), atol=1e-9)
    row(
        "coulomb (transcendental)",
        n,
        p,
        cnb,
        crs,
        best(coulomb_nb, c, q),
        best(rust.coulomb_potential, c, q),
    )
    first = False
...
for n in (5000, 20000):
    ...
    row(
        "neighbor_counts (branchy)",
        n,
        p,
        cnb,
        crs,
        best(neighbor_counts_nb, c, 0.5),
        best(rust.neighbor_counts, c, 0.5),
    )

print("-" * 100)
print(
    f"AGGREGATE COLD across distinct kernels:  numba(JIT)={cold_numba_total * 1000:8.1f} ms   "
...
# warm both parallels
coulomb_nb_parallel(c, q)
rust.coulomb_potential_parallel(c, q)
p = np.allclose(
    coulomb_nb_parallel(c, q), rust.coulomb_potential_parallel(c, q), atol=1e-9
)
...
print(
    f"  parallel: numba={par_nb * 1000:7.2f} ms   rust={par_rs * 1000:7.2f} ms   "
