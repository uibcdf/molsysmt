...
import molsysmt._rust as rust  # noqa: E402
import numpy as np  # noqa: E402

import molsysmt as msm  # noqa: E402
from molsysmt import lib as msmlib  # noqa: E402
from molsysmt import systems  # noqa: E402
...
coords = np.ascontiguousarray(
    puw.get_value(msm.get(m, coordinates=True), "nm"), dtype=np.float64
)
# Replicate atoms to reach a meaningful N for warm timing (real coordinates, shifted).
reps = 24  # 62 * 24 = 1488 atoms
big = np.concatenate(
    [coords[0] + np.array([0.11 * i, 0.07 * i, 0.05 * i]) for i in range(reps)], axis=0
)
coords = big[np.newaxis, :, :].copy()
n_atoms = coords.shape[1]

ortho_box = np.array(
    [[[6.0, 0.0, 0.0], [0.0, 6.0, 0.0], [0.0, 0.0, 6.0]]], dtype=np.float64
)
tric_box = np.array(
    [[[6.0, 0.0, 0.0], [1.2, 6.0, 0.0], [0.8, 0.6, 6.0]]], dtype=np.float64
)

prod = msmlib.structure.get_mic_distances_single_system

print(
    f"Production kernel: get_mic_distances_single_system   N_atoms={n_atoms}  (1 structure)"
)
print("-" * 78)
for name, box in (("orthogonal", ortho_box), ("triclinic", tric_box)):
    # cold: first call of each (Numba compiles here)
    t0 = time.perf_counter()
    nb_out = prod(coords, box)
    t1 = time.perf_counter()
    t2 = time.perf_counter()
    rs_out = rust.get_mic_distances_single_system(coords, box)
    ...
    print(
        f"{name:11s}  parity={parity}  max|diff|={maxdiff:.1e}   "
