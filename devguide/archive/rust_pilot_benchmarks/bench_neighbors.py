...
import molsysmt.configure as config  # noqa: E402
...
print(
    f"{'case':28s} {'parity':7s} {'COLD nb':>10s} {'COLD rs':>9s} {'WARM nb':>9s} {'WARM rs':>9s}"
)
print("-" * 82)
...
for ns, na, label in ((1, 8000, "1 struct x 8000"), (50, 2000, "50 struct x 2000")):
    ...
    kw = dict(
        query_coords=q, box=b, cutoff=0.6, exclude_self=True, sort_by_distance=True
    )

    t0 = time.perf_counter()
    out_nb = rb.neighbor_list_csr_multi(**kw, backend="numba")
    t1 = time.perf_counter()
    t2 = time.perf_counter()
    out_rs = rb.neighbor_list_csr_multi(**kw, backend="rust")
    t3 = time.perf_counter()
    parity = (
        np.array_equal(out_nb[0], out_rs[0])
        and np.array_equal(out_nb[1], out_rs[1])
    ...
    print(
        f"{label:28s} {str(parity):7s} {(t1 - t0) * 1000:9.1f}ms {(t3 - t2) * 1000:8.1f}ms "
