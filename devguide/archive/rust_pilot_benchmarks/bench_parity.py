...
print(
    f"fibonacci_sphere_points  parity={np.allclose(py, rs, atol=1e-12)}  "
...
t0 = time.perf_counter()
nb_cold = pairwise_sqdist_numba(coords)
t1 = time.perf_counter()
t2 = time.perf_counter()
rs_cold = rust.pairwise_sqdistances(coords)
t3 = time.perf_counter()

print(
    f"pairwise_sqdistances     parity={np.allclose(rs_cold, nb_cold, atol=1e-9)}  "
...
print(
    f"  N=2000  COLD  numba(JIT)={(t1 - t0) * 1000:9.1f} ms   rust={(t3 - t2) * 1000:9.1f} ms"
)
print(
    f"  N=2000  WARM  numba     ={nb_warm * 1000:9.1f} ms   rust={rs_warm * 1000:9.1f} ms"
)
print("DONE")
