...
## Addendum — 2026-08-19, third

[#187](https://github.com/uibcdf/molsysmt/issues/187) was rescoped after the commit that
produced its evidence was traced. It was filed above as *"Stable public docstrings
describe parameters with autogenerated placeholders"*, a medium documentation defect. It
is now *"Validators that check form instead of intent admit conforming emptiness"*, high.

The originating commit is `30e3119a5`, 2026-08-18 — one day before this audit: 1,495
files, 184,251 insertions, introducing all 8,810 `Resulting object in object form`
placeholders and 953 `Argument <name>.` ones. `validate_docstrings.py` passed on every
one of them, correctly, because what it checks was satisfied.

That reframes the finding. The placeholders are the instance; the criterion is the cause.
A gate that encodes *form* can be satisfied by conforming output, and generation scales
exactly to the gate and not one step past it. The entry now carries a classification of
all thirteen gates on that axis — five intent-checking, three structural contracts where
form is the subject, five that verify a record is complete and are silent on whether it
says anything — and that table is maintained in
[`../../devtools_and_ci.md`](../../devtools_and_ci.md) with the rule in the root
`AGENTS.md`.

Two of the five silent gates are not about documentation and were not visible from this
assessment as written: `validate_scientific_evidence.py` confirms that a cited test is
*defined*, not that it passes, and `validate_devguide.py` requires a `guard` to name a
path that exists. Every entry this audit opened will satisfy the second one whether or
not its guard would catch the defect.

This also qualifies a claim made in *What was verified and holds*. The governance
apparatus is as strong as recorded there, and the audit's own method depended on it —
claims were checked by executing registries rather than by reading them. The
qualification is that the apparatus is exactly as strong as its weakest criterion, and
one criterion has now been exercised at a scale of 184,251 lines in a day.
